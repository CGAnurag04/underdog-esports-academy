...
(function () {
  ...
  // ---------------------------------------------------------------------------
  // ===========================================================================
// UPGRADED AIM & REFLEX SPEED GYM SUITE
// 5 Reaction Modes, Personal Records KPI HUD, Latency Ladder & Hardware Tuning
// ===========================================================================
...
class ReflexTrainer {
  constructor(arenaId, statsId) {
    ...
    this.mode = 'visual'; // 'visual' | 'gloo' | 'targets' | 'audio' | 'blitz'
    this.round = 0;
    this.maxRounds = 5;
    this.reactionTimes = [];
    this.spawnTime = 0;
    ...
    // Target flicking / Blitz stats
    this.hits = 0;
    this.misses = 0;
    this.blitzTimeLeft = 30;
    this.blitzInterval = null;

    // Web Audio Context for synthesized sound feedback
    this.audioCtx = null;

    // Personal Records from localStorage
    this.records = this.loadRecords();

    this.bindEvents();
    this.updateRecordHUD();
  }

  // --- AUDIO SYNTHESIS (Zero External File Dependencies) ---
  initAudio() {
    if (!this.audioCtx) {
  ...
  playTone(freq = 600, duration = 0.08, type = 'sine', vol = 0.15) {
    try {
      this.initAudio();
  ...
  playSuccessChime() {
    ...
    setTimeout(() => this.playTone(1320, 0.15, 'sine', 0.2), 60);
  }

  playFailBuzz() {
    this.playTone(180, 0.18, 'sawtooth', 0.25);
  }
  ...
  playSniperShot() {
    try {
      ...
      gain.connect(this.audioCtx.destination);
      noise.start();
      this.playTone(120, 0.15, 'square', 0.25);
    } catch (e) {}
  }

  playGlooBlock() {
    this.playTone(400, 0.08, 'triangle', 0.3);
    setTimeout(() => this.playTone(800, 0.14, 'sine', 0.25), 40);
  }

  playTargetPop() {
    this.playTone(950, 0.06, 'triangle', 0.2);
  }

  // --- LOCAL STORAGE PERSONAL RECORDS ---
  loadRecords() {
    try {
      const saved = localStorage.getItem('underdog_reflex_records');
      if (saved) return JSON.parse(saved);
    } catch (e) {}
    return {
      bestVisual: null,
  ...
  saveRecords() {
    try {
      localStorage.setItem('underdog_reflex_records', JSON.stringify(this.records));
      this.updateRecordHUD();
    } catch (e) {}
  }
  ...
  resetRecords() {
    ...
    try {
      localStorage.removeItem('underdog_reflex_records');
    } catch (e) {}
    this.updateRecordHUD();
  }

  updateRecordHUD() {
    const vEl = document.getElementById('kpiBestVisual');
    const gEl = document.getElementById('kpiBestGloo');
    const aEl = document.getElementById('kpiBestAccuracy');
    const tEl = document.getElementById('kpiReflexTier');

    if (vEl) vEl.textContent = this.records.bestVisual ? `${this.records.bestVisual} ms` : '---';
    if (gEl) gEl.textContent = this.records.bestGloo ? `${this.records.bestGloo} ms` : '---';
    if (aEl) aEl.textContent = this.records.bestAccuracy ? `${this.records.bestAccuracy}%` : '---';
    if (tEl) {
      tEl.textContent = this.records.tier || 'Unranked';
      if (this.records.tier.includes('God')) tEl.className = 'text-xs font-heading font-bold text-amber-400 uppercase tracking-wider';
      else if (this.records.tier.includes('Pro') || this.records.tier.includes('Ready')) tEl.className = 'text-xs font-heading font-bold text-emerald-400 uppercase tracking-wider';
      else tEl.className = 'text-xs font-heading font-bold text-primary uppercase tracking-wider';
    }
  }
  ...
  bindEvents() {
    ...
    // Spacebar listener for Gloo Wall / Quick Tap
    window.addEventListener('keydown', (e) => {
      if (e.code === 'Space' && this.isRunning && (this.mode === 'gloo' || this.mode === 'visual' || this.mode === 'audio')) {
        e.preventDefault();
        this.handleArenaClick({ target: this.arena, keyTrigger: true });
      }
    });
  }
  ...
  setMode(newMode) {
    ...
    document.querySelectorAll('.reflex-mode-btn').forEach(b => {
      if (b.getAttribute('data-mode') === newMode) {
        b.classList.add('bg-cyan-500/20', 'text-cyan-300', 'border-cyan-400', 'shadow-[0_0_15px_rgba(0,240,255,0.2)]');
        b.classList.remove('bg-slate-900', 'text-slate-400', 'border-slate-800');
      } else {
        b.classList.remove('bg-cyan-500/20', 'text-cyan-300', 'border-cyan-400', 'shadow-[0_0_15px_rgba(0,240,255,0.2)]');
        b.classList.add('bg-slate-900', 'text-slate-400', 'border-slate-800');
      }
    });

    this.renderModeArena();
  }

  renderModeArena() {
    if (!this.arena) return;
    this.arena.innerHTML = '';
    this.arena.className = 'aim-arena w-full min-h-[360px] rounded-2xl relative flex flex-col items-center justify-center cursor-pointer transition-all overflow-hidden select-none border border-slate-800';

    if (this.mode === 'visual') {
      this.arena.innerHTML = `
        <div class="text-center p-6 pointer-events-none animate-fade-in">
          <div class="w-16 h-16 rounded-full bg-cyan-500/10 border border-cyan-400/40 flex items-center justify-center mx-auto mb-3 shadow-[0_0_20px_rgba(0,240,255,0.2)]">
            <span class="text-3xl">⚡</span>
          </div>
          <h4 class="text-lg font-heading font-black text-white uppercase tracking-wider">Human Benchmark: Pure Neural Reflex</h4>
          <p class="text-xs text-slate-400 mt-1 max-w-md mx-auto leading-relaxed">
            Measures raw synaptic eye-to-finger transmission latency. Click <strong>"Start Test"</strong>, wait for the red screen to turn vibrant green, then tap as fast as humanly possible!
          </p>
          <div class="flex items-center justify-center gap-3 mt-4 text-[11px] font-mono text-slate-500">
            <span>5 Rounds</span>
            <span>&bull;</span>
            <span>False Start Detection</span>
            <span>&bull;</span>
            <span>Spacebar or Tap</span>
          </div>
        </div>
      `;
    } else if (this.mode === 'gloo') {
      this.arena.innerHTML = `
        <div class="text-center p-6 pointer-events-none animate-fade-in">
          <div class="w-16 h-16 rounded-full bg-amber-500/10 border border-amber-400/40 flex items-center justify-center mx-auto mb-3 shadow-[0_0_20px_rgba(245,158,11,0.2)]">
            <span class="text-3xl">🛡️</span>
          </div>
          <h4 class="text-lg font-heading font-black text-amber-300 uppercase tracking-wider">0.15s Fast Gloo Wall Reflex Simulator</h4>
          <p class="text-xs text-slate-400 mt-1 max-w-md mx-auto leading-relaxed">
            Simulates a high-stakes 1v1 in Bermuda Peak. An enemy sniper is locked onto your head. The moment their muzzle flashes and you hear the shot, deploy your Gloo Wall within 200ms to block the fatal headshot!
          </p>
          <div class="flex items-center justify-center gap-3 mt-4 text-[11px] font-mono text-amber-400/80">
            <span>Sub-200ms Required</span>
            <span>&bull;</span>
            <span>Block / Knock Simulation</span>
            <span>&bull;</span>
            <span>Spacebar or Tap</span>
          </div>
        </div>
      ...
      this.arena.innerHTML = `
        <div class="text-center p-6 pointer-events-none animate-fade-in">
          <div class="w-16 h-16 rounded-full bg-rose-500/10 border border-rose-400/40 flex items-center justify-center mx-auto mb-3 shadow-[0_0_20px_rgba(244,63,94,0.2)]">
            <span class="text-3xl">🎯</span>
          </div>
          <h4 class="text-lg font-heading font-black text-white uppercase tracking-wider">Precision Headshot Flick Arena</h4>
          <p class="text-xs text-slate-400 mt-1 max-w-md mx-auto leading-relaxed">
            10 dynamic targets of varying size (Headshots 26px vs Upper Torso 44px) will spawn across the arena. Flick your crosshair and tap them instantly to measure your Accuracy % and Time-to-Damage (TTD).
          </p>
          <div class="flex items-center justify-center gap-3 mt-4 text-[11px] font-mono text-rose-400/80">
            <span>10 High-Speed Targets</span>
            <span>&bull;</span>
            <span>Accuracy & Miss Tracking</span>
            <span>&bull;</span>
            <span>Mouse / Touch Crosshair</span>
          </div>
        </div>
      `;
      ...
      this.arena.innerHTML = `
        <div class="text-center p-6 pointer-events-none animate-fade-in">
          <div class="w-16 h-16 rounded-full bg-purple-500/10 border border-purple-400/40 flex items-center justify-center mx-auto mb-3 shadow-[0_0_20px_rgba(168,85,247,0.2)]">
            <span class="text-3xl">🔊</span>
          </div>
          <h4 class="text-lg font-heading font-black text-purple-300 uppercase tracking-wider">Blind Auditory Reaction Drill</h4>
          <p class="text-xs text-slate-400 mt-1 max-w-md mx-auto leading-relaxed">
            Auditory neural signals reach the brainstem in 8-10ms (30-50ms faster than visual processing). Focus purely on your ears. Tap instantly when you hear the gunshot sound!
          </p>
          <div class="flex items-center justify-center gap-3 mt-4 text-[11px] font-mono text-purple-400/80">
            <span>Pure Sound Cue</span>
            <span>&bull;</span>
      ...
      this.arena.innerHTML = `
        <div class="text-center p-6 pointer-events-none animate-fade-in">
          <div class="w-16 h-16 rounded-full bg-emerald-500/10 border border-emerald-400/40 flex items-center justify-center mx-auto mb-3 shadow-[0_0_20px_rgba(16,185,129,0.2)]">
            <span class="text-3xl">⏱️</span>
          </div>
          <h4 class="text-lg font-heading font-black text-emerald-300 uppercase tracking-wider">30-Second Rapid Blitz Challenge</h4>
          <p class="text-xs text-slate-400 mt-1 max-w-md mx-auto leading-relaxed">
            High-intensity tournament pressure test! 3 targets are continuously active on screen. Eliminate as many targets as you can within 30 seconds to calculate your KPS (Kills Per Second).
          </p>
          <div class="flex items-center justify-center gap-3 mt-4 text-[11px] font-mono text-emerald-400/80">
            <span>30s Countdown</span>
            <span>&bull;</span>
            <span>Continuous Spawns</span>
            <span>&bull;</span>
    ...
    if (this.statsEl) {
      this.statsEl.innerHTML = `
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs text-slate-400 font-mono">
          <span>Active Mode: <strong class="text-white uppercase">${this.getModeName()}</strong></span>
          <span>Click <strong>"Start Test"</strong> to begin session</span>
        </div>
      `;
    }
  }
  ...
  getModeName() {
    switch(this.mode) {
      case 'visual': return '⚡ Pure Neural Reflex';
      case 'gloo': return '🛡️ 0.15s Gloo Wall Reflex';
      case 'targets': return '🎯 Precision Headshots';
      case 'audio': return '🔊 Auditory Reaction';
      case 'blitz': return '⏱️ 30s Rapid Blitz';
      default: return 'Reaction Test';
    }
  }
  ...
  startTest() {
    ...
    if (this.mode === 'visual') {
      this.maxRounds = 5;
      this.startVisualRound();
    } else if (this.mode === 'gloo') {
      this.maxRounds = 5;
      this.startGlooRound();
    } else if (this.mode === 'targets') {
      this.maxRounds = 10;
      this.startTargetsRound();
    } else if (this.mode === 'audio') {
      this.maxRounds = 5;
      this.startAudioRound();
    } else if (this.mode === 'blitz') {
      this.startBlitzChallenge();
    }
  }
  ...
  // --- MODE 1: VISUAL (RED TO GREEN) ---
  startVisualRound() {
    this.round++;
    this.waitingForTrigger = true;
    this.triggerActive = false;

    this.arena.className = 'aim-arena w-full min-h-[360px] rounded-2xl relative flex flex-col items-center justify-center cursor-pointer select-none bg-rose-950/60 border-2 border-rose-600/70 shadow-[inset_0_0_50px_rgba(225,29,72,0.25)] transition-colors duration-150';
    this.arena.innerHTML = `
      <div class="text-center p-6 pointer-events-none animate-pulse">
        <div class="w-20 h-20 rounded-full bg-rose-600/20 border-2 border-rose-500 flex items-center justify-center mx-auto mb-3 shadow-[0_0_25px_rgba(244,63,94,0.4)]">
          <span class="text-4xl">🛑</span>
        </div>
        <h3 class="text-2xl sm:text-3xl font-heading font-black text-rose-300 uppercase tracking-wider">WAIT FOR GREEN...</h3>
        <p class="text-xs text-rose-200 mt-2 font-sub uppercase tracking-wider">Do NOT tap yet! Premature clicks count as false starts.</p>
        <div class="mt-4 inline-block px-3 py-1 rounded-full bg-rose-900/60 border border-rose-500/40 text-[11px] font-mono text-rose-200">
    ...
    if (this.statsEl) {
      this.statsEl.innerHTML = `
        <div class="flex items-center justify-between text-xs font-mono">
          <span class="text-rose-400">⚡ Round ${this.round}/${this.maxRounds}: Waiting for trigger...</span>
          <span class="text-slate-400">Keep finger hovering</span>
        </div>
      `;
    }

    const randomDelay = 1400 + Math.random() * 2600; // 1.4s - 4.0s
    this.timeoutId = setTimeout(() => {
      if (!this.isRunning || !this.waitingForTrigger) return;
      this.triggerVisualGreen();
    }, randomDelay);
  }
  ...
  triggerVisualGreen() {
    ...
    this.playTone(1050, 0.08, 'sine', 0.25);

    this.arena.className = 'aim-arena w-full min-h-[360px] rounded-2xl relative flex flex-col items-center justify-center cursor-pointer select-none bg-emerald-600 border-2 border-emerald-300 shadow-[0_0_60px_rgba(16,185,129,0.7)] transition-colors duration-75';
    this.arena.innerHTML = `
      <div class="text-center p-6 pointer-events-none animate-bounce">
        <div class="w-20 h-20 rounded-full bg-white flex items-center justify-center mx-auto mb-3 shadow-[0_0_30px_#ffffff]">
          <span class="text-4xl">⚡</span>
        </div>
        <h3 class="text-3xl sm:text-4xl font-heading font-black text-slate-950 uppercase tracking-wider">CLICK / TAP NOW!</h3>
        <p class="text-xs text-slate-950 font-heading font-black mt-2 uppercase tracking-wider">TAP ANYWHERE OR HIT SPACEBAR!</p>
      </div>
    `;
  }

  // --- MODE 2: GLOO WALL REFLEX ---
  startGlooRound() {
    this.round++;
    this.waitingForTrigger = true;
    this.triggerActive = false;

    this.arena.className = 'aim-arena w-full min-h-[360px] rounded-2xl relative flex flex-col items-center justify-center cursor-pointer select-none bg-slate-950 border-2 border-amber-500/40 tactical-grid transition-colors';
    this.arena.innerHTML = `
      <div class="text-center p-6 pointer-events-none">
        <div class="w-16 h-16 rounded-full bg-amber-500/10 border border-amber-400 flex items-center justify-center mx-auto mb-3 animate-pulse">
          <span class="text-3xl">⚠️</span>
        </div>
        <h3 class="text-xl sm:text-2xl font-heading font-black text-amber-300 uppercase tracking-wider">ENEMY SNIPER AIMING AT YOU...</h3>
        <p class="text-xs text-slate-400 mt-1 max-w-sm mx-auto">Watch for tracer & muzzle flash! Ready your gloo wall trigger.</p>
        <div class="mt-4 px-4 py-2 rounded-xl bg-slate-900 border border-amber-500/30 text-xs font-mono text-amber-300 inline-block">
          Round ${this.round} of ${this.maxRounds} &bull; Target: Sub-200ms
        </div>
      </div>
      <div class="absolute bottom-6 left-1/2 -translate-x-1/2 px-6 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-black font-heading font-black text-sm uppercase tracking-wider shadow-lg shadow-amber-500/30 pointer-events-none">
        🛡️ TAP HERE / SPACEBAR TO DEPLOY GLOO
      </div>
    `;

    const randomDelay = 1600 + Math.random() * 2800;
    this.timeoutId = setTimeout(() => {
      if (!this.isRunning || !this.waitingForTrigger) return;
      this.triggerGlooAlert();
    }, randomDelay);
  }
  ...
  triggerGlooAlert() {
    ...
    this.arena.className = 'aim-arena w-full min-h-[360px] rounded-2xl relative flex flex-col items-center justify-center cursor-pointer select-none bg-red-600 border-4 border-white shadow-[0_0_80px_rgba(239,68,68,0.9)]';
    this.arena.innerHTML = `
      <div class="text-center p-6 pointer-events-none animate-ping">
        <div class="text-5xl mb-2">💥</div>
        <h2 class="text-3xl sm:text-5xl font-heading font-black text-white uppercase tracking-wider">SNIPER SHOT FIRED!</h2>
        <p class="text-base font-heading font-black text-yellow-300 mt-2 uppercase">DROP GLOO WALL NOW!</p>
      </div>
    `;
  }

  // --- MODE 3: PRECISION TARGET FLICKING ---
  startTargetsRound() {
    this.round++;
    this.arena.className = 'aim-arena w-full min-h-[360px] rounded-2xl relative cursor-crosshair select-none bg-slate-950 border border-cyan-500/40 tactical-grid overflow-hidden';
    this.arena.innerHTML = '';
    ...
    if (this.statsEl) {
      this.statsEl.innerHTML = `
        <div class="flex items-center justify-between text-xs font-mono">
          <span class="text-cyan-400">🎯 Target ${this.round}/${this.maxRounds}</span>
          <span class="text-slate-400">Hits: <strong class="text-emerald-400">${this.hits}</strong> | Misses: <strong class="text-rose-400">${this.misses}</strong></span>
        </div>
      `;
    }
  }
  ...
  spawnFlickTarget() {
    ...
    const isHeadshot = Math.random() > 0.45; // 55% headshots
    const targetSize = isHeadshot ? 28 : 46;

    const minX = targetSize + 15;
    const maxX = Math.max(rect.width - targetSize - 15, targetSize + 30);
    const minY = targetSize + 15;
    const maxY = Math.max(rect.height - targetSize - 15, targetSize + 30);

    const x = Math.floor(minX + Math.random() * (maxX - minX));
    const y = Math.floor(minY + Math.random() * (maxY - minY));

    const target = document.createElement('div');
    target.className = 'aim-target absolute rounded-full flex items-center justify-center cursor-crosshair transition-transform active:scale-90 shadow-2xl';
    ...
    if (isHeadshot) {
      target.style.background = 'radial-gradient(circle, #ef4444 40%, #991b1b 90%)';
      target.style.border = '2px solid #ffffff';
      target.style.boxShadow = '0 0 20px rgba(239,68,68,0.7)';
      target.innerHTML = '<span class="text-[10px] font-black text-white pointer-events-none">HD</span>';
      target.setAttribute('data-target-type', 'headshot');
    } else {
      target.style.background = 'radial-gradient(circle, #00f0ff 30%, #2563eb 90%)';
      target.style.border = '2px solid #ffffff';
      target.style.boxShadow = '0 0 20px rgba(0,240,255,0.6)';
      target.innerHTML = '<span class="text-[9px] font-black text-black pointer-events-none">CR</span>';
      target.setAttribute('data-target-type', 'torso');
    }

    this.arena.appendChild(target);
    this.activeTargets = [target];
    this.spawnTime = performance.now();
    this.playTargetPop();
  }

  // --- MODE 4: AUDITORY REACTION ---
  startAudioRound() {
    this.round++;
    this.waitingForTrigger = true;
    this.triggerActive = false;

    this.arena.className = 'aim-arena w-full min-h-[360px] rounded-2xl relative flex flex-col items-center justify-center cursor-pointer select-none bg-slate-950 border border-purple-500/40 transition-colors';
    this.arena.innerHTML = `
      <div class="text-center p-6 pointer-events-none">
        <div class="w-20 h-20 rounded-full bg-purple-500/10 border-2 border-purple-500 flex items-center justify-center mx-auto mb-3 animate-pulse shadow-[0_0_30px_rgba(168,85,247,0.3)]">
          <span class="text-4xl">🎧</span>
        </div>
        <h3 class="text-2xl font-heading font-black text-purple-300 uppercase tracking-wider">CLOSE EYES / LISTEN FOR SHOT</h3>
        <p class="text-xs text-slate-400 mt-2 max-w-sm mx-auto">Pure auditory stimulus. Tap screen or press Spacebar the microsecond you hear the gunshot sound!</p>
        <div class="mt-4 px-3 py-1 rounded-full bg-purple-950/60 border border-purple-500/40 text-[11px] font-mono text-purple-300 inline-block">
          Round ${this.round} of ${this.maxRounds}
        </div>
      </div>
    `;

    const randomDelay = 1800 + Math.random() * 2600;
    this.timeoutId = setTimeout(() => {
      if (!this.isRunning || !this.waitingForTrigger) return;
  ...
  triggerAudioCue() {
    ...
    this.arena.className = 'aim-arena w-full min-h-[360px] rounded-2xl relative flex flex-col items-center justify-center cursor-pointer select-none bg-purple-600 border-4 border-white shadow-[0_0_80px_rgba(168,85,247,0.9)]';
    this.arena.innerHTML = `
      <div class="text-center p-6 pointer-events-none animate-ping">
        <span class="text-6xl">🔊</span>
        <h2 class="text-4xl font-heading font-black text-white uppercase tracking-wider mt-2">SOUND TRIGGERED!</h2>
        <p class="text-sm font-heading font-black text-purple-200 uppercase">TAP NOW!</p>
      </div>
    `;
  }

  // --- MODE 5: 30S RAPID BLITZ ---
  startBlitzChallenge() {
    this.blitzTimeLeft = 30;
    this.hits = 0;
    this.misses = 0;
    ...
    this.arena.className = 'aim-arena w-full min-h-[360px] rounded-2xl relative cursor-crosshair select-none bg-slate-950 border border-emerald-500/40 tactical-grid overflow-hidden';
    this.arena.innerHTML = '';

    // Spawn 3 initial targets
    for (let i = 0; i < 3; i++) {
      this.spawnBlitzTarget();
    }

    this.blitzInterval = setInterval(() => {
      this.blitzTimeLeft--;
      this.updateBlitzStats();
      if (this.blitzTimeLeft <= 0) {
        clearInterval(this.blitzInterval);
        this.blitzInterval = null;
        this.endBlitzSession();
      }
    }, 1000);
  ...
  spawnBlitzTarget() {
    ...
    const size = isHeadshot ? 30 : 48;

    const minX = size + 20;
    const maxX = Math.max(rect.width - size - 20, size + 40);
    const minY = size + 20;
    const maxY = Math.max(rect.height - size - 20, size + 40);

    const x = Math.floor(minX + Math.random() * (maxX - minX));
    const y = Math.floor(minY + Math.random() * (maxY - minY));

    const target = document.createElement('div');
    target.className = 'aim-target absolute rounded-full flex items-center justify-center cursor-crosshair active:scale-90 shadow-xl';
    target.style.width = `${size}px`;
    target.style.height = `${size}px`;
    target.style.left = `${x}px`;
    target.style.top = `${y}px`;
    target.style.transform = 'translate(-50%, -50%)';
    target.setAttribute('data-spawn-time', performance.now().toString());

    if (isHeadshot) {
      target.style.background = 'radial-gradient(circle, #ef4444 30%, #7f1d1d 90%)';
      target.style.border = '2px solid #ffffff';
      target.innerHTML = '<span class="text-[10px] font-black text-white pointer-events-none">HD</span>';
    } else {
      target.style.background = 'radial-gradient(circle, #10b981 30%, #065f46 90%)';
      target.style.border = '2px solid #ffffff';
      target.innerHTML = '<span class="text-[10px] font-black text-white pointer-events-none">HIT</span>';
    }
  ...
  updateBlitzStats() {
    ...
    const kps = (this.hits / Math.max(1, 30 - this.blitzTimeLeft)).toFixed(1);
    this.statsEl.innerHTML = `
      <div class="flex items-center justify-between text-xs font-mono">
        <span class="text-emerald-400 font-bold">⏱️ Time Remaining: <span class="text-white text-sm">${this.blitzTimeLeft}s</span></span>
        <span class="text-slate-300">Hits: <strong class="text-emerald-400 text-sm">${this.hits}</strong> | Speed: <strong class="text-cyan-400">${kps} targets/s</strong></span>
      </div>
    `;
  }

  // --- ARENA CLICK & MISCLICK DISPATCHER ---
  handleArenaClick(e) {
    if (!this.isRunning) return;

    // 1. Visual or Audio Mode Click Handler
    if (this.mode === 'visual' || this.mode === 'audio') {
      if (this.waitingForTrigger) {
        // False start! Clicked while Red
        this.playFailBuzz();
        this.waitingForTrigger = false;
        if (this.timeoutId) clearTimeout(this.timeoutId);

        this.arena.className = 'aim-arena w-full min-h-[360px] rounded-2xl relative flex flex-col items-center justify-center cursor-pointer select-none bg-slate-900 border-2 border-rose-500';
        this.arena.innerHTML = `
          <div class="text-center p-6 animate-shake">
            <div class="text-5xl mb-2">❌</div>
            <h3 class="text-2xl font-heading font-black text-rose-400 uppercase tracking-wider">TOO EARLY! FALSE START!</h3>
            <p class="text-xs text-slate-300 mt-1 font-mono">You anticipated before the trigger signal. Relax your fingers.</p>
            <button class="mt-4 px-4 py-2 bg-rose-600 hover:bg-rose-500 text-white font-heading font-black text-xs uppercase rounded-lg">
              Restart Round ${this.round}
            </button>
          </div>
        `;
        setTimeout(() => {
          if (this.isRunning) {
            if (this.mode === 'visual') this.startVisualRound();
            else this.startAudioRound();
          }
        }, 1400);
        return;
      }
      ...
      if (this.triggerActive) {
        ...
        this.arena.className = 'aim-arena w-full min-h-[360px] rounded-2xl relative flex flex-col items-center justify-center cursor-pointer select-none bg-slate-950 border border-emerald-500/60 shadow-[inset_0_0_30px_rgba(16,185,129,0.2)]';
        this.arena.innerHTML = `
          <div class="text-center p-6 animate-fade-in">
            <div class="text-4xl mb-1">⚡</div>
            <div class="text-4xl sm:text-5xl font-heading font-black text-emerald-400 tracking-tight">${rt} <span class="text-sm font-normal text-slate-400">ms</span></div>
            <p class="text-xs font-heading font-bold text-slate-300 mt-2 uppercase tracking-wider">${this.getSpeedRating(rt)}</p>
            <div class="mt-4 inline-block px-3 py-1 rounded-full bg-slate-900 border border-slate-700 text-[11px] font-mono text-slate-400">
              Round ${this.round}/${this.maxRounds} Complete
            </div>
          </div>
        `;
    ...
    // 2. Gloo Wall Reflex Mode Click Handler
    if (this.mode === 'gloo') {
      if (this.waitingForTrigger) {
        this.playFailBuzz();
        this.waitingForTrigger = false;
        ...
        this.arena.innerHTML = `
          <div class="text-center p-6 animate-shake">
            <div class="text-5xl mb-2">❌</div>
            <h3 class="text-2xl font-heading font-black text-rose-400 uppercase tracking-wider">PREMATURE GLOO WALL!</h3>
            <p class="text-xs text-slate-300 mt-1 font-mono">You wasted a gloo wall before the enemy even fired! Wait for the shot.</p>
          </div>
        `;
        setTimeout(() => {
          if (this.isRunning) this.startGlooRound();
        }, 1400);
        return;
      }
      ...
      if (this.triggerActive) {
        ...
        if (rt <= 210) {
          this.playGlooBlock();
          this.arena.className = 'aim-arena w-full min-h-[360px] rounded-2xl relative flex flex-col items-center justify-center cursor-pointer select-none bg-emerald-950/80 border-2 border-emerald-400 shadow-[0_0_50px_rgba(16,185,129,0.4)]';
          this.arena.innerHTML = `
            <div class="text-center p-6 animate-fade-in">
              <div class="text-5xl mb-2">🛡️</div>
              <div class="text-3xl sm:text-4xl font-heading font-black text-emerald-300 uppercase tracking-wider">SNIPER BULLET BLOCKED!</div>
              <div class="text-2xl font-mono font-bold text-white mt-2">${rt} ms</div>
              <p class="text-xs text-emerald-200 mt-1 max-w-sm mx-auto font-sub font-bold uppercase">Flawless 0.15s Gloo Wall reflex! You took 0 damage and saved your life.</p>
            </div>
          `;
        } else {
          this.playFailBuzz();
          this.arena.className = 'aim-arena w-full min-h-[360px] rounded-2xl relative flex flex-col items-center justify-center cursor-pointer select-none bg-rose-950/80 border-2 border-rose-500 shadow-[0_0_50px_rgba(244,63,94,0.4)]';
          this.arena.innerHTML = `
            <div class="text-center p-6 animate-shake">
              <div class="text-5xl mb-2">💥</div>
              <div class="text-3xl sm:text-4xl font-heading font-black text-rose-400 uppercase tracking-wider">HEADSHOT TAKEN (-180 HP)!</div>
              <div class="text-2xl font-mono font-bold text-white mt-2">${rt} ms</div>
              <p class="text-xs text-rose-200 mt-1 max-w-sm mx-auto font-sub font-bold uppercase">Too slow! Fast gloo walls require sub-210ms reflex to survive Tier-1 snipers.</p>
            </div>
          `;
        }
    ...
    // 3. Targets Flicking Click Handler
    ...
    // 4. Blitz Click Handler
    if (this.mode === 'blitz') {
      const targetEl = e.target.closest('.aim-target');
      if (targetEl) {
        const spawnT = parseFloat(targetEl.getAttribute('data-spawn-time') || performance.now().toString());
        this.reactionTimes.push(Math.round(performance.now() - spawnT));
        this.hits++;
        this.playSuccessChime();
        targetEl.remove();
        this.activeTargets = this.activeTargets.filter(t => t !== targetEl);
        this.spawnBlitzTarget();
        this.updateBlitzStats();
      } else {
        this.misses++;
  ...
  getSpeedRating(ms) {
    if (ms < 180) return '⚡ Tier-1 Pro Phenom (God Reflexes)';
    if (ms <= 215) return '🛡️ Scrim Demon (Tournament Ready)';
    if (ms <= 255) return '🎯 Contender (Solid Grassroots)';
    return '⏱️ Underdog Rookie (Warmup Needed)';
  }
  ...
  endSession() {
    ...
    const avg = this.reactionTimes.length ? Math.round(this.reactionTimes.reduce((a, b) => a + b, 0) / this.reactionTimes.length) : 250;
    const best = this.reactionTimes.length ? Math.min(...this.reactionTimes) : avg;

    // Update Personal Records
    if (this.mode === 'visual' || this.mode === 'audio') {
      if (!this.records.bestVisual || best < this.records.bestVisual) {
        this.records.bestVisual = best;
      }
    } else if (this.mode === 'gloo') {
      if (!this.records.bestGloo || best < this.records.bestGloo) {
        this.records.bestGloo = best;
      }
    } else if (this.mode === 'targets') {
      const totalTaps = this.hits + this.misses;
      const acc = totalTaps > 0 ? Math.round((this.hits / totalTaps) * 100) : 100;
      if (!this.records.bestAccuracy || acc > this.records.bestAccuracy) {
        this.records.bestAccuracy = acc;
      }
    }

    // Calculate Overall Reflex Athlete Tier
    const currentBest = this.records.bestVisual || avg;
    if (currentBest < 185) this.records.tier = 'Tier-1 God Reflexes 👑';
    else if (currentBest <= 220) this.records.tier = 'Tournament LAN Ready ⚡';
    else if (currentBest <= 260) this.records.tier = 'Competitive Scrim Pro 🎯';
    else this.records.tier = 'Grassroots Contender 🛡️';

    this.saveRecords();

    // Render Victory / Diagnostic Card inside Arena
    this.arena.className = 'aim-arena w-full min-h-[360px] rounded-2xl relative flex flex-col items-center justify-center p-6 bg-slate-950 border border-cyan-500/50 shadow-2xl';
    this.arena.innerHTML = `
      <div class="text-center max-w-lg mx-auto animate-fade-in">
        <div class="inline-block px-3 py-1 rounded-full bg-cyan-950/80 border border-cyan-400 text-cyan-300 text-xs font-sub font-bold uppercase tracking-wider mb-2">
          ${this.getModeName()} &bull; Session Results
        </div>
        <div class="text-4xl sm:text-5xl font-heading font-black text-white my-1">
          ${avg} <span class="text-sm font-normal text-slate-400 font-mono">ms avg</span>
        </div>
        <div class="text-sm font-heading font-bold text-amber-400 mb-3">${this.getSpeedRating(avg)}</div>

        <!-- Round-by-round pills -->
        <div class="flex items-center justify-center gap-1.5 mb-4 flex-wrap">
          ${this.reactionTimes.map((t, idx) => `
            <div class="px-2.5 py-1 rounded-lg ${t < 200 ? 'bg-emerald-950/80 border-emerald-500/50 text-emerald-300' : t < 240 ? 'bg-cyan-950/80 border-cyan-500/50 text-cyan-300' : 'bg-slate-900 border-slate-700 text-slate-300'} border text-[11px] font-mono">
              R${idx + 1}: <strong>${t}ms</strong>
            </div>
          `).join('')}
        </div>

        <p class="text-xs text-slate-300 leading-relaxed max-w-md mx-auto mb-4">
          ${this.getProAdvice(avg)}
        </p>

        <div class="flex items-center justify-center gap-3">
          <button id="retryActiveTestBtn" class="px-5 py-2 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 text-black font-sub font-bold text-xs uppercase rounded-xl tracking-wider shadow-lg">
            ⚡ Run Test Again
          </button>
          <button id="nextModeTestBtn" class="px-4 py-2 bg-slate-900 hover:bg-slate-800 border border-slate-700 text-slate-200 font-sub text-xs uppercase rounded-xl">
            Switch Mode
          </button>
        </div>
      </div>
    ...
    document.getElementById('retryActiveTestBtn')?.addEventListener('click', () => this.startTest());
    document.getElementById('nextModeTestBtn')?.addEventListener('click', () => {
      const modes = ['visual', 'gloo', 'targets', 'audio', 'blitz'];
  ...
  endBlitzSession() {
    ...
    const totalTaps = this.hits + this.misses;
    const acc = totalTaps > 0 ? Math.round((this.hits / totalTaps) * 100) : 100;
    const kps = (this.hits / 30).toFixed(1);

    if (!this.records.bestBlitz || this.hits > this.records.bestBlitz) {
      this.records.bestBlitz = this.hits;
    }
    this.saveRecords();

    this.arena.className = 'aim-arena w-full min-h-[360px] rounded-2xl relative flex flex-col items-center justify-center p-6 bg-slate-950 border border-emerald-500/50 shadow-2xl';
    this.arena.innerHTML = `
      <div class="text-center max-w-lg mx-auto animate-fade-in">
        <div class="inline-block px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-400 text-emerald-300 text-xs font-sub font-bold uppercase tracking-wider mb-2">
          ⏱️ 30s Rapid Blitz Results
        </div>
        <div class="text-5xl font-heading font-black text-white my-1">
          ${this.hits} <span class="text-sm font-normal text-slate-400 font-mono">Targets Hit</span>
        </div>
        <div class="flex items-center justify-center gap-4 text-xs font-mono text-slate-300 my-2">
          <span>Speed: <strong class="text-emerald-400 text-sm">${kps} kills/sec</strong></span>
          <span>&bull;</span>
          <span>Accuracy: <strong class="text-cyan-400 text-sm">${acc}%</strong></span>
          <span>&bull;</span>
          <span>Misses: <strong class="text-rose-400 text-sm">${this.misses}</strong></span>
        </div>
        <p class="text-xs text-slate-300 mt-2 max-w-md mx-auto leading-relaxed">
          ${this.hits >= 35 ? '🔥 Godlike flick speed! You are performing at Tier-1 FFWS/BGIS tournament fragging pace.' : 'Solid endurance! Practice keeping your eyes fixed on crosshair center to shave down transition time.'}
        </p>
        <button id="retryActiveTestBtn" class="mt-4 px-5 py-2 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 text-black font-sub font-bold text-xs uppercase rounded-xl tracking-wider shadow-lg">
          ⚡ Retry Blitz Challenge
        </button>
      </div>
    `;
  ...
  getProAdvice(avg) {
    if (avg < 185) return 'Your neurological motor reaction speed is in the top 1% of global competitive athletes. Focus 80% of your training on macro IGL calls and positioning.';
    if (avg <= 220) return 'Tournament-ready reaction speed! Ideal for Entry Fraggers and 0.15s Fast Gloo Wall rushers. Maintain with 5-minute pre-scrim hand warmups.';
    if (avg <= 260) return 'Solid grassroots competitive speed. Shave off 25-35 milliseconds by turning on 120Hz touch sampling rate and using thumb sleeves.';
    return 'Your nervous system or device touch polling is cold. Wash hands in warm water, perform finger extensor glides, and verify phone touch boost settings.';
  }
}
  ...
  class UnderdogApp {
    constructor() {
      ...
      this.activePlaybookView = 'warmup'; // 'warmup' | 'callouts' | 'mindset'
      this.calloutActiveCard = 'blue_double';
      this.calloutActiveFilter = 'all';
      this.warmupTimer = {
        active: false,
        currentStage: 0,
        timeLeft: 180,
        interval: null,
        completed: false,
        streak: parseInt(localStorage.getItem('underdog_warmup_streak') || '0', 10),
        stages: [
          { id: 'mobility', title: 'Hand & Finger Mobility', icon: '🕷️', duration: 180, completed: false, desc: 'Spider taps on flat surface, wrist extensor stretches, and warm water nerve activation.', proTip: 'Cold hands (<20°C) increase nerve conduction latency by 25ms. Warm hands move 15% faster.' },
          { id: 'calibration', title: 'Drag Headshot Calibration', icon: '🎯', duration: 300, completed: false, desc: 'Shotgun flick headshots, SMG recoil drag consistency, and 30m / 50m range testing in Training Ground.', proTip: 'Keep crosshair at upper-chest height before dragging fire button. Smooth drag beats violent jerk.' },
          { id: 'gloo_drill', title: '360° Gloo Wall Muscle Memory', icon: '🛡️', duration: 240, completed: false, desc: 'Sit-down gloo placement, jump-shoot-wall combo, and double-wall perimeter sealing in 0.15s rhythm.', proTip: 'Never look up when placing gloo walls. Drag joystick down and pull crosshair to your feet.' },
          { id: 'clutch_test', title: '1v1 Lone Wolf Clutch Test', icon: '⚔️', duration: 180, completed: false, desc: 'Real opponent duel: practice movement baiting, high-pressure trade knocks, and reset pacing.', proTip: 'Commit 100% to first shot advantage. If you whiff your shotgun shot, instantly deploy gloo and reset.' }
        ]
      };
      this.zoneClockState = {
        active: false,
        ...
        timeLeft: 210,
        interval: null,
        strategyMode: 'early', // 'early' | 'late'
        phases: [
          { phase: 1, name: 'Phase 1: Initial Drop & Outer Loot Boundary', duration: 210, dps: 1, zoneSize: '1000m → 650m', earlyAdvice: 'Crash forward compound before bridges and main choke points are occupied.', lateAdvice: 'Loot outer perimeter undisturbed; use the blue edge as rear security.' },
          { phase: 2, name: 'Phase 2: Mid-Game Territory Claim', duration: 150, dps: 2, zoneSize: '650m → 420m', earlyAdvice: 'Take central 2-story compound or high ridge with vehicles. Set up 360° perimeter.', lateAdvice: 'Hug weak side of zone (fewer plane drops). Let center teams battle and trade knocks.' },
          { phase: 3, name: 'Phase 3: Critical Hard-Shift Phase', duration: 120, dps: 4, zoneSize: '420m → 240m', earlyAdvice: 'If zone pulls onto open fields, commit 100% vehicle crash into nearest hard cover.', lateAdvice: 'Do NOT run into open. Hold edge until 0:20, follow blue line in, vulture knocked players.' },
          { phase: 4, name: 'Phase 4: Late Circle Collapse', duration: 90, dps: 8, zoneSize: '240m → 110m', earlyAdvice: 'Gatekeep the edge! Any late-rotating team must cross your crosshair in open ground.', lateAdvice: 'Deploy double-stacked gloo walls along blue line. Move only when blue zone pushes your back.' },
          { phase: 5, name: 'Phase 5: End-Game Heal-Off & Showdown', duration: 60, dps: 15, zoneSize: '110m → 0m (Full Closure)', earlyAdvice: 'High-ground crossfire and grenade barrage into lower compounds.', lateAdvice: 'Heal-off protocol: Prep Super Medkits, Inhalers, and Luqueta HP boost for final collapse.' }
        ]
      };
...
this.ffSelectedLoadout = { 
      ...
      this.ffScrimMode = 'scorekeeper'; // 'lobby' (PointCalc style) or 'my_squad'
      this.activeScrimsView = 'scorekeeper'; // 'scorekeeper' | 'famous_scrims' | 'room_dispatcher' | 'lobby' | 'my_squad' | 'tuition_time'
      this.scrimMatches = this.loadSavedScrimMatches();
      this.voiceRoom = {
        connected: false,
        muted: true,
        deafened: false,
        ...
        notes: 'Match 1: Bermuda • Peak Drop. 2-2 Split. Phase 3 North Rotate. Room Pass: 7711',
        seats: [
          { id: 1, role: '👑 IGL / Captain', name: 'Coach Duker / Anurag', ign: 'TITAN_IGL', status: 'idle', avatar: '👑' },
          { id: 2, role: '⚡ Primary Rusher', name: 'Star Fragger', ign: 'APEX_Rush', status: 'idle', avatar: '⚡' },
          { id: 3, role: '🎯 Assaulter / Fragger', name: 'Re-Frag Anchor', ign: 'HAVOC_99', status: 'idle', avatar: '🎯' },
          { id: 4, role: '🔭 Sniper / Scout', name: 'Overwatch Sniper', ign: 'DEADEYE_M82B', status: 'idle', avatar: '🔭' }
        ]
      };
      this.backpackState = {
        level: 3,
        role: 'rusher' // 'rusher' | 'assaulter' | 'sniper'
      };
      this.clutchTrainer = {
        activeScenario: 0,
        selectedChoice: null,
      ...
      this.ffScrimMatches = this.loadFFScrimMatches();
...
${curPreset.proTip}`;
    ...
    playChimeSound(type) {
      try {
        ...
        if (type === 'stage_complete') {
          ...
          o1.frequency.setValueAtTime(783.99, ctx.currentTime + 0.15);
        ...
        } else if (type === 'warning_30s') {
          for (let i = 0; i < 2; i++) {
            const o = ctx.createOscillator();
            const g = ctx.createGain();
            o.type = 'sawtooth';
            o.frequency.setValueAtTime(880, ctx.currentTime + i * 0.16);
            g.gain.setValueAtTime(0.2, ctx.currentTime + i * 0.16);
            g.gain.exponentialRampToValueAtTime(0.01, ctx.currentTime + i * 0.16 + 0.13);
            o.connect(g);
            g.connect(ctx.destination);
            o.start(ctx.currentTime + i * 0.16);
            o.stop(ctx.currentTime + i * 0.16 + 0.13);
          }
        } else if (type === 'warmup_finish') {
          const freqs = [523.25, 659.25, 783.99, 1046.50];
          freqs.forEach((f, idx) => {
            const o = ctx.createOscillator();
            const g = ctx.createGain();
            o.type = 'sine';
            o.frequency.setValueAtTime(f, ctx.currentTime + idx * 0.12);
    ...
    // --- SAFE ZONE MATCH CLOCK & ROTATION METHODS ---
    startZoneClock() {
      if (this.zoneClockState.active) return;
      this.zoneClockState.active = true;
      clearInterval(this.zoneClockState.interval);
      this.zoneClockState.interval = setInterval(() => {
        if (this.zoneClockState.timeLeft > 0) {
          this.zoneClockState.timeLeft--;
          if (this.zoneClockState.timeLeft === 30) {
            this.playChimeSound('warning_30s');
          }
          this.updateZoneClockUI();
        } else {
          // Phase finished, auto advance or chime
          this.playChimeSound('stage_complete');
          if (this.zoneClockState.currentPhase < 5) {
            this.setZonePhase(this.zoneClockState.currentPhase + 1);
          } else {
            this.pauseZoneClock();
          }
        }
    ...
    pauseZoneClock() {
      this.zoneClockState.active = false;
      clearInterval(this.zoneClockState.interval);
      this.updateZoneClockUI();
    }
    ...
    resetZoneClock() {
      this.pauseZoneClock();
      const current = this.zoneClockState.phases.find(p => p.phase === this.zoneClockState.currentPhase);
      this.zoneClockState.timeLeft = current ? current.duration : 210;
      this.updateZoneClockUI();
    }

    setZonePhase(phaseNum) {
      this.zoneClockState.currentPhase = phaseNum;
      const phaseData = this.zoneClockState.phases.find(p => p.phase === phaseNum);
      if (phaseData) {
        this.zoneClockState.timeLeft = phaseData.duration;
      }
      this.renderTabContent();
    }

    toggleZoneStrategyMode(mode) {
      this.zoneClockState.strategyMode = mode;
      this.renderTabContent();
    }
    ...
    updateZoneClockUI() {
      const timeDisplay = document.getElementById('zoneClockTimeDisplay');
      const toggleBtn = document.getElementById('zoneClockToggleBtn');
      const warningBox = document.getElementById('zoneClockWarningBox');
      const warningSeconds = document.getElementById('zoneWarningSeconds');

      if (timeDisplay) {
        timeDisplay.textContent = this.formatClockTime(this.zoneClockState.timeLeft);
      }
      if (toggleBtn) {
        toggleBtn.innerHTML = this.zoneClockState.active
          ? '<span>⏸️</span> Pause Clock'
          : '<span>▶️</span> Start Match Clock';
        toggleBtn.className = this.zoneClockState.active
          ? 'px-4 py-2 rounded-xl bg-amber-600 hover:bg-amber-500 text-white font-sub font-bold uppercase text-xs tracking-wider shadow-lg flex items-center gap-1.5 transition-all'
          : 'px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-sub font-bold uppercase text-xs tracking-wider shadow-lg flex items-center gap-1.5 transition-all';
      }
      if (warningBox) {
        if (this.zoneClockState.timeLeft <= 30 && this.zoneClockState.timeLeft > 0) {
          warningBox.classList.remove('hidden');
          if (warningSeconds) warningSeconds.textContent = this.zoneClockState.timeLeft;
        } else {
    ...
    renderZoneClockView() {
      const currentPhaseData = this.zoneClockState.phases.find(p => p.phase === this.zoneClockState.currentPhase) || this.zoneClockState.phases[0];
      const isEarly = this.zoneClockState.strategyMode === 'early';

      return `
        <div class="space-y-6 animate-fade-in">
          <!-- LIVE SAFE ZONE COUNTDOWN HUD -->
          <div class="cyber-panel p-6 rounded-2xl border border-purple-500/40 bg-gradient-to-r from-slate-950 via-[#130b22] to-slate-950 shadow-2xl relative overflow-hidden">
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6 border-b border-slate-800 pb-6">
              <div>
                <div class="flex items-center gap-2">
                  <span class="w-3 h-3 rounded-full ${this.zoneClockState.active ? 'bg-emerald-400 animate-ping' : 'bg-amber-400'}"></span>
                  <span class="text-xs font-sub font-bold text-purple-400 uppercase tracking-widest">
                    ${this.zoneClockState.active ? 'MATCH IN PROGRESS • LIVE ZONE TELEMETRY' : 'SCRIM CLOCK READY • WAITING FOR PLANE DROP'}
                  </span>
                </div>
                <h4 class="text-xl font-heading font-black text-white uppercase tracking-wider mt-1 flex items-center gap-3">
                  ⏱️ Tournament Safe Zone & Rotation Clock
                  <span class="cyber-badge bg-purple-950 text-purple-300 border border-purple-500/40 text-[10px]">
                    PHASE ${currentPhaseData.phase} OF 5
                  </span>
                </h4>
                <p class="text-xs text-slate-400 mt-1 max-w-2xl">
                  Run this clock during your scrims. Automatically provides audio countdown warnings and dictates whether your squad must execute an <strong>Early Center Crash</strong> or a <strong>Late Edge Snake</strong>.
                </p>
              </div>

              <!-- Main Clock Display -->
              <div class="flex items-center gap-4">
                <div class="bg-slate-950 px-6 py-3 rounded-2xl border-2 ${this.zoneClockState.timeLeft <= 30 && this.zoneClockState.timeLeft > 0 ? 'border-rose-500 animate-pulse bg-rose-950/30' : 'border-purple-500/40'} text-center shadow-xl">
                  <div class="text-[10px] font-sub uppercase font-bold text-slate-400">Zone Shrink Timer</div>
                  <div id="zoneClockTimeDisplay" class="text-3xl lg:text-4xl font-mono font-black text-white tracking-widest mt-0.5">
                    ${this.formatClockTime(this.zoneClockState.timeLeft)}
                  </div>
                  <div class="text-[10px] font-sub text-rose-400 font-bold mt-0.5">DPS: ${currentPhaseData.dps} HP / sec</div>
                </div>

                <!-- Control Buttons -->
                <div class="flex flex-col gap-2">
                  <button id="zoneClockToggleBtn" class="px-4 py-2 rounded-xl ${this.zoneClockState.active ? 'bg-amber-600 hover:bg-amber-500' : 'bg-emerald-600 hover:bg-emerald-500'} text-white font-sub font-bold uppercase text-xs tracking-wider shadow-lg flex items-center gap-1.5 transition-all">
                    ${this.zoneClockState.active ? '<span>⏸️</span> Pause Clock' : '<span>▶️</span> Start Match Clock'}
                  </button>
                  <div class="flex items-center gap-2">
                    <button id="zoneClockPrevBtn" class="px-2.5 py-1.5 rounded-lg bg-slate-900 border border-slate-700 hover:border-slate-500 text-slate-300 hover:text-white text-xs font-sub font-bold transition-all" title="Previous Phase">
                      ⏮️ Prev
                    </button>
                    <button id="zoneClockNextBtn" class="px-2.5 py-1.5 rounded-lg bg-slate-900 border border-slate-700 hover:border-slate-500 text-slate-300 hover:text-white text-xs font-sub font-bold transition-all" title="Next Phase">
                      Next ⏭️
                    </button>
                    <button id="zoneClockResetBtn" class="px-2.5 py-1.5 rounded-lg bg-slate-900 border border-slate-700 hover:border-slate-500 text-slate-300 hover:text-white text-xs font-sub font-bold transition-all" title="Reset Clock">
                      🔄 Reset
                    </button>
                  </div>
                </div>
            ...
            <!-- 30-SECOND DANGER ALERT (Dynamic) -->
            <div id="zoneClockWarningBox" class="${this.zoneClockState.timeLeft <= 30 && this.zoneClockState.timeLeft > 0 ? '' : 'hidden'} mt-4 p-3.5 rounded-xl bg-rose-500/20 border-2 border-rose-500 text-rose-300 flex items-center justify-between gap-4 animate-pulse">
              <div class="flex items-center gap-3">
                <span class="text-2xl">🚨</span>
                <div>
                  <div class="text-xs font-black uppercase tracking-wider font-heading text-white">
                    CRITICAL 30-SECOND ROTATION ALERT: ZONE COLLAPSING IN <span id="zoneWarningSeconds">${this.zoneClockState.timeLeft}</span> SECONDS!
                  </div>
                  <div class="text-[11px] text-rose-200 mt-0.5">
                    Disengage from poke fights immediately! Any squad caught in the open during this phase takes ${currentPhaseData.dps} HP/s lethal blue damage.
                  </div>
                </div>
              </div>
              <span class="px-3 py-1 rounded-full bg-rose-950 border border-rose-400 text-rose-200 text-[10px] font-sub font-bold uppercase shrink-0">
                COMMIT NOW
              </span>
            </div>

            <!-- 5-Phase Interactive Stepper Bar -->
            <div class="grid grid-cols-2 sm:grid-cols-5 gap-2.5 mt-5">
              ${this.zoneClockState.phases.map(p => `
                <button class="zone-phase-btn p-3 rounded-xl text-left border transition-all ${p.phase === this.zoneClockState.currentPhase ? 'bg-purple-950/90 border-purple-400 text-white shadow-lg ring-1 ring-purple-400' : 'bg-slate-950/60 border-slate-800 text-slate-400 hover:text-white hover:border-slate-700'}" data-phase="${p.phase}">
                  <div class="flex items-center justify-between text-[10px] font-sub uppercase font-bold">
                    <span>Phase ${p.phase}</span>
                    <span class="text-rose-400 font-mono">${p.dps} HP/s</span>
                  </div>
                  <div class="text-xs font-bold text-white mt-1 truncate">${p.name.split(':')[1] || p.name}</div>
                  <div class="text-[10px] text-slate-500 font-mono mt-0.5">${p.duration}s (${Math.floor(p.duration/60)}:${p.duration%60 === 0 ? '00' : p.duration%60})</div>
                </button>
              `).join('')}
            </div>
          </div>

          <!-- IN-GAME SITUATION MATRIX: EARLY SHRINK VS LATE SHRINK -->
          <div class="cyber-panel p-6 rounded-2xl border border-cyan-500/40 bg-gradient-to-b from-slate-950 via-[#071324] to-slate-950 space-y-6">
              ...
              <div>
                <div class="flex items-center gap-2">
                  ...
                  <h4 class="text-base font-heading font-black text-white uppercase tracking-wider">
                    🧭 In-Game Situation Matrix: Early Shrink vs. Late Shrink Tactics
                  </h4>
                </div>
                <p class="text-xs text-slate-400 mt-1">
                  How does a tournament IGL know whether to rotate immediately or wait for the blue zone to push? Match your live match condition:
                </p>
              </div>

              <!-- Strategy Mode Toggle -->
              <div class="flex items-center gap-2 bg-slate-950 p-1.5 rounded-xl border border-slate-800">
                <button class="zone-strategy-btn ${isEarly ? 'active bg-gradient-to-r from-cyan-600 to-blue-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'} px-4 py-2 rounded-lg text-xs font-sub font-bold uppercase tracking-wider transition-all flex items-center gap-1.5" data-mode="early">
                  <span>⚡</span> Early Shrink (Center Crash)
                </button>
                <button class="zone-strategy-btn ${!isEarly ? 'active bg-gradient-to-r from-purple-600 to-rose-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'} px-4 py-2 rounded-lg text-xs font-sub font-bold uppercase tracking-wider transition-all flex items-center gap-1.5" data-mode="late">
                  <span>⏱️</span> Late Shrink (Edge Snaking)
                </button>
              </div>
            </div>
            ...
            ${isEarly ? `
              <div class="space-y-6 animate-fade-in">
                <!-- Overview Header -->
                <div class="p-4 rounded-xl bg-cyan-950/30 border border-cyan-500/30 flex flex-col md:flex-row md:items-center justify-between gap-3">
                  <div>
                    <div class="flex items-center gap-2">
                      <span class="cyber-badge bg-cyan-900 text-cyan-200 border border-cyan-400/50 text-[10px] font-bold">
                        FAST CENTER CRASH
                      </span>
                      <span class="text-xs font-heading font-black text-white uppercase">Philosophy: Claim the King's Throne Before Anyone Else Arrives</span>
                    </div>
                    <p class="text-xs text-slate-300 mt-1">
                      Executing an early rotation means moving <strong>2 to 3 minutes before the zone closes</strong>. You willingly sacrifice extra looting time to capture the most defensible fortress in the center of the next zone.
                    </p>
                  </div>
                  <div class="text-right shrink-0">
                    <span class="text-[11px] font-sub text-cyan-400 font-bold">Recommended for:</span>
                    <div class="text-xs font-bold text-white">Full 4-Man Squads with 2+ Vehicles</div>
                  </div>
                </div>

                <!-- 4 In-Game Situations That Dictate Early Rotation -->
                <div>
                  <h5 class="text-xs font-sub uppercase font-bold text-cyan-400 tracking-wider mb-3">
                    4 Specific In-Game Scenarios That Trigger Early Rotation:
                  </h5>
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="p-4 rounded-xl bg-slate-900/80 border border-cyan-500/20 space-y-2">
                      <div class="flex items-center justify-between">
                        <span class="text-xs font-bold text-white flex items-center gap-1.5">
                          <span>🚗</span> Situation 1: Vehicle Superiority (2+ Cars / Pickups)
                        </span>
                        <span class="text-[10px] px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-400 font-bold border border-emerald-500/30">High Speed Mobility</span>
                      </div>
                      <p class="text-[11px] text-slate-300 leading-relaxed">
                        If your squad secured 2 cars or Jeeps from drop, do NOT stay on foot. Rotate immediately at the start of Phase 2 or 3. You can cover 400m in 15 seconds, bypass roadside ambushes, and use vehicles as indestructible metal shields.
                      </p>
                      <div class="text-[11px] text-cyan-300 font-mono bg-slate-950 p-2 rounded border border-slate-800">
                        <strong>IGL Action:</strong> Drive in convoy 30m apart. Park vehicles perpendicular to compound doors to seal entrances.
                      </div>
                    </div>
                    ...
                    <div class="p-4 rounded-xl bg-slate-900/80 border border-cyan-500/20 space-y-2">
                      <div class="flex items-center justify-between">
                        <span class="text-xs font-bold text-white flex items-center gap-1.5">
                          <span>🌊</span> Situation 2: River Crossing / Wide Flat Plain Ahead
                        </span>
                        <span class="text-[10px] px-2 py-0.5 rounded bg-rose-500/10 text-rose-400 font-bold border border-rose-500/30">Choke Point Death Trap</span>
                      </div>
                      <p class="text-[11px] text-slate-300 leading-relaxed">
                        If the new zone pulls across the river (e.g. Bullseye/Cemetery or Rim Nam bridge) or an open plain with zero natural rocks, <strong>you must cross instantly</strong>. Waiting late means 3 squads will gatekeep the shoreline and eliminate you in the water.
                      </p>
                      <div class="text-[11px] text-cyan-300 font-mono bg-slate-950 p-2 rounded border border-slate-800">
                        <strong>IGL Action:</strong> Drop smoke lines across bridge, cross immediately, and set up your own gatekeep on the opposite bank.
                      </div>
                    </div>
                    ...
                    <div class="p-4 rounded-xl bg-slate-900/80 border border-cyan-500/20 space-y-2">
                      <div class="flex items-center justify-between">
                        <span class="text-xs font-bold text-white flex items-center gap-1.5">
                          <span>🛡️</span> Situation 3: Heavy Gloo Economy Surplus (12+ Walls)
                        </span>
                        <span class="text-[10px] px-2 py-0.5 rounded bg-cyan-500/10 text-cyan-400 font-bold border border-cyan-500/30">Utility Fortified</span>
                      </div>
                      <p class="text-[11px] text-slate-300 leading-relaxed">
                        Holding a central compound attracts pressure from multiple angles. When your squad has 12 to 18 gloo walls, you have the defense to withstand grenade cluster spam, repair destroyed walls, and establish 360° crossfire boxes.
                      </p>
                      <div class="text-[11px] text-cyan-300 font-mono bg-slate-950 p-2 rounded border border-slate-800">
                        <strong>IGL Action:</strong> Deploy 2-2 V-wall wedges around compound perimeter. Keep 4 gloos in reserve for emergency revives.
                      </div>
                    </div>
                    ...
                    <div class="p-4 rounded-xl bg-slate-900/80 border border-cyan-500/20 space-y-2">
                      <div class="flex items-center justify-between">
                        <span class="text-xs font-bold text-white flex items-center gap-1.5">
                          <span>🔭</span> Situation 4: 3-1 Scout Confirms Compound is Unoccupied
                        </span>
                        <span class="text-[10px] px-2 py-0.5 rounded bg-purple-500/10 text-purple-400 font-bold border border-purple-500/30">Information Advantage</span>
                      </div>
                      <p class="text-[11px] text-slate-300 leading-relaxed">
                        Your forward scout rides ahead and confirms that a prime compound (e.g. Peak Central Mansion or Pochinok Double Door) is 100% empty. You must crash before neighboring edge teams spot the vacancy.
                      </p>
                      <div class="text-[11px] text-cyan-300 font-mono bg-slate-950 p-2 rounded border border-slate-800">
                        <strong>IGL Action:</strong> 15-second reinforcement countdown! Full squad crashes simultaneously to fortify all windows.
                      </div>
                    </div>
                  </div>
                </div>

                <!-- 3-Step Execution & Voice Comms -->
                <div class="p-4 rounded-xl bg-slate-900/90 border border-slate-800 flex flex-col md:flex-row justify-between gap-4">
                  <div class="space-y-1.5">
                    <div class="text-xs font-sub font-bold text-cyan-400 uppercase">Early Shrink IGL Voice Comms Script:</div>
                    <div class="text-xs italic text-cyan-200 bg-slate-950 p-2.5 rounded border border-cyan-500/30">
                      "Mount up vehicles now! Do not finish looting that house! Peak mansion is clear. Drive the north ditch line, crash the compound, park cars at the door, and set up 360° crossfire! Move now!"
                    </div>
                  </div>
                  <div class="space-y-1 shrink-0">
                    <div class="text-[11px] font-sub font-bold text-slate-400 uppercase">Tier-1 Team Reference:</div>
                    <div class="text-xs font-bold text-white">EVOS Phoenix & Total Gaming</div>
                    <div class="text-[11px] text-slate-400">Pioneers of the 90-second early vehicle compound crash.</div>
                  </div>
                </div>
              </div>
            ` : `
              <div class="space-y-6 animate-fade-in">
                <!-- Overview Header -->
                <div class="p-4 rounded-xl bg-rose-950/30 border border-rose-500/30 flex flex-col md:flex-row md:items-center justify-between gap-3">
                  <div>
                    <div class="flex items-center gap-2">
                      <span class="cyber-badge bg-rose-900 text-rose-200 border border-rose-400/50 text-[10px] font-bold">
                        BLUE LINE EDGE SNAKING
                      </span>
                      <span class="text-xs font-heading font-black text-white uppercase">Philosophy: Let Other Teams Kill Each Other, Then Steal Their Points</span>
                    </div>
                    <p class="text-xs text-slate-300 mt-1">
                      Executing a late rotation means <strong>holding the perimeter until the zone timer reaches 0:20 or starts moving</strong>. The blue zone acts as your 100% bulletproof rear armor, ensuring nobody can flank your squad from behind.
                    </p>
                  </div>
                  <div class="text-right shrink-0">
                    <span class="text-[11px] font-sub text-rose-400 font-bold">Recommended for:</span>
                    <div class="text-xs font-bold text-white">Low Gloo Economy, Shorthanded Squads (Duo/Solo)</div>
                  </div>
                </div>

                <!-- 4 In-Game Situations That Dictate Late Rotation -->
                <div>
                  <h5 class="text-xs font-sub uppercase font-bold text-rose-400 tracking-wider mb-3">
                    4 Specific In-Game Scenarios That Trigger Late Rotation:
                  </h5>
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="p-4 rounded-xl bg-slate-900/80 border border-rose-500/20 space-y-2">
                      <div class="flex items-center justify-between">
                        <span class="text-xs font-bold text-white flex items-center gap-1.5">
                          <span>📉</span> Situation 1: Starved Gloo Economy (< 6 Walls Total)
                        </span>
                        <span class="text-[10px] px-2 py-0.5 rounded bg-rose-500/10 text-rose-400 font-bold border border-rose-500/30">High Vulnerability</span>
                      </div>
                      <p class="text-[11px] text-slate-300 leading-relaxed">
                        If your whole squad only has 3 to 5 gloo walls combined, driving into the center is instant suicide. Center compounds face crossfire from 3-4 teams. Staying on the edge lets natural terrain (rocks, dips, trees) serve as your cover.
                      </p>
                      <div class="text-[11px] text-rose-300 font-mono bg-slate-950 p-2 rounded border border-slate-800">
                        <strong>IGL Action:</strong> Hug the slow-moving edge of the zone (weak side). Save your few gloo walls strictly for endgame heal-offs.
                      </div>
                    </div>
                    ...
                    <div class="p-4 rounded-xl bg-slate-900/80 border border-rose-500/20 space-y-2">
                      <div class="flex items-center justify-between">
                        <span class="text-xs font-bold text-white flex items-center gap-1.5">
                          <span>💀</span> Situation 2: Shorthanded Squad (Duo or Solo Rat)
                        </span>
                        <span class="text-[10px] px-2 py-0.5 rounded bg-amber-500/10 text-amber-400 font-bold border border-amber-500/30">Survival Priority</span>
                      </div>
                      <p class="text-[11px] text-slate-300 leading-relaxed">
                        When 2 teammates are knocked out early, holding a building is impossible against aggressive 4-man squads who will grenade and breach you. A duo must snake the blue edge, stay prone, and extract placement points.
                      </p>
                      <div class="text-[11px] text-rose-300 font-mono bg-slate-950 p-2 rounded border border-slate-800">
                        <strong>IGL Action:</strong> Equip silencers or marksman rifles. Never initiate fights; only shoot when an enemy is running through the open away from the zone.
                      </div>
                    </div>
                    ...
                    <div class="p-4 rounded-xl bg-slate-900/80 border border-rose-500/20 space-y-2">
                      <div class="flex items-center justify-between">
                        <span class="text-xs font-bold text-white flex items-center gap-1.5">
                          <span>🔥</span> Situation 3: Active Heavy Gunfire Directly on Rotation Path
                        </span>
                        <span class="text-[10px] px-2 py-0.5 rounded bg-cyan-500/10 text-cyan-400 font-bold border border-cyan-500/30">Third-Party Opportunity</span>
                      </div>
                      <p class="text-[11px] text-slate-300 leading-relaxed">
                        Two Tier-1 squads are fighting on the crest ahead of you. If you push early, both squads will turn and shoot you. Hold your position until the blue zone forces both teams to break cover, then third-party the wounded survivors.
                      </p>
                      <div class="text-[11px] text-rose-300 font-mono bg-slate-950 p-2 rounded border border-slate-800">
                        <strong>IGL Action:</strong> Wait for killfeed knocks. The moment 2 players are eliminated, push the flank and sweep their loot boxes.
                      </div>
                    </div>
                    ...
                    <div class="p-4 rounded-xl bg-slate-900/80 border border-rose-500/20 space-y-2">
                      <div class="flex items-center justify-between">
                        <span class="text-xs font-bold text-white flex items-center gap-1.5">
                          <span>🏔️</span> Situation 4: Enemy Squad Holding Mountain Ridge Gatekeep
                        </span>
                        <span class="text-[10px] px-2 py-0.5 rounded bg-purple-500/10 text-purple-400 font-bold border border-purple-500/30">High Ground Disadvantage</span>
                      </div>
                      <p class="text-[11px] text-slate-300 leading-relaxed">
                        An enemy squad is positioned on top of the mountain ridge looking straight down into your path. Never push uphill into their crosshairs. Wait late, wrap 200m horizontally along the blue zone line, and approach from their blind flank.
                      </p>
                      <div class="text-[11px] text-rose-300 font-mono bg-slate-950 p-2 rounded border border-slate-800">
                        <strong>IGL Action:</strong> Smoke the base of the hill to bait their attention, then wrap wide along the blue perimeter to equal elevation.
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="p-4 rounded-xl bg-slate-900/90 border border-slate-800 flex flex-col md:flex-row justify-between gap-4">
                  <div class="space-y-1.5">
                    <div class="text-xs font-sub font-bold text-rose-400 uppercase">Late Shrink IGL Voice Comms Script:</div>
                    <div class="text-xs italic text-rose-200 bg-slate-950 p-2.5 rounded border border-rose-500/30">
                      "Hold fire! Do not shoot! Let Team Soul and Orangutan fight on the hill! We stay on the blue edge. When the blue zone ticks them at 0:15, they have to run downhill. We take the third-party kills and loot their gloos!"
                    </div>
                  </div>
                  <div class="space-y-1 shrink-0">
                    <div class="text-[11px] font-sub font-bold text-slate-400 uppercase">Tier-1 Team Reference:</div>
                    <div class="text-xs font-bold text-white">Fluxo & WJB Esports</div>
                    <div class="text-[11px] text-slate-400">Masters of the weak-side blue line perimeter wrap and third-party vulture plays.</div>
                  </div>
                </div>
              </div>
            `}

            <!-- END-GAME HEAL-OFF CONSUMABLE CALCULATOR (PHASE 5 HUD) -->
            <div class="p-4 rounded-xl bg-slate-950/80 border border-purple-500/30">
              <div class="flex items-center justify-between border-b border-slate-800 pb-2 mb-3">
                <div class="flex items-center gap-2">
                  <span class="text-base">💊</span>
                  <span class="text-xs font-sub uppercase font-bold text-purple-400">Phase 5 Final Circle Heal-Off Consumables Math</span>
                </div>
                <span class="text-[10px] text-slate-400 font-mono">Final Zone DPS: 15 HP/sec</span>
              </div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
                <div class="p-3 rounded-lg bg-slate-900 border border-slate-800">
                  <div class="font-bold text-white flex items-center justify-between">
                    <span>Super Medkit</span>
                    <span class="text-emerald-400">+200 HP</span>
                  </div>
                  <div class="text-[11px] text-slate-400 mt-1">Cast Time: <strong>4.0s</strong> (Cannot move). Pop at <strong>120 HP</strong> to finish cast before zero HP.</div>
                </div>

                <div class="p-3 rounded-lg bg-slate-900 border border-slate-800">
                  <div class="font-bold text-white flex items-center justify-between">
                    <span>Standard Medkit</span>
                    <span class="text-cyan-400">+75 HP</span>
                  </div>
                  <div class="text-[11px] text-slate-400 mt-1">Cast Time: <strong>4.0s</strong>. In Phase 5, 15 DPS drains 60 HP during cast. Net gain: +15 HP only.</div>
                </div>

                <div class="p-3 rounded-lg bg-slate-900 border border-slate-800">
                  <div class="font-bold text-white flex items-center justify-between">
                    <span>Inhaler / EP Boost</span>
                    <span class="text-amber-400">+20 HP / +20 EP</span>
                  </div>
                  <div class="text-[11px] text-slate-400 mt-1">Cast Time: <strong>Instant (0.0s)</strong>. Can pop while sprinting or sliding. Crucial buffer while moving.</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="space-y-6 animate-fade-in">
          ...
          <div class="cyber-panel p-2.5 rounded-xl border border-purple-500/40 bg-slate-950/90 flex flex-wrap items-center justify-between gap-3 shadow-lg">
            ...
            <div class="flex flex-wrap items-center gap-2">
              ...
              <button class="coaching-nav-btn ${this.coachingActiveView === 'zone_clock' ? 'active bg-purple-600 text-white shadow ring-2 ring-purple-400/40' : 'bg-slate-900 text-slate-400 hover:text-white border border-slate-800'} px-3.5 py-1.5 rounded-xl text-xs font-sub font-bold uppercase tracking-wider transition-all" data-view="zone_clock">
                🧭 Safe Zone Clock & Early/Late Shrink Advisor
              </button>
          ...
          <!-- ============================================================= -->
          ${this.coachingActiveView === 'zone_clock' ? this.renderZoneClockView() : ''}
    ...
    bindCoachingEvents() {
      ...
      const resetBtn = document.getElementById('zoneClockResetBtn');
      if (resetBtn) {
        resetBtn.addEventListener('click', () => this.resetZoneClock());
      }

      const prevBtn = document.getElementById('zoneClockPrevBtn');
      if (prevBtn) {
        prevBtn.addEventListener('click', () => {
          if (this.zoneClockState.currentPhase > 1) {
            this.setZonePhase(this.zoneClockState.currentPhase - 1);
          }
        });
      }

      const nextBtn = document.getElementById('zoneClockNextBtn');
      if (nextBtn) {
      ...
      document.querySelectorAll('.zone-phase-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          const phase = parseInt(btn.getAttribute('data-phase'), 10);
          if (phase) this.setZonePhase(phase);
        });
      });

      document.querySelectorAll('.zone-strategy-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          const mode = btn.getAttribute('data-mode');
          if (mode) this.toggleZoneStrategyMode(mode);
        });
      });
...
`;
      ...
      return [
        {
          ...
          totalPts: 21,
          mistakeId: 'clean',
          fatalMistake: 'None (Clean Booyah / Flawless Fight)',
          vodUrl: 'https://youtu.be/ffws-m1-booyah',
          debriefNotes: 'Double vehicle rotation from Rim Nam. Anchored high ground on Peak and held 3-1 split for clean Booyah.'
        },
        {
          id: 1789851002,
          matchNum: 2,
          ...
          vodUrl: 'https://youtu.be/ffws-m2-river',
          debriefNotes: 'Held river bank 45s too long waiting for loot; Team Falcons gatekept bridge with DMR chips. Took 120 blue zone damage.'
        },
        {
          id: 1789851003,
          matchNum: 3,
          ...
          placementPts: 3,
          totalPts: 5,
          mistakeId: 'trigger_discipline',
          fatalMistake: 'Trigger Discipline Failure (Premature Firing)',
          vodUrl: 'https://youtu.be/ffws-m3-council',
          debriefNotes: 'Rusher opened fire on solo scout at Council Hall without team ready. Orangutan 3rd-partied from Refinery ridge and wiped us.'
        },
        {
          id: 1789851004,
          matchNum: 4,
          ...
          placementPts: 8,
          totalPts: 14,
          mistakeId: 'split_anchor_no_refrag',
          fatalMistake: 'Split from Anchor / No Re-Frag Available',
          vodUrl: 'https://youtu.be/ffws-m4-grav',
          debriefNotes: 'Grav Labs 2v2 final zone; anchor was caught on lower platform 50m back unable to re-frag Dimitri before Nairi shield broke.'
        },
        {
          id: 1789851005,
          matchNum: 5,
          ...
          fatalMistake: 'Late Zone Rotation (Gatekept in Blue Zone)',
          vodUrl: '',
          debriefNotes: 'Delayed exit from Solar Plant. Ran out of gloo walls in open blue zone field.'
        }
      ];
    }
    ...
    loadFFScrimMatches() {
      try {
        const saved = localStorage.getItem('underdog_ff_scrims_v2');
        if (saved) {
          const parsed = JSON.parse(saved);
          if (Array.isArray(parsed) && parsed.length > 0) return parsed;
        }
      } catch (e) {
        console.warn('Error loading FF scrim matches:', e);
      }
      return this.getDefaultFFScrimMatches();
    }

    saveFFScrimMatches() {
      try {
        localStorage.setItem('underdog_ff_scrims_v2', JSON.stringify(this.ffScrimMatches));
        if (window.underdogSupabase && window.underdogSupabase.isConnected && window.underdogSupabase.currentUser) {
          window.underdogSupabase.syncUserData('scrims', this.ffScrimMatches).catch(e => console.warn('Cloud scrim sync error:', e));
        }
      } catch (e) {
        console.warn('Error saving FF scrim matches:', e);
      }
    }

    getFFMistakeCategories() {
      return [
        {
          id: 'clean',
          name: 'None (Clean Booyah / Flawless Fight)',
          icon: '🏆',
          severity: 'clean',
          tag: 'CLEAN',
          color: 'emerald',
          desc: 'Flawless execution, traded kills, solid zone timing, and superior high ground.'
        },
        {
          id: 'trigger_discipline',
          name: 'Trigger Discipline Failure (Premature Firing)',
          icon: '🎯',
          severity: 'critical',
          tag: 'TRIGGER',
          color: 'rose',
          desc: 'Shot too early without knock guarantee; gave squad location away to 3rd parties.'
        },
        {
          id: 'late_zone_rotation',
          name: 'Late Zone Rotation (Gatekept in Blue Zone)',
          icon: '⚡',
          severity: 'critical',
          tag: 'ROTATION',
          color: 'amber',
          desc: 'Delayed rotation past 1:00 on zone timer; pinned outside circle by compound team.'
        },
        {
          id: 'open_ground_no_gloo',
          name: 'Pushed Open Ground Without Gloo / Smoke',
          icon: '🛡️',
          severity: 'high',
          tag: 'COVER',
          color: 'rose',
          desc: 'Ego sprinted across open terrain without 0.15s gloo reflex or smoke path.'
        },
        {
          id: 'split_anchor_no_refrag',
          name: 'Split from Anchor / No Re-Frag Available',
          icon: '👥',
          severity: 'high',
          tag: 'ISOLATION',
          color: 'purple',
          desc: 'Rusher died 70m+ away from Dimitri/Nairi anchor; squad unable to trade the knock.'
        },
        {
          id: 'third_party_blindspot',
          name: 'Third-Party Blindspot (Tunnel Vision)',
          icon: '👀',
          severity: 'critical',
          tag: 'AWARENESS',
          color: 'orange',
          desc: 'Squad hyper-focused on 1 squad in front; rear scout failed 360-degree perimeter check.'
        },
        {
          id: 'drop_contest_starved',
          name: 'Poor Drop Contest / Loot Starved',
          icon: '📦',
          severity: 'medium',
          tag: 'EARLY_GAME',
          color: 'yellow',
          desc: 'Forced 50-50 4v4 drop contest with no weapon parity; early lobby exit (Rank 10-12).'
        },
        {
          id: 'greedy_box_looting',
          name: 'Greedy Death-Box Looting Under Fire',
          icon: '💀',
          severity: 'medium',
          tag: 'GREED',
          color: 'pink',
          desc: 'Crouched looting open death boxes in Phase 4-6; picked off by sniper or grenade.'
        },
        {
          id: 'utility_starved',
          name: 'Utility Starved / Mistimed Frag Cooking',
          icon: '💣',
          severity: 'medium',
          tag: 'UTILITY',
          color: 'cyan',
          desc: 'Ran out of gloo walls or self-damaged / failed grenade trajectory bounce.'
        }
      ];
    }

    getFFScrimAnalytics() {
      const matches = this.ffScrimMatches || [];
      const totalMatches = matches.length;
      const categories = this.getFFMistakeCategories();
      ...
      if (totalMatches === 0) {
        return {
          ...
          topMistake: null,
          blunderRate: 0,
          totalWipesWithBlunder: 0,
          mistakeCounts: {},
          categories
        };
      }

      const totalKills = matches.reduce((sum, m) => sum + (parseInt(m.kills, 10) || 0), 0);
      const totalPlacementPts = matches.reduce((sum, m) => sum + (parseInt(m.placementPts, 10) || 0), 0);
      const totalPoints = matches.reduce((sum, m) => sum + (parseInt(m.totalPts, 10) || 0), 0);
      const booyahCount = matches.filter(m => m.placement === 1).length;
      const booyahRate = Math.round((booyahCount / totalMatches) * 100);
      const avgPts = (totalPoints / totalMatches).toFixed(1);
      const avgKills = (totalKills / totalMatches).toFixed(1);

      const mistakeCounts = {};
      let totalWipesWithBlunder = 0;

      matches.forEach(m => {
        let mId = m.mistakeId;
        if (!mId) {
          if (m.fatalMistake && m.fatalMistake.includes('Clean')) mId = 'clean';
          else if (m.fatalMistake && m.fatalMistake.includes('Late Zone')) mId = 'late_zone_rotation';
          else if (m.fatalMistake && m.fatalMistake.includes('Trigger')) mId = 'trigger_discipline';
          else if (m.fatalMistake && m.fatalMistake.includes('Split')) mId = 'split_anchor_no_refrag';
          else mId = 'late_zone_rotation';
        }
        mistakeCounts[mId] = (mistakeCounts[mId] || 0) + 1;
        if (mId !== 'clean') {
          totalWipesWithBlunder++;
        }
      });

      let topMistakeId = null;
      let maxBlunderCount = 0;
      Object.entries(mistakeCounts).forEach(([id, count]) => {
        if (id !== 'clean' && count > maxBlunderCount) {
          maxBlunderCount = count;
          topMistakeId = id;
        }
      });

      const topMistakeCategory = categories.find(c => c.id === topMistakeId) || null;
      const topMistakePercent = totalWipesWithBlunder > 0 ? Math.round((maxBlunderCount / totalWipesWithBlunder) * 100) : 0;

      let disciplineScore = 100;
      matches.forEach(m => {
        const id = m.mistakeId || (m.fatalMistake && m.fatalMistake.includes('Clean') ? 'clean' : 'late_zone_rotation');
        if (id === 'trigger_discipline' || id === 'late_zone_rotation' || id === 'third_party_blindspot') {
          disciplineScore -= 12;
        } else if (id === 'open_ground_no_gloo' || id === 'split_anchor_no_refrag') {
          disciplineScore -= 8;
        } else if (id !== 'clean') {
          disciplineScore -= 5;
        }
      });
      disciplineScore = Math.max(15, Math.min(100, Math.round(disciplineScore)));
      const blunderRate = totalMatches > 0 ? Math.round((totalWipesWithBlunder / totalMatches) * 100) : 0;
      ...
      return {
        ...
        blunderRate,
        totalWipesWithBlunder,
        topMistake: topMistakeCategory ? {
          ...topMistakeCategory,
          count: maxBlunderCount,
          percent: topMistakePercent
        } : null,
        mistakeCounts,
        categories
      };
    ...
    getCoachDukerAdvice(topMistakeId) {
      const adviceMap = {
        'late_zone_rotation': {
          title: "The 1:15 Early Vehicle Lock Protocol",
          quote: "If your squad is walking on foot when the zone starts moving, you have already donated 12 points to the compound teams waiting for you.",
          verdict: "Late zone rotations account for the majority of your lost scrim points. Underdog teams hesitate because they want to loot every single house. In Tier-1 lobbies (FFWS/BGIS), the team that rotates 40 seconds earlier gets free kills on every straggler behind them.",
          rule: "THE 1:15 ROTATION RULE: Anchor calls rotation route when the white timer reaches 1:15. Commits to 2x vehicle split (Pickup + Jeep) through low-elevation terrain depressions rather than running ridge tops.",
          drill: "20-Min Purgatory River Crossing Drill: In a custom room, practice taking low ground across Brasilia river under simulated sniper fire using double vehicles without losing a single player.",
          priority: "HIGH PRIORITY (WIPING SQUAD CONSISTENCY)"
        },
        'trigger_discipline': {
          title: "The 3-Count Knock Confirmation Rule",
          quote: "Shooting at an enemy without 100% knock confirmation is not aggression—it's an invitation for third parties to pinch your squad from behind.",
          verdict: "Your fraggers are giving away squad positioning by firing single poke shots across 150m. In competitive lobbies, a premature gunshot alerts all neighboring teams to collapse on your location.",
          rule: "THE 3-COUNT COMMITTED TARGET RULE: The IGL or Anchor calls 'TARGET SPOTTED - HOLD FIRE'. No player shoots until at least TWO team members have simultaneous crosshair alignment and an instant knock is guaranteed.",
          drill: "Bermuda Silent Ambush Drill: Squad drops Peak triangle and tracks enemy rotations for 45 seconds strictly on ping callouts with ZERO gunshots until the target squad enters open crossfire.",
          priority: "CRITICAL PRIORITY (SOLO EGO SHOOTING)"
        },
        'split_anchor_no_refrag': {
          title: "The 15-Meter Re-Frag Tether",
          quote: "A knock in competitive Free Fire is not a mistake—dying without an immediate re-frag within 2 seconds is the mistake.",
          verdict: "Your rushers are over-extending 60m+ away from the Dimitri/Nairi anchor. When the point-man gets knocked, nobody is in position to trade the elimination, turning every 4v4 into an easy 3v4 wipe for the opponent.",
          rule: "THE 15-METER TETHER: Rusher 1 and Rusher 2 must maintain maximum 12-15m spacing. Support anchor (Nairi) must never lose line-of-sight to the assault lead to deploy defensive gloos.",
          drill: "Buddy System 2v2 Custom Scrim: Squad plays custom 2v2s. If any player dies and their partner does not knock the enemy within 2.5 seconds, the round is counted as an automatic forfeit.",
          priority: "HIGH PRIORITY (TEAM TETHERING)"
        },
        'open_ground_no_gloo': {
          title: "The Double-Gloo Triangle & 0.15s Reflex",
          quote: "Crossing open grass without gloo cover is handing free DMR headshots to the lobby.",
          verdict: "Squad members are getting caught sprinting across open fields without placing advance staging gloo walls. In modern Free Fire, 0.15s sit-down gloo wall deployment is a non-negotiable fundamental.",
          rule: "THE ADVANCE GLOO TRIANGLE: Plant 1st gloo at 45 degrees, sprint 8m, plant 2nd gloo. Always carry minimum 4 gloo walls per player entering Phase 4 circles.",
          drill: "50-Rep Gloo Speedrun in Training Grounds: 50 consecutive sit-down 360-degree gloo wall placements aiming for sub-180ms reaction time in the Reflex Gym.",
          priority: "HIGH PRIORITY (MECHANICAL REFLEX)"
        },
        'third_party_blindspot': {
          title: "The 3-1 Perimeter Overwatch Protocol",
          quote: "When two dogs fight over a bone, the third dog walks away with the points.",
          verdict: "Your squad suffers from tunnel vision. When you engage an enemy in front, all 4 players aim down sights at the same target, leaving your flanks completely unprotected.",
          rule: "THE 3-1 REARGUARD LOCK: In any front-line teamfight, Player 4 (Anchor/Scout) DOES NOT fire at the front target. Player 4 turns 180 degrees, mounts high ground, and watches for third-party flankers.",
          drill: "Rearguard Callout Drill: 15 minutes of custom rooms where Anchor must call out enemy positions 150m away while squad takes 3v3 compound fight.",
          priority: "CRITICAL PRIORITY (MACRO VISION)"
        },
        'drop_contest_starved': {
          title: "The 30-Second Loot Threshold & Bailout Route",
          quote: "Dying at #12 in a 50-50 drop contest ruins an entire 6-match scrim set in 90 seconds.",
          verdict: "Your squad is forcing 50-50 hot drops without weapon advantage. In Tier-1 tournaments, early contest without armor is a coin-toss that destroys tournament consistency.",
          rule: "THE 30-SECOND BAILOUT RULE: If the opposing squad gets shotgun/SMG first or grabs the high-tier warehouse, immediately take the designated vehicle bailout route to secondary compound.",
          drill: "Secondary Compound Drop Pathing: Practice landing Clock Tower outer split or Rim Nam secondary huts within 18 seconds of drop.",
          priority: "MEDIUM PRIORITY (EARLY GAME SURVIVAL)"
        },
        'greedy_box_looting': {
          title: "The 3-Second Box Rule & Smoke Looting",
          quote: "The most expensive Medkit in the game is the one you died looting.",
          verdict: "Players are crouch-looting open death boxes in Phase 5 circles. Sniper rifles (AWM/M82B) and Marksman rifles punish stationary looting instantaneously.",
          rule: "THE 3-SECOND LOOT RULE: Never touch an open death box without a gloo wall or smoke grenade. Loot only ammo and gloos within 3 seconds while continuously wiggling joystick.",
          drill: "Continuous Motion Looting: Train joystick wiggle + quick inventory swipe in training grounds.",
          priority: "MEDIUM PRIORITY (UNFORCED ERROR)"
        },
        'utility_starved': {
          title: "Utility Economy & Grenade Trajectory Cooking",
          quote: "Gloo walls win fights; grenades end stalemates.",
          verdict: "Squad runs dry of utility before the final zone or throws un-cooked grenades that enemies easily dodge with a quick gloo placement.",
          rule: "THE 2.5-SECOND GRENADE COOK: Cook offensive grenades to 2.5s before release to guarantee detonation on enemy gloo wall impact with zero reaction window.",
          drill: "Cooked Frag Accuracy Drill: Practice bouncing cooked grenades over gloo walls into compound windows in Training Grounds.",
          priority: "MEDIUM PRIORITY (UTILITY DISCIPLINE)"
        }
      };

      return adviceMap[topMistakeId] || {
        title: "Championship Consistency Maintenance",
        quote: "Winning one match is easy; staying in the top 3 across all 6 matches requires disciplined macro play.",
        verdict: "Your squad is playing exceptionally clean with few unforced blunders. Focus on high-elevation endzone locking and maintaining 4+ gloo walls per player entering Phase 5.",
        rule: "THE ENDZONE KING RULE: Secure the highest elevation within Phase 4 circle before 0:45 remaining on timer.",
        drill: "High-Ground Defensive Hold: 20 minutes practicing 4-man crossfire setup from high elevation rocks.",
        priority: "MAINTAIN DISCIPLINE (S-TIER FORM)"
      };
    }

    getTuitionTimeLessons() {
      return [
        {
          id: 'lesson_zone_shift',
          title: 'Lesson 1: Zone 4 & 5 Hard Shift Predictions & Low-Ground Pathing',
          subtitle: 'Why 90% of Underdog Teams Get Gatekept & How Tier-1 Teams Predict Safe Zones',
          badge: 'ZONE MACRO',
          badgeColor: 'amber',
          duration: '14:20 Mins',
          videoTimestamp: '02:15',
          proTeams: ['Total Gaming', 'Buriram United', 'Team Falcons'],
          summary: 'In Phase 4, the safe zone shifts hard away from dense compounds 70% of the time. Running late guarantees a 4-man wipe against gatekeeping DMR squads.',
          keyRules: [
            'Rule of Water: If the circle borders a river (e.g. Purgatory/Bermuda), cross BEFORE the timer hits 1:00.',
            'Double Vehicle Scouting: Never rotate on foot across 200m+ open zone. Send Pickup truck 40m ahead of Jeep.',
            'Depression Pathing: Use river beds and topological contour dips rather than high crests to block line of sight.'
          ],
          tuitionNotes: "Coach Duker Breakdown: Notice how Buriram United starts their rotation when the zone timer has 1 minute and 25 seconds left. They don't wait for the blue wall to touch their backs. By securing the low-ground trench under the bridge early, they picked up 7 easy kills on squads running out of the storm.",
          drillAction: 'Execute 3 custom room matches focusing exclusively on vehicle-led rotations starting at 1:15 timer.'
        },
        {
          id: 'lesson_gloo_reset',
          title: 'Lesson 2: 0.15s Fast Gloo Wall Box Reset Under Crossfire',
          subtitle: 'Surviving 3rd-Party Crossfire and Resetting Squad Health in 3 Seconds',
          badge: 'MICRO DEFENSE',
          badgeColor: 'rose',
          duration: '11:45 Mins',
          videoTimestamp: '01:30',
          proTeams: ['Fluxo W7M', 'Orangutan', 'EVOS Esports'],
          summary: 'When pinched between two squads, placing 1 single gloo wall gets you killed from the flank. Brazilian Tier-1 players deploy the 360-degree Box-In with Nairi.',
          keyRules: [
            'The 360 Box-In: Drop 3 gloo walls in a tight triangle around your knocked teammate within 0.3s.',
            'Nairi + Dimitri Synergy: Nairi provides 150% gloo durability against enemy spam while Dimitri allows auto-revive.',
            'Crouch-Plant Technique: Always crouch before tapping gloo button to eliminate body exposure gap beneath the wall.'
          ],
          tuitionNotes: "Coach Duker Breakdown: Watch Fluxo's defense when pinched at Grav Labs. The moment the first bullet hits, the anchor drops a 3-wall box. Dimitri's active healing aura goes down immediately, and all 4 players are back at 200 HP before the enemy can close the 40m gap.",
          drillAction: 'Practice 50 reps of crouch-plant 3-wall triangles in Training Grounds under 220ms.'
        },
        {
          id: 'lesson_refrag_math',
          title: 'Lesson 3: 3-1 Split vs 2-2 Anchor Formation (Re-Frag Math)',
          subtitle: 'Eliminating Isolated Rusher Deaths and Perfecting the 2-Second Kill Trade',
          badge: 'SQUAD TETHER',
          badgeColor: 'purple',
          duration: '16:10 Mins',
          videoTimestamp: '03:20',
          proTeams: ['Team Elite', 'RRQ Kazu', 'GodLike'],
          summary: 'Why aggressive rusher squads lose to patient anchor teams. The math of competitive trades dictates that whoever isolates a 1v1 loses.',
          keyRules: [
            'The 15m Re-Frag Radius: Rusher 1 and Rusher 2 must NEVER exceed 15m distance during compound entry.',
            'Line-of-Sight Anchor: Support anchor must hold high-angle line of sight onto the doorway the rusher breaches.',
            'Immediate Commit: If Rusher 1 is knocked, Rusher 2 must swing within 1.5 seconds while enemy is reloading or chambering.'
          ],
          tuitionNotes: "Coach Duker Breakdown: Team Elite's masterclass on 2-2 split. Notice how Pahadi holds the high rock with Marksman rifle while Killer pushes the gloo wall. The second an enemy peeks to shoot Killer, Pahadi's headshot lands before the enemy can finish the knock.",
          drillAction: 'Run 2v2 buddy drills in custom room where un-traded deaths count as automatic round loss.'
        },
        {
          id: 'lesson_trigger_discipline',
          title: 'Lesson 4: Trigger Discipline in 3rd-Party Sandwiches',
          subtitle: 'Reading the Kill-Feed & Executing the 5-Second Clean-Up Ambush',
          badge: 'AMBUSH MACRO',
          badgeColor: 'emerald',
          duration: '13:50 Mins',
          videoTimestamp: '01:55',
          proTeams: ['Team Falcons', 'S8UL', 'Blind Esports'],
          summary: 'How championship teams turn a 1-kill fight into a 6-kill wipe by letting opponent squads knock each other first.',
          keyRules: [
            'Wait for the 2nd Knock: When two teams fight, DO NOT fire on the 1st knock. Wait for 2+ knocks in the kill feed.',
            'Grenade Synchronized Launch: Launch cooked grenades over opponent gloo walls the exact second the fight peaks.',
            'The 5-Second Clean-Up: Flood the remaining surviving players while their armor is broken and abilities are on cooldown.'
          ],
          tuitionNotes: "Coach Duker Breakdown: Falcons hold their fire for 38 seconds on the ridge outside Peak. Even though enemies were in plain view, they waited until Orangutan and Total Gaming knocked 2 players each. The moment the kill feed lit up, Falcons wiped both wounded teams in 8 seconds.",
          drillAction: 'Play 3 scrim matches where the squad is forbidden from shooting until IGL confirms 2 knocks in the kill feed.'
        }
      ];
    }

    
    // --- SCRIM SCOREKEEPER, FAMOUS SCRIMS & ROOM DISPATCHER METHODS ---
    loadSavedScrimMatches() {
      try {
        const saved = localStorage.getItem('underdog_scrim_tracker_matches');
        if (saved) return JSON.parse(saved);
      } catch (e) {}
      return [
        { matchNum: 1, map: 'Bermuda', placement: 1, kills: 9, placementPts: 12, totalPts: 21 },
        { matchNum: 2, map: 'Purgatory', placement: 3, kills: 5, placementPts: 8, totalPts: 13 },
        { matchNum: 3, map: 'Kalahari', placement: 4, kills: 6, placementPts: 7, totalPts: 13 }
      ];
    }

    saveScrimMatches() {
      try {
        localStorage.setItem('underdog_scrim_tracker_matches', JSON.stringify(this.scrimMatches));
      } catch (e) {}
    }

    logNewScrimMatch(map, placement, kills) {
      const pPts = this.getFFPlacementPoints(placement);
      const total = pPts + kills;
      const nextNum = this.scrimMatches.length + 1;
      this.scrimMatches.push({
        matchNum: nextNum,
        map: map,
        placement: placement,
        kills: kills,
      ...
      this.saveScrimMatches();
      this.renderTabContent();
    }

    deleteScrimMatch(index) {
      this.scrimMatches.splice(index, 1);
      // Re-index match numbers
      this.scrimMatches.forEach((m, idx) => m.matchNum = idx + 1);
      this.saveScrimMatches();
      this.renderTabContent();
    }
    ...
    getScrimScorecardStats() {
      const matches = this.scrimMatches;
      const totalMatches = matches.length;
      const totalPts = matches.reduce((sum, m) => sum + m.totalPts, 0);
      const totalKills = matches.reduce((sum, m) => sum + m.kills, 0);
      const totalPlacementPts = matches.reduce((sum, m) => sum + m.placementPts, 0);
      const booyahs = matches.filter(m => m.placement === 1).length;
      const avgPts = totalMatches > 0 ? (totalPts / totalMatches).toFixed(1) : '0.0';

      let projection = 'Underdog Contender 🛡️';
      if (avgPts >= 15) projection = 'Tier-1 Champion Dominance 👑';
      else if (avgPts >= 10) projection = 'Grand Finals Qualifier 🔥';
      else if (avgPts >= 7) projection = 'Mid-Table Survivor ⚡';

      return { totalMatches, totalPts, totalKills, totalPlacementPts, booyahs, avgPts, projection };
    }

    getWhatsAppScorecardString() {
      const stats = this.getScrimScorecardStats();
      const date = new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
      let txt = `🏆 *[UNDERDOG ESPORTS] OFFICIAL SCRIM SCORECARD*\n`;
      txt += `📅 Date: ${date} | Set: ${stats.totalMatches} Matches\n`;
      txt += `👑 Booyahs: ${stats.booyahs} | 🎯 Total Kills: ${stats.totalKills}\n`;
      txt += `🔥 Total Points: *${stats.totalPts} pts* (Avg: ${stats.avgPts} pts/match)\n`;
      txt += `📊 Projection: ${stats.projection}\n\n`;
      txt += `*MATCH-BY-MATCH BREAKDOWN:*\n`;
      this.scrimMatches.forEach(m => {
        const medal = m.placement === 1 ? '👑' : m.placement <= 3 ? '🥉' : '▪️';
        txt += `${medal} Match ${m.matchNum} (${m.map}): #${m.placement} (${m.kills} Kills) = *${m.totalPts} pts*\n`;
      });
      txt += `\n🚀 Powered by Underdog Esports Academy (underdog.gg)`;
      return txt;
    }

    getFamousIndianScrims() {
      return [
        {
          id: 'eagle_esports',
          name: 'Eagle Esports Scrims (Official Daily Customs)',
          organizer: 'Eagle Esports India',
          badge: 'TIER-1 & UNDERDOG',
          badgeColor: 'bg-amber-950 text-amber-300 border-amber-500/40',
          accent: 'amber',
          icon: '🦅',
          timings: 'Slot 1: 2:00 PM | Slot 2: 6:00 PM | Slot 3: 9:00 PM (Daily)',
          matchesPerSet: '6 Matches (2 Bermuda, 2 Purgatory, 2 Kalahari)',
          pointSystem: 'Official Esports 12-Pt System (1st = 12, Kill = 1)',
          description: 'India\'s most renowned daily competitive scrims. Top Tier-1 organizations practice here alongside rising underdog champions. Strict anti-hack verification and automated Discord point calculation.',
          bookingRules: [
            'Registration opens 2 hours before match time in Discord #slot-confirmation.',
            'Submit Clan Tag, 4 Player UIDs, and IGL Discord tag.',
            'Room ID & Password dispatched 10 minutes prior; 2-minute entry rule strictly enforced.'
          ],
          proTeamsPlaying: 'Total Gaming Esports, Orangutan, GodLike FF, Team Elite Alumni',
          discordTag: 'discord.gg/eagleesports',
          bookingTemplate: '[EAGLE SCRIMS REGISTRATION]\nTeam Name: Underdog Esports\nTag: [UDG]\nIGL UID: 7770001\nPlayer 2: 7770002\nPlayer 3: 7770003\nPlayer 4: 7770004\nPreferred Slot: 6:00 PM Tier-1'
        },
        {
          id: 'villager_esports',
          name: 'Villager Esports (VE) Premier LAN Qualifiers',
          organizer: 'Villager Esports',
          badge: 'LAN QUALIFIER',
          badgeColor: 'bg-rose-950 text-rose-300 border-rose-500/40',
          accent: 'rose',
          icon: '🛡️',
          timings: 'Slot 1: 4:00 PM | Slot 2: 8:00 PM (Grand Scrims)',
          matchesPerSet: '6 Matches with live broadcast observer feeds',
          pointSystem: 'FFWS Standard 12-Pt System + MVP Kill Bounties',
          description: 'The highest tier of Indian competitive scrims with LAN tournament qualifier seedings. Monitored by official esports referees with mandatory post-match screenshot submission.',
          bookingRules: [
            'Tier-2 / Tier-1 invitation or proof of top-3 finish in weekly qualifiers required.',
            'Zero tolerance for emulator players or ping-spoofers; device check required.',
            'Screenshot of match scorecard must be submitted within 10 minutes of match end.'
          ],
          proTeamsPlaying: 'S8UL Esports, Orangutan, Team Falcons, Revenant',
          discordTag: 'discord.gg/villageresports',
          bookingTemplate: '[VILLAGER ESPORTS SCRIM SLOT]\nClan: Underdog Esports\nCategory: Tier-1 Invite\nCaptain Discord: Anurag#1234\nUIDs: 7770001, 7770002, 7770003, 7770004'
        },
        {
          id: 'upthrust_esports',
          name: 'Upthrust Esports Daily Grinders (T3 to T1)',
          organizer: 'Upthrust Esports',
          badge: 'HIGHWAY TO PRO',
          badgeColor: 'bg-cyan-950 text-cyan-300 border-cyan-500/40',
          accent: 'cyan',
          icon: '⚡',
          timings: '1:00 PM, 5:00 PM, and 7:30 PM (3 Parallel Lobbies)',
          matchesPerSet: '4 Matches fast-track sets',
          pointSystem: 'Official 12-Point with Tier Promotion / Relegation',
          description: 'The perfect testing ground for underdog squads looking to prove themselves. Features a dynamic promotion and relegation system where top 3 T3 squads earn promotion into the T1 evening lobby.',
          bookingRules: [
            'First-come-first-served reaction bot in #underdog-daily-slots.',
            'Clans with 3 consecutive unnotified slot misses receive 1-week blacklist.',
            'Room codes delivered automatically via automated DM bots.'
          ],
          proTeamsPlaying: 'Reckoning, Medal Esports, Enigma Gaming, Grassroots Champions',
          discordTag: 'discord.gg/upthrust',
          bookingTemplate: '[UPTHRUST SCRIMS SLOT REQUEST]\nTeam: Underdog Esports\nLobby: Tier-2 Promotion Group\nIGL Phone/Discord: +91-9876543210\nPlayer UIDs: 7770001, 7770002, 7770003, 7770004'
        },
        {
          id: '7sea_esports',
          name: '7Sea Esports Night Clashes & Scouting',
          organizer: '7Sea Esports',
          badge: 'SCOUTING CLASH',
          badgeColor: 'bg-blue-950 text-blue-300 border-blue-500/40',
          accent: 'blue',
          icon: '🌊',
          timings: '8:30 PM Night Clashes (6 Matches Non-Stop)',
          matchesPerSet: '6 Matches (Bermuda / Purgatory alternating)',
          pointSystem: 'Standard Esports System + Highest Fragger Cash Reward',
          description: 'Top Indian esports scouts actively watch 7Sea night scrims to recruit rising underdogs. Teams with high kill consistency are regularly scouted for national tier-1 tournaments.',
          bookingRules: [
            'Open slot registration closes daily at 5:00 PM.',
            'Only 4-man full squads permitted; zero solo/duo substitute entries without notice.',
            'Live discord voice channel presence encouraged for verification.'
          ],
          proTeamsPlaying: '7Sea Roster, Marcos Gaming, Blind Esports, Underdog Elites',
          discordTag: 'discord.gg/7seaesports',
          bookingTemplate: '[7SEA NIGHT SCRIMS REGISTRATION]\nTeam: Underdog Esports [UDG]\nLineup: Anurag, StarFragger, Havoc, Deadeye\nCaptain UID: 7770001\nContact: discord.gg/underdog'
        },
        {
          id: 'skyesports',
          name: 'Skyesports Championship Grassroots Cups',
          organizer: 'Skyesports',
          badge: 'NATIONAL TOURNAMENT',
          badgeColor: 'bg-purple-950 text-purple-300 border-purple-500/40',
          accent: 'purple',
          icon: '🏆',
          timings: 'Wednesday & Saturday 3:00 PM (Weekly Mega Cups)',
          matchesPerSet: '6 Matches Grand Finals Format',
          pointSystem: 'Official FFWS International Rules',
          description: 'Official tournament qualifiers connecting Indian underdogs directly to national streaming stages. Free registration with verified certificates and cash prize pools for podium finishers.',
          bookingRules: [
            'Register on official Skyesports portal or tournament bot.',
            'Government ID / Age verification required for prize pool claims.',
            'Mandatory match recording for anti-cheat audit.'
          ],
          proTeamsPlaying: 'GodLike, Orangutan, Team Elite, Global Esports, Top 48 Underdogs',
          discordTag: 'skyesports.in/tournaments',
          bookingTemplate: '[SKYESPORTS CUP REGISTRATION]\nTeam: Underdog Esports\nState/Region: India National\nLineup: 4 Main + 1 Sub\nCaptain Email: team@underdog.gg'
        }
      ];
    }
    ...
    parseRoomText(text) {
      if (!text) return;
      this.roomDispatcher.rawText = text;

      // Extract Room ID
      const idMatch = text.match(/(?:room\s*(?:id)?|id)[\s*:#=-]+(\d{5,10})/i);
      if (idMatch && idMatch[1]) {
        this.roomDispatcher.roomId = idMatch[1];
      }

      // Extract Password
      const passMatch = text.match(/(?:pass(?:word)?|pw)[\s*:#=-]+(\w+)/i);
      if (passMatch && passMatch[1]) {
        this.roomDispatcher.pass = passMatch[1];
      }

      // Extract Slot
      const slotMatch = text.match(/(?:slot|sl)[\s*:#=-]+(\d{1,2})/i);
      if (slotMatch && slotMatch[1]) {
        this.roomDispatcher.slot = slotMatch[1];
      }

      // Extract Map
      if (/purgatory/i.test(text)) this.roomDispatcher.map = 'Purgatory';
      else if (/kalahari/i.test(text)) this.roomDispatcher.map = 'Kalahari';
      else if (/alpine/i.test(text)) this.roomDispatcher.map = 'Alpine';
      else if (/nexterra/i.test(text)) this.roomDispatcher.map = 'NeXTerra';
      else if (/bermuda/i.test(text)) this.roomDispatcher.map = 'Bermuda';

      this.renderTabContent();
    }

    startRoomCountdown() {
      if (this.roomDispatcher.timerActive) return;
      this.roomDispatcher.timerActive = true;
      this.roomDispatcher.timeLeft = 120; // 2 minutes
      clearInterval(this.roomDispatcher.interval);

      this.roomDispatcher.interval = setInterval(() => {
        if (this.roomDispatcher.timeLeft > 0) {
          this.roomDispatcher.timeLeft--;
          if (this.roomDispatcher.timeLeft === 30) {
            this.playChimeSound('warning_30s');
          }
          this.updateRoomCountdownUI();
        } else {
          this.roomDispatcher.timerActive = false;
          clearInterval(this.roomDispatcher.interval);
          this.playChimeSound('stage_complete');
          this.updateRoomCountdownUI();
        }
    ...
    resetRoomCountdown() {
      this.roomDispatcher.timerActive = false;
      clearInterval(this.roomDispatcher.interval);
      this.roomDispatcher.timeLeft = 120;
      this.updateRoomCountdownUI();
    }

    updateRoomCountdownUI() {
      const el = document.getElementById('roomTimerDisplay');
      const box = document.getElementById('roomTimerBox');
      if (el) {
        const m = Math.floor(this.roomDispatcher.timeLeft / 60);
        const s = this.roomDispatcher.timeLeft % 60;
        el.textContent = `${m < 10 ? '0' : ''}${m}:${s < 10 ? '0' : ''}${s}`;
      }
      if (box) {
        if (this.roomDispatcher.timeLeft <= 30 && this.roomDispatcher.timeLeft > 0) {
          box.classList.add('border-rose-500', 'animate-pulse', 'bg-rose-950/40');
        } else {
          box.classList.remove('border-rose-500', 'animate-pulse', 'bg-rose-950/40');
        }
      }
    }

    toggleVoiceRoomConnect() {
      this.voiceRoom.connected = !this.voiceRoom.connected;
      this.playRadioBeep();

      if (this.voiceRoom.connected) {
        this.voiceRoom.muted = false;
        // Start simulated periodic voice activity
        clearInterval(this.voiceRoom.speakingInterval);
        this.voiceRoom.speakingInterval = setInterval(() => {
          if (!this.voiceRoom.connected || this.voiceRoom.deafened) return;
          const randomSeat = Math.floor(Math.random() * 4);
          this.voiceRoom.seats.forEach((s, idx) => {
            s.status = idx === randomSeat ? 'speaking' : 'listening';
          });
          const activeEl = document.getElementById('voiceRoomSpeakerIndicator');
          if (activeEl) {
            activeEl.textContent = `${this.voiceRoom.seats[randomSeat].role} Speaking 🟢`;
          }
          this.updateVoiceSeatsUI();
        }, 3500);
      } else {
        clearInterval(this.voiceRoom.speakingInterval);
        this.voiceRoom.seats.forEach(s => s.status = 'idle');
      }
      this.renderTabContent();
    }

    toggleVoiceRoomMute() {
      this.voiceRoom.muted = !this.voiceRoom.muted;
      this.playRadioBeep();
      const mySeat = this.voiceRoom.seats[0];
      if (mySeat) {
        mySeat.status = this.voiceRoom.muted ? 'muted' : 'speaking';
      }
      this.renderTabContent();
    }

    toggleVoiceRoomDeafen() {
      this.voiceRoom.deafened = !this.voiceRoom.deafened;
      this.playRadioBeep();
      if (this.voiceRoom.deafened) {
        this.voiceRoom.muted = true;
      }
      this.renderTabContent();
    }

    updateVoiceSeatsUI() {
      this.voiceRoom.seats.forEach(seat => {
        const seatEl = document.querySelector(`[data-voice-seat="${seat.id}"]`);
        if (seatEl) {
          const statusText = seatEl.querySelector('.voice-seat-status');
          const wave = seatEl.querySelector('.voice-wave-anim');
          if (statusText) {
            if (seat.status === 'speaking') {
              statusText.textContent = 'Speaking 🟢';
              statusText.className = 'voice-seat-status text-[10px] font-mono text-emerald-400 font-bold';
              if (wave) wave.classList.remove('hidden');
            } else if (seat.status === 'muted') {
              statusText.textContent = 'Muted 🔴';
              statusText.className = 'voice-seat-status text-[10px] font-mono text-rose-400';
              if (wave) wave.classList.add('hidden');
            } else {
              statusText.textContent = 'Listening 🎧';
              statusText.className = 'voice-seat-status text-[10px] font-mono text-cyan-400';
              if (wave) wave.classList.add('hidden');
            }
          }
        }
    ...
    renderFFScrimsTab(container) {
      if (!this.ffScrimMode) this.ffScrimMode = 'scorekeeper';
      ...
      const analytics = this.getFFScrimAnalytics();
      const coachAdvice = this.getCoachDukerAdvice(analytics.topMistake ? analytics.topMistake.id : 'clean');
      const tuitionLessons = this.getTuitionTimeLessons();

      const scorecardStats = this.getScrimScorecardStats();
      const famousScrims = this.getFamousIndianScrims();

      container.innerHTML = `
        <div class="space-y-6 animate-fade-in">
          <!-- Top Cyber Banner -->
          <div class="cyber-panel p-5 rounded-2xl border border-rose-500/40 bg-gradient-to-r from-slate-950 via-[#220d18] to-slate-950 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-xl">
            <div>
              <div class="flex items-center gap-2">
                <span class="text-2xl">📊</span>
                <h3 class="text-xl font-heading font-black text-white uppercase tracking-wider">
                  Tournament Scrims, Voice Room & PointCalc Suite
                </h3>
              ...
              <p class="text-xs text-slate-300 mt-1 max-w-3xl leading-relaxed">
                Log daily scrim points, view famous Indian scrims (Eagle Esports), dispatch room credentials in 2-minutes, and discuss rotations in the live Squad Voice Room.
              </p>
            </div>

            <!-- Mode Switcher -->
            <div class="flex flex-wrap items-center gap-1.5 bg-slate-900/90 p-1.5 rounded-xl border border-slate-800 shrink-0">
              <button class="ff-scrim-mode-btn ${this.ffScrimMode === 'scorekeeper' ? 'active bg-rose-600 text-white shadow' : 'text-slate-400 hover:text-white'} px-3 py-1.5 rounded-lg text-xs font-sub font-bold uppercase tracking-wider transition-all" data-mode="scorekeeper">
                🧮 1-Tap Scorekeeper
              </button>
              <button class="ff-scrim-mode-btn ${this.ffScrimMode === 'famous_scrims' ? 'active bg-amber-600 text-black font-black shadow' : 'text-slate-400 hover:text-white'} px-3 py-1.5 rounded-lg text-xs font-sub font-bold uppercase tracking-wider transition-all" data-mode="famous_scrims">
                🦅 Famous Indian Scrims
              </button>
              <button class="ff-scrim-mode-btn ${this.ffScrimMode === 'room_dispatcher' ? 'active bg-cyan-600 text-white shadow' : 'text-slate-400 hover:text-white'} px-3 py-1.5 rounded-lg text-xs font-sub font-bold uppercase tracking-wider transition-all" data-mode="room_dispatcher">
                🔑 Room Dispatcher
              </button>
              <button class="ff-scrim-mode-btn ${this.ffScrimMode === 'voice_room' ? 'active bg-purple-600 text-white shadow' : 'text-slate-400 hover:text-white'} px-3 py-1.5 rounded-lg text-xs font-sub font-bold uppercase tracking-wider transition-all" data-mode="voice_room">
                🎙️ Voice Room
              </button>
              <button class="ff-scrim-mode-btn ${this.ffScrimMode === 'lobby' ? 'active bg-slate-700 text-white shadow' : 'text-slate-400 hover:text-white'} px-3 py-1.5 rounded-lg text-xs font-sub font-bold uppercase tracking-wider transition-all" data-mode="lobby">
                📊 12-Team Broadcast
              </button>
              <button class="ff-scrim-mode-btn ${this.ffScrimMode === 'my_squad' ? 'active bg-slate-700 text-white shadow' : 'text-slate-400 hover:text-white'} px-3 py-1.5 rounded-lg text-xs font-sub font-bold uppercase tracking-wider transition-all" data-mode="my_squad">
                🎯 Mistake Analytics
              </button>
            </div>
          </div>
          ...
          <!-- ============================================================ -->
          <!-- MODE 1: 1-TAP SCRIM SCOREKEEPER & WHATSAPP SCORECARD -->
          <!-- ============================================================ -->
            ...
            <div class="space-y-6 animate-fade-in">
              ...
              <div class="grid grid-cols-2 sm:grid-cols-5 gap-3">
                <div class="cyber-panel p-4 rounded-xl border border-rose-500/30 bg-slate-950/80 text-center">
                  <div class="text-[10px] font-sub uppercase font-bold text-slate-400">Total Points</div>
                  <div class="text-2xl lg:text-3xl font-mono font-black text-rose-400 mt-1">${scorecardStats.totalPts} <span class="text-xs font-normal">pts</span></div>
                  <div class="text-[10px] text-slate-500 font-mono mt-0.5">${scorecardStats.totalMatches} Matches Logged</div>
                </div>
                <div class="cyber-panel p-4 rounded-xl border border-amber-500/30 bg-slate-950/80 text-center">
                  <div class="text-[10px] font-sub uppercase font-bold text-slate-400">Booyahs 👑</div>
                  <div class="text-2xl lg:text-3xl font-mono font-black text-amber-400 mt-1">${scorecardStats.booyahs}</div>
                  <div class="text-[10px] text-slate-500 font-mono mt-0.5">${scorecardStats.totalPlacementPts} Placement Pts</div>
                </div>
                <div class="cyber-panel p-4 rounded-xl border border-cyan-500/30 bg-slate-950/80 text-center">
                  <div class="text-[10px] font-sub uppercase font-bold text-slate-400">Team Kills ⚔️</div>
                  <div class="text-2xl lg:text-3xl font-mono font-black text-cyan-400 mt-1">${scorecardStats.totalKills}</div>
                  <div class="text-[10px] text-slate-500 font-mono mt-0.5">1 pt per kill</div>
                </div>
                <div class="cyber-panel p-4 rounded-xl border border-purple-500/30 bg-slate-950/80 text-center">
                  <div class="text-[10px] font-sub uppercase font-bold text-slate-400">Average Pts / M</div>
                  <div class="text-2xl lg:text-3xl font-mono font-black text-purple-400 mt-1">${scorecardStats.avgPts}</div>
                  <div class="text-[10px] text-slate-500 font-mono mt-0.5">Tournament Pace</div>
                </div>
                <div class="cyber-panel p-4 rounded-xl border border-emerald-500/30 bg-slate-950/80 text-center col-span-2 sm:col-span-1">
                  <div class="text-[10px] font-sub uppercase font-bold text-slate-400">Status Projection</div>
                  <div class="text-xs font-bold text-emerald-400 mt-2 truncate">${scorecardStats.projection}</div>
                  <div class="text-[10px] text-slate-500 font-mono mt-1">FFWS Standard</div>
                </div>
              </div>

              <!-- Fast Match Entry Bar -->
              <div class="cyber-panel p-5 rounded-2xl border border-rose-500/40 bg-gradient-to-r from-slate-950 via-[#1a0c16] to-slate-950 space-y-4 shadow-xl">
                <div class="flex items-center justify-between border-b border-slate-800 pb-3">
                  <div class="flex items-center gap-2">
                    <span class="w-3 h-3 rounded-full bg-rose-500 animate-pulse"></span>
                    <h4 class="text-sm font-heading font-black text-white uppercase tracking-wider">
                      ➕ Fast Log Match #${this.scrimMatches.length + 1}
                    </h4>
                  </div>
                  <span class="text-xs text-slate-400 font-mono">1-Tap Scrim Scorekeeper</span>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                  <div>
                    <label class="block text-[10px] font-sub uppercase font-bold text-slate-400 mb-1">Select Map</label>
                    <select id="scrimLogMap" class="w-full bg-slate-900 border border-slate-700 rounded-xl p-2.5 text-xs text-white outline-none focus:border-rose-400 font-sub">
                  ...
                  <div>
                    <label class="block text-[10px] font-sub uppercase font-bold text-slate-400 mb-1">Finish Placement (#1 to #12)</label>
                    <select id="scrimLogPlacement" class="w-full bg-slate-900 border border-slate-700 rounded-xl p-2.5 text-xs text-amber-300 font-bold outline-none focus:border-amber-400 font-sub">
                      <option value="1">#1 Booyah (12 pts) 👑</option>
                      <option value="2">#2 Position (9 pts)</option>
                      <option value="3">#3 Position (8 pts)</option>
                      <option value="4">#4 Position (7 pts)</option>
                      <option value="5">#5 Position (6 pts)</option>
                      <option value="6">#6 Position (5 pts)</option>
                      <option value="7">#7 Position (4 pts)</option>
                      <option value="8">#8 Position (3 pts)</option>
                      <option value="9">#9 Position (2 pts)</option>
                      <option value="10">#10 Position (1 pt)</option>
                      <option value="11">#11 Position (0 pts)</option>
                      <option value="12">#12 Position (0 pts)</option>
                    </select>
                  </div>

                  <div>
                    <label class="block text-[10px] font-sub uppercase font-bold text-slate-400 mb-1">Total Squad Kills</label>
                    <input type="number" id="scrimLogKills" min="0" max="48" value="6" class="w-full bg-slate-900 border border-slate-700 rounded-xl p-2 text-xs text-white outline-none focus:border-rose-400 font-mono font-bold" />
                  </div>

                  <div>
                    <button id="addScrimMatchBtn" class="w-full py-2.5 bg-gradient-to-r from-rose-600 to-amber-600 hover:from-rose-500 hover:to-amber-500 text-white font-sub font-black uppercase text-xs rounded-xl tracking-wider shadow-lg transition-transform hover:scale-102 flex items-center justify-center gap-1.5">
              ...
              <div class="cyber-panel p-5 rounded-2xl border border-slate-800 bg-slate-950 space-y-4">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800 pb-3">
                  <h4 class="text-sm font-heading font-black text-white uppercase tracking-wider flex items-center gap-2">
                    📋 Logged Matches (${this.scrimMatches.length})
                  </h4>

                  <div class="flex items-center gap-2">
                    <button id="copyWhatsAppScorecardBtn" class="px-4 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-sub font-bold uppercase text-xs rounded-xl tracking-wider shadow-lg flex items-center gap-1.5 transition-transform hover:scale-105">
                      <span>📋</span>
                      <span id="copyScorecardBtnText">Copy WhatsApp / Discord Card</span>
                    </button>
                    <button id="resetScrimSetBtn" class="px-3 py-2 bg-slate-900 border border-slate-700 hover:border-slate-500 text-slate-400 hover:text-white text-xs rounded-xl font-sub transition-all" title="Clear all matches">
                      🔄 Reset Set
                    </button>
                  </div>
                </div>

                ${this.scrimMatches.length === 0 ? `
                  <div class="p-8 text-center text-xs text-slate-500">
                    No matches logged yet. Use the fast entry bar above to log Match 1!
                  </div>
                ` : `
                  <div class="overflow-x-auto">
                    <table class="w-full text-left text-xs text-slate-300">
                      <thead class="text-[10px] font-sub uppercase font-bold text-slate-400 border-b border-slate-800 bg-slate-900/60">
                        <tr>
                          <th class="p-3">Match</th>
                          <th class="p-3">Map</th>
                          <th class="p-3">Placement</th>
                          <th class="p-3">Placement Pts</th>
                          <th class="p-3">Kills</th>
                          <th class="p-3 font-bold text-white">Total Points</th>
                          <th class="p-3 text-right">Action</th>
                        </tr>
                      </thead>
                      <tbody class="divide-y divide-slate-800/60">
                        ${this.scrimMatches.map((m, idx) => `
                          <tr class="hover:bg-slate-900/40 transition-colors">
                            <td class="p-3 font-mono font-bold text-white">Match #${m.matchNum}</td>
                            <td class="p-3 text-rose-300 font-sub">${m.map}</td>
                            <td class="p-3">
                              <span class="px-2 py-0.5 rounded text-[11px] font-bold ${m.placement === 1 ? 'bg-amber-500/20 text-amber-300 border border-amber-500/40' : 'bg-slate-800 text-slate-300'}">
                                #${m.placement} ${m.placement === 1 ? '👑' : ''}
                              </span>
                            </td>
                            <td class="p-3 font-mono text-slate-400">${m.placementPts} pts</td>
                            <td class="p-3 font-mono text-cyan-400 font-bold">${m.kills} kills</td>
                            <td class="p-3 font-mono text-base font-black text-rose-400">${m.totalPts} pts</td>
                            <td class="p-3 text-right">
                              <button class="delete-scrim-match-btn text-rose-500 hover:text-rose-300 text-xs px-2 py-1 rounded bg-rose-950/40 hover:bg-rose-950 border border-rose-500/30 transition-all" data-match-idx="${idx}">
                                🗑️ Delete
                              </button>
                            </td>
                          </tr>
          ...
          <!-- ============================================================ -->
          <!-- MODE 2: FAMOUS INDIAN SCRIMS DIRECTORY (EAGLE ESPORTS, ETC) -->
          <!-- ============================================================ -->
          ${this.ffScrimMode === 'famous_scrims' ? `
            <div class="space-y-6 animate-fade-in">
              <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800 pb-3">
                <div>
                  <h4 class="text-base font-heading font-black text-white uppercase tracking-wider flex items-center gap-2">
                    🦅 Famous Indian Scrims & Tournament Directory
                  </h4>
                  <p class="text-xs text-slate-400 mt-0.5">
                    Verified tournament schedules, slot booking rules, and 1-click Discord registration templates for India's top scrim organizers:
                  </p>
                </div>
                <span class="px-3 py-1 rounded-full bg-amber-950 border border-amber-500/40 text-amber-300 text-xs font-mono font-bold shrink-0">
                  5 Premier Organizers
                </span>
              </div>

              <!-- Grid of Organizers -->
              <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
                ${famousScrims.map(org => `
                  <div class="cyber-panel p-5 rounded-2xl border border-slate-800 hover:border-amber-500/40 transition-all flex flex-col justify-between space-y-4 bg-slate-950/90 shadow-xl">
                    <div class="space-y-3">
                      <!-- Header -->
                      <div class="flex items-start justify-between gap-3 border-b border-slate-800/80 pb-3">
                        <div class="flex items-center gap-3">
                          <span class="text-3xl">${org.icon}</span>
                          <div>
                            <h5 class="text-base font-heading font-black text-white leading-tight">${org.name}</h5>
                            <span class="text-[11px] font-sub text-slate-400">${org.organizer}</span>
                          </div>
                        </div>
                        <span class="cyber-badge ${org.badgeColor} text-[10px] font-bold">
                          ${org.badge}
                        </span>
                      </div>
                      ...
                      <div class="grid grid-cols-2 gap-2 text-xs">
                        <div class="p-2.5 rounded-lg bg-slate-900 border border-slate-800">
                          <span class="text-[10px] font-sub uppercase font-bold text-slate-400 block">Timings:</span>
                          <span class="text-amber-300 font-sub font-bold">${org.timings}</span>
                        </div>
                        <div class="p-2.5 rounded-lg bg-slate-900 border border-slate-800">
                          <span class="text-[10px] font-sub uppercase font-bold text-slate-400 block">Match Set:</span>
                          <span class="text-slate-200 font-sub">${org.matchesPerSet}</span>
                        </div>
                      </div>

                      <!-- Booking Rules -->
                      <div class="space-y-1 text-xs">
                        <span class="text-[10px] font-sub uppercase font-bold text-slate-400 block">Slot Booking Rules:</span>
                        <ul class="space-y-1 text-slate-400 text-[11px] list-disc list-inside">
                          ${org.bookingRules.map(r => `<li>${r}</li>`).join('')}
                        </ul>
                      </div>

                      <!-- Pro Teams Competing -->
                      <div class="p-2.5 rounded-lg bg-slate-900/60 border border-slate-800 text-[11px] text-slate-300">
                        <strong class="text-amber-400">Regular Tier-1 Teams:</strong> ${org.proTeamsPlaying}
                      </div>
                    </div>
          ...
          <!-- ============================================================ -->
          <!-- MODE 3: CUSTOM ROOM ID/PASS QUICK DISPATCHER -->
          <!-- ============================================================ -->
          ${this.ffScrimMode === 'room_dispatcher' ? `
            <div class="space-y-6 animate-fade-in">
              <div class="cyber-panel p-6 rounded-2xl border border-cyan-500/40 bg-gradient-to-r from-slate-950 via-[#0a1827] to-slate-950 shadow-xl space-y-5">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-4">
                  <div>
                    <h4 class="text-base font-heading font-black text-white uppercase tracking-wider flex items-center gap-2">
                      🔑 Custom Room ID/Pass Quick Dispatcher
                    </h4>
                    <p class="text-xs text-slate-400 mt-0.5">
                      Paste messy WhatsApp or Discord room codes below to auto-extract credentials and forward to your teammates instantly:
                    </p>
                  </div>

                  <!-- 2-Min Entry Countdown Clock -->
                  <div id="roomTimerBox" class="p-3 px-5 rounded-2xl bg-slate-950 border-2 border-cyan-500/40 text-center shadow-lg shrink-0">
                    <div class="text-[10px] font-sub uppercase font-bold text-slate-400">Room Closes In</div>
                    <div id="roomTimerDisplay" class="text-2xl font-mono font-black text-cyan-300 mt-0.5">02:00</div>
                    <button id="startRoomCountdownBtn" class="mt-1 px-2.5 py-0.5 rounded bg-cyan-600 hover:bg-cyan-500 text-black font-sub font-bold text-[10px] uppercase transition-all">
                      ${this.roomDispatcher.timerActive ? 'Timer Active' : 'Start 2-Min Clock'}
                    </button>
                  </div>
                </div>

                <!-- Input Textarea for Auto-Extraction -->
                <div class="space-y-2">
                  <label class="block text-xs font-sub uppercase font-bold text-slate-300">
                    Paste Messy WhatsApp / Discord Room Message:
                  </label>
                  <textarea id="rawRoomTextInput" rows="2" class="w-full bg-slate-900 border border-slate-700 rounded-xl p-3 text-xs text-white font-mono outline-none focus:border-cyan-400" placeholder="e.g. Eagle Scrims Match 1 Room ID: 8941203 Pass: 1234 Slot: 7 Map: Bermuda"></textarea>
                  <div class="flex items-center gap-2">
                    <button id="parseRoomTextBtn" class="px-4 py-1.5 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-black font-sub font-bold uppercase text-xs tracking-wider transition-all">
                      ⚡ Auto-Extract Room Credentials
                    </button>
                    <button id="sampleRoomTextBtn" class="px-3 py-1.5 rounded-xl bg-slate-900 border border-slate-700 text-slate-400 hover:text-white text-xs font-sub transition-all">
                      Insert Sample Message
                    </button>
                  </div>
                </div>

                <!-- Extracted Credentials Cards -->
                <div class="grid grid-cols-1 sm:grid-cols-4 gap-3 pt-2">
                  <div class="p-4 rounded-xl bg-slate-900/90 border border-cyan-500/30 flex flex-col justify-between">
                    <div>
                      <span class="text-[10px] font-sub uppercase font-bold text-slate-400">Room ID</span>
                      <div class="text-xl font-mono font-black text-white mt-1">${this.roomDispatcher.roomId}</div>
                    </div>
                    <button id="copyRoomIdBtn" class="mt-3 px-3 py-1.5 rounded-lg bg-cyan-600/20 hover:bg-cyan-600/40 border border-cyan-500/40 text-cyan-300 font-sub text-xs font-bold uppercase transition-all">
                      Copy Room ID 📋
                    </button>
                  </div>

                  <div class="p-4 rounded-xl bg-slate-900/90 border border-amber-500/30 flex flex-col justify-between">
                    <div>
                      <span class="text-[10px] font-sub uppercase font-bold text-slate-400">Password</span>
                      <div class="text-xl font-mono font-black text-amber-300 mt-1">${this.roomDispatcher.pass}</div>
                    </div>
                    <button id="copyRoomPassBtn" class="mt-3 px-3 py-1.5 rounded-lg bg-amber-600/20 hover:bg-amber-600/40 border border-amber-500/40 text-amber-300 font-sub text-xs font-bold uppercase transition-all">
                      Copy Pass 📋
                    </button>
                  </div>

                  <div class="p-4 rounded-xl bg-slate-900/90 border border-purple-500/30 flex flex-col justify-between">
                    <div>
                      <span class="text-[10px] font-sub uppercase font-bold text-slate-400">Our Assigned Slot</span>
                      <div class="text-xl font-mono font-black text-purple-300 mt-1">Slot #${this.roomDispatcher.slot}</div>
                    </div>
                    <span class="mt-3 text-[11px] text-slate-400">Do not sit in wrong slot!</span>
                  </div>

                  <div class="p-4 rounded-xl bg-slate-900/90 border border-emerald-500/30 flex flex-col justify-between">
                    <div>
          ...
          <!-- ============================================================ -->
          <!-- MODE 4: SQUAD TACTICAL VOICE ROOM & DISCUSSION LOUNGE -->
          <!-- ============================================================ -->
          ${this.ffScrimMode === 'voice_room' ? `
            <div class="space-y-6 animate-fade-in">
              <div class="cyber-panel p-6 rounded-2xl border border-purple-500/40 bg-gradient-to-r from-slate-950 via-[#150a22] to-slate-950 shadow-xl space-y-6">
                ...
                <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-slate-800 pb-4">
                  <div>
                    <div class="flex items-center gap-2">
                      <span class="w-3 h-3 rounded-full ${this.voiceRoom.connected ? 'bg-emerald-400 animate-ping' : 'bg-slate-500'}"></span>
                      <span class="text-xs font-sub font-bold ${this.voiceRoom.connected ? 'text-emerald-400' : 'text-slate-500'} uppercase tracking-wider">
                        ${this.voiceRoom.connected ? 'VOICE ROOM ONLINE • LOBBY #07' : 'VOICE ROOM STANDBY • DISCONNECTED'}
                      </span>
                    </div>
                    <h4 class="text-xl font-heading font-black text-white uppercase tracking-wide mt-0.5">
                      🎙️ Squad Tactical Voice Discussion Lounge
                    </h4>
                  </div>

                  <!-- Audio Action Buttons -->
                  <div class="flex items-center gap-2 flex-wrap">
                    <button id="toggleVoiceConnectBtn" class="px-4 py-2 rounded-xl ${this.voiceRoom.connected ? 'bg-rose-600 hover:bg-rose-500' : 'bg-emerald-600 hover:bg-emerald-500'} text-white font-sub font-bold uppercase text-xs tracking-wider shadow-lg transition-all flex items-center gap-1.5">
                      <span>${this.voiceRoom.connected ? '🔴 Disconnect' : '🟢 Connect Voice'}</span>
                    </button>
                    <button id="toggleVoiceMuteBtn" class="px-3.5 py-2 rounded-xl ${this.voiceRoom.muted ? 'bg-rose-950 border border-rose-500/50 text-rose-300' : 'bg-slate-900 border border-slate-700 text-white'} font-sub font-bold text-xs uppercase transition-all" ${!this.voiceRoom.connected ? 'disabled opacity-50' : ''}>
                      <span>${this.voiceRoom.muted ? '🔇 Muted' : '🎤 Mic On'}</span>
                    </button>
                    <button id="toggleVoiceDeafenBtn" class="px-3.5 py-2 rounded-xl ${this.voiceRoom.deafened ? 'bg-amber-950 border border-amber-500/50 text-amber-300' : 'bg-slate-900 border border-slate-700 text-white'} font-sub font-bold text-xs uppercase transition-all" ${!this.voiceRoom.connected ? 'disabled opacity-50' : ''}>
                      <span>${this.voiceRoom.deafened ? '🔕 Deafened' : '🎧 Listening'}</span>
                    </button>
                    <button id="testVoiceRadioBeepBtn" class="px-3 py-2 rounded-xl bg-purple-950/60 border border-purple-500/40 text-purple-300 hover:text-white font-sub text-xs uppercase transition-all">
                      📻 Radio Check
                    </button>
                  </div>
                </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  ${this.voiceRoom.seats.map(seat => `
                    <div class="p-4 rounded-2xl border transition-all text-center space-y-2 ${seat.status === 'speaking' ? 'bg-purple-950/40 border-purple-400 ring-2 ring-purple-400 shadow-xl' : 'bg-slate-900/80 border-slate-800'}" data-voice-seat="${seat.id}">
                      <div class="w-14 h-14 mx-auto rounded-2xl bg-purple-500/10 border border-purple-500/30 flex items-center justify-center text-2xl relative">
                      ...
                      <div>
                        <div class="text-xs font-heading font-black text-white truncate">${seat.name}</div>
                        <div class="text-[10px] text-purple-400 font-sub truncate">${seat.role}</div>
                      </div>

                      <div class="voice-seat-status text-[10px] font-mono ${seat.status === 'speaking' ? 'text-emerald-400 font-bold' : seat.status === 'muted' ? 'text-rose-400' : 'text-slate-400'}">
                        ${seat.status === 'speaking' ? 'Speaking 🟢' : seat.status === 'muted' ? 'Muted 🔴' : 'Listening 🎧'}
                      </div>
                    </div>
                  `).join('')}
                </div>

                <!-- Live Strategy Scratchpad & Quick Tactical Pings -->
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 pt-2">
                  <!-- Strategy Scratchpad -->
                  <div class="lg:col-span-2 space-y-2">
                    <label class="block text-xs font-sub uppercase font-bold text-slate-300">
                      📝 Live Match Strategy Scratchpad (Shared with Squad):
                    </label>
                    <textarea id="voiceRoomNotesInput" rows="3" class="w-full bg-slate-900 border border-slate-700 rounded-xl p-3 text-xs text-white font-mono outline-none focus:border-purple-400" placeholder="Type tactical notes during voice discussion...">${this.voiceRoom.notes}</textarea>
                  </div>

                  <!-- Quick Tactical Pings -->
                  <div class="space-y-2">
                    <label class="block text-xs font-sub uppercase font-bold text-slate-300">
                      🔊 Quick Tactical Radio Pings:
                    </label>
                    <div class="grid grid-cols-2 gap-2">
                      <button class="voice-quick-ping-btn p-2 rounded-lg bg-slate-900 border border-rose-500/40 hover:border-rose-500 text-rose-300 text-[10px] font-sub font-bold uppercase transition-all" data-ping="CRASH NOW!">
                        🚨 CRASH NOW!
                      </button>
                      <button class="voice-quick-ping-btn p-2 rounded-lg bg-slate-900 border border-amber-500/40 hover:border-amber-500 text-amber-300 text-[10px] font-sub font-bold uppercase transition-all" data-ping="HOLD FIRE">
                        🛑 HOLD FIRE
                      </button>
                      <button class="voice-quick-ping-btn p-2 rounded-lg bg-slate-900 border border-cyan-500/40 hover:border-cyan-500 text-cyan-300 text-[10px] font-sub font-bold uppercase transition-all" data-ping="VEHICLE READY">
                        🚗 VEHICLES READY
                      </button>
                      <button class="voice-quick-ping-btn p-2 rounded-lg bg-slate-900 border border-purple-500/40 hover:border-purple-500 text-purple-300 text-[10px] font-sub font-bold uppercase transition-all" data-ping="COMM SILENCE">
                        📻 COMM SILENCE
                      </button>
                    </div>
                  </div>
          ...
          <!-- ============================================================ -->
          <!-- MODE 5: 12-TEAM LOBBY LEADERBOARD & BROADCAST EXPORTER -->
          <!-- ============================================================ -->
              ...
              <div class="cyber-panel p-4 rounded-2xl border border-amber-500/30 bg-slate-950/80 space-y-4">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-slate-800 pb-3">
                  ...
                  <div class="flex flex-wrap items-center gap-2">
                    ...
                    <button id="fillProLobbyBtn" class="px-3 py-2 bg-indigo-950/60 hover:bg-indigo-900/80 text-indigo-300 font-sub font-bold uppercase rounded-xl text-xs tracking-wider border border-indigo-500/40">
                      ⚡ Fill Pro Teams
                    </button>
                    <button id="resetLobbyScoresBtn" class="px-3 py-2 bg-rose-950/40 hover:bg-rose-900/60 text-rose-400 font-sub font-bold uppercase rounded-xl text-xs tracking-wider border border-rose-500/30">
                      🔄 Reset Scores
                    </button>
                  </div>
                </div>

                <!-- Tournament Metadata Inputs -->
                <div class="grid grid-cols-1 md:grid-cols-3 gap-3 pt-1">
                  <div>
                    <label class="block text-[10px] font-sub uppercase font-bold text-slate-400 mb-1">Tournament / Scrim Title</label>
                    <input type="text" id="tourneyTitleInput" value="${this.ffTourneyMeta.title}" class="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-xs text-white outline-none focus:border-amber-400 font-heading tracking-wider" placeholder="e.g. FREE FIRE MAX TIER-1 SCRIMS"/>
                  </div>
                  <div>
                    <label class="block text-[10px] font-sub uppercase font-bold text-slate-400 mb-1">Tournament Stage / Day</label>
                    <input type="text" id="tourneyStageInput" value="${this.ffTourneyMeta.stage}" class="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-xs text-white outline-none focus:border-amber-400 font-heading tracking-wider" placeholder="e.g. GRAND FINALS DAY 1"/>
                  </div>
                  <div>
                    <label class="block text-[10px] font-sub uppercase font-bold text-slate-400 mb-1">Organized By</label>
                    <input type="text" id="tourneyOrganizerInput" value="${this.ffTourneyMeta.organizer}" class="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-xs text-amber-300 outline-none focus:border-amber-400 font-heading tracking-wider" placeholder="e.g. UNDERDOG ESPORTS"/>
                  </div>
                </div>
              </div>

              <!-- Graphic Preview Modal (Inline) -->
              <div id="leaderboardGraphicPreviewContainer" class="hidden cyber-panel p-4 rounded-2xl border border-rose-500/50 bg-slate-950 space-y-3">
                <div class="flex items-center justify-between border-b border-slate-800 pb-2">
                  <div class="flex items-center gap-2">
                    <span class="w-2 h-2 rounded-full bg-rose-500 animate-ping"></span>
                    <h4 class="text-xs font-heading font-bold uppercase text-white tracking-wider">Live Broadcast Graphic Canvas Preview</h4>
                  </div>
                  <button id="closeGraphicPreviewBtn" class="text-slate-400 hover:text-white text-xs font-mono">✕ Close Preview</button>
                </div>
                <div class="flex justify-center bg-slate-900/60 p-4 rounded-xl overflow-auto max-h-[500px]">
                  <img id="leaderboardPreviewImg" src="" alt="Leaderboard Preview" class="max-w-full md:max-w-md h-auto rounded-lg shadow-2xl border border-slate-800"/>
                </div>
              </div>

              <!-- Top 3 Podium Highlights -->
              <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div class="cyber-panel p-3.5 rounded-xl border border-amber-500/40 bg-gradient-to-b from-amber-950/20 to-slate-950 flex items-center justify-between">
                  <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-xl bg-amber-500/20 border border-amber-500/60 flex items-center justify-center text-lg font-black text-amber-400">#1</div>
                    <div>
                      <div class="text-[10px] font-sub uppercase font-bold text-amber-400">Lobby Leader</div>
                      <div class="text-sm font-heading font-black text-white truncate max-w-[140px]">${top1.name}</div>
                    </div>
                  </div>
                  <div class="text-right">
                    <div class="text-xl font-heading font-black text-amber-400">${top1.totalPts} <span class="text-[10px] font-mono text-slate-400">pts</span></div>
                    <div class="text-[10px] text-slate-400">${top1.booyahs} 🏆 • ${top1.kills} ⚔️</div>
                  </div>
                </div>

                <div class="cyber-panel p-3.5 rounded-xl border border-slate-400/30 bg-gradient-to-b from-slate-900/40 to-slate-950 flex items-center justify-between">
                  <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-xl bg-slate-700/30 border border-slate-400/50 flex items-center justify-center text-lg font-black text-slate-300">#2</div>
                    <div>
                      <div class="text-[10px] font-sub uppercase font-bold text-slate-300">2nd Position</div>
                      <div class="text-sm font-heading font-black text-white truncate max-w-[140px]">${top2.name}</div>
                    </div>
                  </div>
                  <div class="text-right">
                    <div class="text-xl font-heading font-black text-slate-300">${top2.totalPts} <span class="text-[10px] font-mono text-slate-400">pts</span></div>
                    <div class="text-[10px] text-slate-400">${top2.booyahs} 🏆 • ${top2.kills} ⚔️</div>
                  </div>
                </div>

                <div class="cyber-panel p-3.5 rounded-xl border border-amber-700/30 bg-gradient-to-b from-amber-950/10 to-slate-950 flex items-center justify-between">
                  <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-xl bg-amber-900/20 border border-amber-700/40 flex items-center justify-center text-lg font-black text-amber-600">#3</div>
                    <div>
                      <div class="text-[10px] font-sub uppercase font-bold text-slate-400">3rd Position</div>
                      <div class="text-sm font-heading font-black text-white truncate max-w-[140px]">${top3.name}</div>
                    </div>
                  </div>
                  <div class="text-right">
                    <div class="text-xl font-heading font-black text-amber-500">${top3.totalPts} <span class="text-[10px] font-mono text-slate-400">pts</span></div>
                    <div class="text-[10px] text-slate-400">${top3.booyahs} 🏆 • ${top3.kills} ⚔️</div>
                  </div>
                </div>
              </div>

              <!-- 12-Team Table -->
              <div class="cyber-panel p-4 rounded-2xl border border-slate-800 bg-slate-950 overflow-x-auto">
                <table class="w-full text-left text-xs text-slate-300">
                  <thead class="text-[10px] font-sub uppercase font-bold text-slate-400 border-b border-slate-800 bg-slate-900/60">
                    <tr>
                      <th class="p-3">#</th>
                      ...
                      <th class="p-3 text-center">Booyah 🏆</th>
                      <th class="p-3 text-center">Place Pts</th>
                      <th class="p-3 text-center">Kills ⚔️</th>
                      <th class="p-3 text-center font-bold text-white">Total Pts</th>
                  ...
                  <tbody class="divide-y divide-slate-800/60">
                    ${sortedTeams.map((team, idx) => `
                      <tr class="hover:bg-slate-900/40 transition-colors">
                        <td class="p-3 font-mono font-bold ${idx < 3 ? 'text-amber-400' : 'text-slate-500'}">#${team.rank}</td>
                        <td class="p-3 font-bold text-white">${team.name}</td>
                        <td class="p-3 font-mono text-slate-400">${team.tag}</td>
                        <td class="p-3 text-center font-mono">${team.matches}</td>
                        <td class="p-3 text-center font-mono text-amber-400 font-bold">${team.booyahs}</td>
                        <td class="p-3 text-center font-mono">${team.placementPts}</td>
                        <td class="p-3 text-center font-mono text-cyan-400">${team.kills}</td>
                        <td class="p-3 text-center font-mono font-bold text-rose-400 text-sm">${team.totalPts}</td>
                        <td class="p-3 text-right">
                          <button class="edit-team-score-btn text-xs text-slate-400 hover:text-white px-2 py-1 rounded bg-slate-900 border border-slate-700" data-team-id="${team.id}">
                            ✏️ Edit
                          </button>
                        </td>
                      </tr>
      ...
      this.bindFFScrimsEvents(container);
    }

    bindFFScrimsEvents(container) {
      // 1. Mode Switcher Buttons
      container.querySelectorAll('.ff-scrim-mode-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          const mode = btn.getAttribute('data-mode');
          if (mode) {
      ...
      // 2. 1-Tap Scorekeeper: Log Match Button
      const addMatchBtn = container.querySelector('#addScrimMatchBtn');
      if (addMatchBtn) {
        addMatchBtn.addEventListener('click', () => {
          const mapEl = container.querySelector('#scrimLogMap');
          const placementEl = container.querySelector('#scrimLogPlacement');
          const killsEl = container.querySelector('#scrimLogKills');
          if (mapEl && placementEl && killsEl) {
            const map = mapEl.value;
            const placement = parseInt(placementEl.value, 10);
            const kills = parseInt(killsEl.value, 10) || 0;
            this.logNewScrimMatch(map, placement, kills);
            this.playChimeSound('stage_complete');
          }
        });
      }

      // 3. Delete Match Button
      container.querySelectorAll('.delete-scrim-match-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          const idx = parseInt(btn.getAttribute('data-match-idx'), 10);
          if (!isNaN(idx)) this.deleteScrimMatch(idx);
        });
      });

      // 4. Reset Scrim Set
      const resetSetBtn = container.querySelector('#resetScrimSetBtn');
      if (resetSetBtn) {
        resetSetBtn.addEventListener('click', () => {
          if (confirm('Reset all logged matches in this set?')) {
            this.resetScrimMatches();
          }
        });
      }

      // 5. Copy WhatsApp / Discord Scorecard
      const copyScorecardBtn = container.querySelector('#copyWhatsAppScorecardBtn');
      if (copyScorecardBtn) {
        copyScorecardBtn.addEventListener('click', () => {
          const text = this.getWhatsAppScorecardString();
          if (navigator.clipboard) {
            navigator.clipboard.writeText(text).then(() => {
              const btnText = container.querySelector('#copyScorecardBtnText');
              if (btnText) {
                btnText.textContent = 'COPIED TO CLIPBOARD! ✅';
                setTimeout(() => { btnText.textContent = 'Copy WhatsApp / Discord Card'; }, 2000);
              }
            });
          }
        });
      ...
      // 7. Room Dispatcher: Parse Raw Text
      const parseRoomBtn = container.querySelector('#parseRoomTextBtn');
      if (parseRoomBtn) {
        parseRoomBtn.addEventListener('click', () => {
          const raw = container.querySelector('#rawRoomTextInput');
          if (raw && raw.value.trim()) {
            this.parseRoomText(raw.value);
            this.playRadioBeep();
          }
        });
      }

      const sampleRoomBtn = container.querySelector('#sampleRoomTextBtn');
      if (sampleRoomBtn) {
        sampleRoomBtn.addEventListener('click', () => {
          const raw = container.querySelector('#rawRoomTextInput');
          if (raw) {
            raw.value = 'Eagle Scrims Match 1 ID: 8941203 Pass: 1234 Slot: 7 Map: Bermuda';
            this.parseRoomText(raw.value);
          }
        });
      }

      // 8. Room Dispatcher: Copy ID & Pass
      const copyIdBtn = container.querySelector('#copyRoomIdBtn');
      if (copyIdBtn) {
        copyIdBtn.addEventListener('click', () => {
          if (navigator.clipboard) {
            navigator.clipboard.writeText(this.roomDispatcher.roomId).then(() => {
              copyIdBtn.textContent = 'Copied ID! ✅';
              setTimeout(() => { copyIdBtn.textContent = 'Copy Room ID 📋'; }, 1500);
            });
          }
        });
      }

      const copyPassBtn = container.querySelector('#copyRoomPassBtn');
      if (copyPassBtn) {
        copyPassBtn.addEventListener('click', () => {
          if (navigator.clipboard) {
            navigator.clipboard.writeText(this.roomDispatcher.pass).then(() => {
              copyPassBtn.textContent = 'Copied Pass! ✅';
              setTimeout(() => { copyPassBtn.textContent = 'Copy Pass 📋'; }, 1500);
            });
          }
        });
      }

      const copyForwardBtn = container.querySelector('#copySquadForwardBtn');
      if (copyForwardBtn) {
      ...
      const startCountdownBtn = container.querySelector('#startRoomCountdownBtn');
      ...
      // 9. Voice Room Controls
      const toggleVoiceBtn = container.querySelector('#toggleVoiceConnectBtn');
      if (toggleVoiceBtn) {
        toggleVoiceBtn.addEventListener('click', () => this.toggleVoiceRoomConnect());
      }

      const toggleMuteBtn = container.querySelector('#toggleVoiceMuteBtn');
      if (toggleMuteBtn) {
        toggleMuteBtn.addEventListener('click', () => this.toggleVoiceRoomMute());
      }

      const toggleDeafenBtn = container.querySelector('#toggleVoiceDeafenBtn');
      if (toggleDeafenBtn) {
        toggleDeafenBtn.addEventListener('click', () => this.toggleVoiceRoomDeafen());
      }

      const testRadioBtn = container.querySelector('#testVoiceRadioBeepBtn');
      if (testRadioBtn) {
        testRadioBtn.addEventListener('click', () => this.playRadioBeep());
      }

      // 10. Quick Voice Pings
      container.querySelectorAll('.voice-quick-ping-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          this.playRadioBeep();
          const ping = btn.getAttribute('data-ping');
          const notesInput = container.querySelector('#voiceRoomNotesInput');
          if (notesInput && ping) {
            notesInput.value += `\n[${new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}] 📢 ${ping}`;
            this.voiceRoom.notes = notesInput.value;
          }
        });
      });
      ...
      if (notesInput) {
        notesInput.addEventListener('input', (e) => {
          this.voiceRoom.notes = e.target.value;
        });
      }
      ...
      // 12. Open Tuition Time Lesson Modal
      document.querySelectorAll('.open-lesson-modal-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          const lessonId = btn.getAttribute('data-lesson-id');
          const lesson = this.getTuitionTimeLessons().find(l => l.id === lessonId);
          if (lesson) {
            this.showTuitionLessonModal(lesson);
          }
        });
      });

      // 13. Close VOD Modal
      const closeVodBtn = document.getElementById('closeVodModalBtn');
      const vodModal = document.getElementById('vodReviewModal');
      if (closeVodBtn && vodModal) {
        closeVodBtn.addEventListener('click', () => {
          vodModal.classList.add('hidden');
          const container = document.getElementById('vodPlayerContainer');
          if (container) container.innerHTML = '';
        });
      }
    }

    showVodModal(vodUrl, title, notes) {
      const modal = document.getElementById('vodReviewModal');
      const modalTitle = document.getElementById('vodModalTitle');
      const playerContainer = document.getElementById('vodPlayerContainer');
      const notesContainer = document.getElementById('vodModalNotesContainer');

      if (!modal || !playerContainer) return;

      modalTitle.innerText = title || 'Scrim Match VOD Review';
      notesContainer.innerHTML = `
        <div class="flex items-center gap-1.5 font-bold text-amber-400 uppercase text-[10px] mb-1">
          <span>📋</span> Coach / IGL Debrief Notes:
        </div>
        <p class="text-slate-200">${notes || 'No debrief notes recorded for this scrim match.'}</p>
        ${vodUrl ? `<div class="mt-2 text-[10px] text-slate-400 font-mono">Original Source: <a href="${vodUrl}" target="_blank" class="text-cyan-400 hover:underline">${vodUrl}</a></div>` : ''}
      `;

      // Embed YouTube if valid link
      let ytId = null;
      if (vodUrl) {
        const match = vodUrl.match(/(?:youtu\.be\/|youtube\.com\/(?:embed\/|v\/|watch\?v=|watch\?.+&v=))([\w-]{11})/);
        if (match && match[1]) ytId = match[1];
      }

      if (ytId) {
        playerContainer.innerHTML = `
          <iframe class="w-full h-full" src="https://www.youtube-nocookie.com/embed/${ytId}?autoplay=1" title="VOD Review Player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
        `;
      } else {
        playerContainer.innerHTML = `
          <div class="text-center p-6 space-y-3">
            <span class="text-4xl">🎬</span>
            <h5 class="text-sm font-heading font-bold text-white uppercase">VOD Clip Telemetry Ready</h5>
            <p class="text-xs text-slate-400 max-w-sm mx-auto">
              Clip saved at: <strong class="text-cyan-300 font-mono">${vodUrl || 'Simulated Match Replay'}</strong>
            </p>
            ${vodUrl ? `
              <a href="${vodUrl}" target="_blank" class="inline-block px-4 py-2 bg-gradient-to-r from-cyan-500 to-blue-600 text-black font-sub font-bold uppercase rounded-lg text-xs tracking-wider">
                Open External Stream / Clip ↗
              </a>
            ` : `
              <div class="text-[11px] text-amber-400 font-mono">
                Tip: Enter a YouTube, Medal, or Google Drive link when logging scrims for 1-click video playback.
              </div>
            `}
          </div>
        `;
    ...
    showTuitionLessonModal(lesson) {
      ...
      modalTitle.innerText = `[Tuition Time] ${lesson.title}`;
      notesContainer.innerHTML = `
        <div class="space-y-2 text-xs">
          <div class="flex items-center justify-between border-b border-slate-800 pb-1.5">
            <span class="text-amber-400 font-bold uppercase text-[10px]">Coach Duker Masterclass Notes</span>
            <span class="text-slate-400 font-mono text-[10px]">Timestamp Marker: ${lesson.videoTimestamp}</span>
          </div>
          <p class="text-amber-200/90 italic font-serif leading-relaxed">"${lesson.tuitionNotes}"</p>
          <div class="p-2 rounded bg-emerald-950/30 border border-emerald-500/30 text-emerald-300 text-[11px]">
            <strong>Recommended Squad Counter-Drill:</strong> ${lesson.drillAction}
          </div>
        </div>
      `;

      playerContainer.innerHTML = `
        <div class="p-6 text-center space-y-3 bg-gradient-to-b from-slate-900 to-slate-950 w-full h-full flex flex-col items-center justify-center">
          <div class="w-14 h-14 rounded-2xl bg-amber-500/20 border border-amber-500/60 flex items-center justify-center text-2xl font-black text-amber-400 mb-1">
            🎓
          </div>
          <h5 class="text-sm font-heading font-black text-white uppercase tracking-wider">${lesson.title}</h5>
          <p class="text-xs text-slate-300 max-w-md mx-auto">${lesson.summary}</p>
          <div class="flex items-center gap-2 pt-2">
            <span class="cyber-badge bg-rose-950 border border-rose-500/50 text-rose-300 text-[10px]">STUDYING: ${lesson.proTeams.join(', ')}</span>
            <span class="cyber-badge bg-slate-800 text-slate-300 text-[10px]">TIMESTAMP ${lesson.videoTimestamp}</span>
          </div>
        </div>
      `;
    ...
    // --- BGMI / MULTI-GAME DEFAULT SCRIM TRACKER TAB ---
    renderDefaultScrimsTab(container) {
      container.innerHTML = `
        <div class="space-y-6 animate-fade-in">
          <div class="cyber-panel p-5 rounded-2xl border border-amber-500/40 bg-gradient-to-r from-slate-950 via-[#1b1509] to-slate-950 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div>
              <div class="flex items-center gap-2">
                <span class="text-2xl">🎯</span>
                <h3 class="text-xl font-heading font-black text-white uppercase tracking-wider">
                  Competitive Scrim Performance & Mistake Diagnostic Suite
                </h3>
                <span class="cyber-badge bg-amber-950 border border-amber-500/50 text-amber-300 text-[10px] font-bold">
                  BGIS / ESPORTS TIER-1
                </span>
              </div>
              <p class="text-xs text-slate-300 mt-1 max-w-2xl leading-relaxed">
                Log match placements, track team frags, and diagnose root-cause wipe mistakes to optimize rotations across Erangel, Miramar, and Sanhok.
              </p>
            </div>
            <span class="cyber-badge bg-slate-900 border border-slate-700 text-slate-300 text-xs font-mono">
              BGIS 10-PT SYSTEM
            </span>
          </div>
    ...
    // --- AIM & REACTION SPEED GYM TAB ---
    renderReflexTab(container) {
      container.innerHTML = `
        <div class="space-y-6 animate-fade-in">

          <!-- Main Gym Header -->
          <div class="cyber-panel p-5 rounded-2xl border border-cyan-500/40 bg-gradient-to-r from-slate-950 via-[#0a1829] to-slate-950 shadow-xl">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <div class="flex items-center gap-2">
                  <span class="w-3 h-3 rounded-full bg-cyan-400 animate-ping"></span>
                  <h3 class="text-xl font-heading font-black text-white uppercase tracking-wider">
                    Aim & Reaction Speed Gym &bull; Pro Neural Lab
                  </h3>
                  <span class="cyber-badge bg-cyan-950 text-cyan-300 border border-cyan-500/40 text-[10px]">
                    v3.2 Esports
                  </span>
                </div>
                <p class="text-xs text-slate-300 mt-1 max-w-3xl leading-relaxed">
                  Millisecond-precision motor latency testing. Master raw neural reaction, the <strong>0.15s Fast Gloo Wall Reflex</strong>, precision headshot flicking, and auditory stimulus to dominate Tier-1 LAN tournaments.
                </p>
              </div>

              <!-- Quick Audio Indicator -->
              <div class="flex items-center gap-2 bg-slate-900/90 px-3 py-1.5 rounded-xl border border-slate-800 text-xs text-slate-300 font-mono">
                <span>🔊 Web Audio:</span>
                <span class="text-emerald-400 font-bold">Active (Synthesized)</span>
              </div>
            </div>
          </div>

          <!-- Personal Records KPI HUD -->
          <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <div class="cyber-panel p-3.5 rounded-xl border border-cyan-500/30 bg-slate-950/80">
              <div class="text-[10px] font-sub uppercase tracking-wider text-slate-400">⚡ Best Pure Reflex</div>
              <div id="kpiBestVisual" class="text-2xl font-heading font-bold text-cyan-400 my-0.5">---</div>
              <div class="text-[10px] text-slate-500">Human Benchmark (Red/Green)</div>
            </div>

            <div class="cyber-panel p-3.5 rounded-xl border border-amber-500/30 bg-slate-950/80">
              <div class="text-[10px] font-sub uppercase tracking-wider text-slate-400">🛡️ Fastest Gloo Wall</div>
              <div id="kpiBestGloo" class="text-2xl font-heading font-bold text-amber-400 my-0.5">---</div>
              <div class="text-[10px] text-slate-500">Sub-200ms Sniper Block</div>
            </div>

            <div class="cyber-panel p-3.5 rounded-xl border border-rose-500/30 bg-slate-950/80">
              <div class="text-[10px] font-sub uppercase tracking-wider text-slate-400">🎯 Flick Accuracy</div>
              <div id="kpiBestAccuracy" class="text-2xl font-heading font-bold text-rose-400 my-0.5">---</div>
              <div class="text-[10px] text-slate-500">Headshot Arena Consistency</div>
            </div>

            <div class="cyber-panel p-3.5 rounded-xl border border-emerald-500/30 bg-slate-950/80 flex flex-col justify-between">
              <div>
                <div class="text-[10px] font-sub uppercase tracking-wider text-slate-400">👑 Reflex Athlete Tier</div>
                <div id="kpiReflexTier" class="text-xs font-heading font-bold text-emerald-400 my-1 uppercase tracking-wider">Unranked</div>
              </div>
              <button id="resetReflexRecordsBtn" class="text-[9px] font-mono text-slate-500 hover:text-rose-400 text-left underline">
                Reset Records
              </button>
            </div>
          </div>
          ...
          <div class="cyber-panel p-2.5 rounded-xl border border-slate-800 bg-slate-950/90 flex flex-wrap items-center justify-between gap-2.5">
            <div class="flex items-center gap-1.5 overflow-x-auto pb-1 sm:pb-0 w-full sm:w-auto">
              <button class="reflex-mode-btn px-3 py-1.5 rounded-lg text-xs font-sub font-bold uppercase tracking-wider border transition-all bg-cyan-500/20 text-cyan-300 border-cyan-400 shadow-[0_0_15px_rgba(0,240,255,0.2)] whitespace-nowrap" data-mode="visual">
                ⚡ 1. Pure Neural (Red/Green)
              </button>
              <button class="reflex-mode-btn px-3 py-1.5 rounded-lg text-xs font-sub font-bold uppercase tracking-wider border transition-all bg-slate-900 text-slate-400 border-slate-800 hover:border-slate-600 whitespace-nowrap" data-mode="gloo">
                🛡️ 2. 0.15s Fast Gloo Wall
              </button>
              <button class="reflex-mode-btn px-3 py-1.5 rounded-lg text-xs font-sub font-bold uppercase tracking-wider border transition-all bg-slate-900 text-slate-400 border-slate-800 hover:border-slate-600 whitespace-nowrap" data-mode="targets">
                🎯 3. Precision Headshots
              </button>
              <button class="reflex-mode-btn px-3 py-1.5 rounded-lg text-xs font-sub font-bold uppercase tracking-wider border transition-all bg-slate-900 text-slate-400 border-slate-800 hover:border-slate-600 whitespace-nowrap" data-mode="audio">
                🔊 4. Auditory Reaction
              </button>
              <button class="reflex-mode-btn px-3 py-1.5 rounded-lg text-xs font-sub font-bold uppercase tracking-wider border transition-all bg-slate-900 text-slate-400 border-slate-800 hover:border-slate-600 whitespace-nowrap" data-mode="blitz">
                ⏱️ 5. 30s Rapid Blitz
              </button>
            </div>

            <!-- Start / Control Button -->
            <button id="startReflexBtn" class="w-full sm:w-auto px-6 py-2 bg-gradient-to-r from-cyan-500 via-blue-600 to-indigo-600 hover:from-cyan-400 hover:to-blue-500 text-black font-sub font-black uppercase rounded-lg text-xs tracking-wider shadow-lg shadow-cyan-950/40">
              ⚡ Start Test (5 Rounds)
            </button>
          </div>

          <!-- Interactive Reaction Arena Box -->
          <div class="cyber-panel p-3 rounded-2xl border border-cyan-500/30 bg-slate-950 relative shadow-2xl">
            <div id="aimArenaBox" class="aim-arena w-full min-h-[360px] rounded-2xl relative flex flex-col items-center justify-center cursor-pointer select-none border border-slate-800">
              <!-- Rendered dynamically by ReflexTrainer -->
            </div>

            <!-- Real-time Status / Telemetry Box -->
            <div id="reflexStatsBox" class="mt-3 p-3 bg-slate-900/90 rounded-xl border border-slate-800">
              <div class="flex items-center justify-between text-xs font-mono text-slate-400">
                <span>Status: Arena Ready &bull; Select a mode above and click <strong>"Start Test"</strong></span>
                <span class="text-cyan-400">Spacebar / Tap Enabled</span>
              </div>
            </div>
          </div>

          <!-- TIER-1 ESPORTS LATENCY BENCHMARK LADDER -->
          <div class="cyber-panel p-5 rounded-2xl border border-amber-500/40 bg-gradient-to-r from-slate-950 via-[#16120b] to-slate-950">
            <div class="flex items-center gap-2 mb-3">
              <span class="w-2.5 h-2.5 rounded-full bg-amber-400 animate-pulse"></span>
              <h4 class="text-base font-heading font-bold text-white uppercase tracking-wider">
                Official Tier-1 Esports Latency Benchmark Ladder
              </h4>
            </div>
            <p class="text-xs text-slate-300 mb-4 leading-relaxed">
              In competitive Free Fire and BGMI, millisecond reaction speed dictates who wins 50-50 entry duels and whether gloo walls deploy before high-caliber sniper bullets hit your head:
            </p>

            <div class="overflow-x-auto">
              <table class="w-full text-left text-xs border-collapse">
                <thead>
                  <tr class="border-b border-slate-800 text-[10px] font-sub uppercase text-slate-400 bg-slate-900/60">
                    <th class="p-2.5">Latency Bracket</th>
                    <th class="p-2.5">Competitive Tier</th>
                    <th class="p-2.5">Reference Esports Athletes</th>
                    <th class="p-2.5">Combat Advantage Unlocked</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-slate-800/60 font-mono text-[11px]">
                  <tr class="hover:bg-amber-500/5 transition-colors">
                    <td class="p-2.5 text-amber-400 font-bold">⚡ Sub-180 ms</td>
                    <td class="p-2.5"><span class="px-2 py-0.5 rounded bg-amber-500/20 text-amber-300 font-bold">God-Tier Phenom</span></td>
                    <td class="p-2.5 text-white">TheCruz (EVOS), Jonathan (GodLike), Spower</td>
                    <td class="p-2.5 text-slate-300">0.15s double-gloo wall box, point-blank shotgun flick headshots, dodges M82B anti-material fire.</td>
                  </tr>
                  <tr class="hover:bg-emerald-500/5 transition-colors">
                    <td class="p-2.5 text-emerald-400 font-bold">⚡ 180 - 215 ms</td>
                    <td class="p-2.5"><span class="px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-300 font-bold">LAN Tournament Ready</span></td>
                    <td class="p-2.5 text-white">Bops, Dewa, ClutchGod, Manya, FozyAjay</td>
                    <td class="p-2.5 text-slate-300">Fast entry-fragger pacing, trades knocks in under 0.3s, 1.5s cooked grenade baiting.</td>
                  </tr>
                  <tr class="hover:bg-cyan-500/5 transition-colors">
                    <td class="p-2.5 text-cyan-400 font-bold">⚡ 215 - 255 ms</td>
                    <td class="p-2.5"><span class="px-2 py-0.5 rounded bg-cyan-500/20 text-cyan-300 font-bold">Competitive Scrim Tier</span></td>
                    <td class="p-2.5 text-white">Tier-2 / Tier-3 Discord Scrim Grinders</td>
                    <td class="p-2.5 text-slate-300">Solid DMR sniper holding, compound anchor, secondary re-fragging.</td>
                  </tr>
                  <tr class="hover:bg-slate-500/5 transition-colors">
                    <td class="p-2.5 text-slate-400 font-bold">⚡ 255 - 295 ms</td>
                    <td class="p-2.5"><span class="px-2 py-0.5 rounded bg-slate-800 text-slate-300">Grassroots Baseline</span></td>
                    <td class="p-2.5 text-slate-400">Average Human Baseline</td>
                    <td class="p-2.5 text-slate-400">Susceptible to being wiped by sub-200ms entry rushers. Requires hand warmup routine.</td>
                  </tr>
                  <tr class="hover:bg-rose-500/5 transition-colors">
                    <td class="p-2.5 text-rose-400 font-bold">⚡ 295 ms+</td>
                    <td class="p-2.5"><span class="px-2 py-0.5 rounded bg-rose-950/60 text-rose-300">Hardware Lag / Cold</span></td>
                    <td class="p-2.5 text-slate-400">Pre-Warmup / 60Hz Input Bottleneck</td>
                    <td class="p-2.5 text-rose-300">Check touchscreen sampling rate and wash hands in warm water immediately.</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <!-- TOUCHSCREEN HARDWARE & INPUT LAG CALIBRATION LAB -->
          <div class="cyber-panel p-5 rounded-2xl border border-cyan-500/40 bg-gradient-to-r from-slate-950 via-[#0d1728] to-slate-950">
            <div class="flex items-center gap-2 mb-3">
            ...
            <p class="text-xs text-slate-300 mb-4 leading-relaxed">
              Your true in-game reaction latency equals <strong>Neural Latency + Device Touch Input Lag</strong>. Shave off 30–60ms by tuning your hardware settings:
            </p>

            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <!-- Card 1 -->
              <div class="p-3.5 rounded-xl bg-slate-900/90 border border-slate-800">
                <div class="text-cyan-400 font-heading font-bold text-xs uppercase mb-1">1. Touch Sampling Rate</div>
                <div class="text-xl font-heading font-black text-white my-1">240Hz+ <span class="text-xs font-normal text-slate-400">(4.1ms)</span></div>
                <p class="text-[11px] text-slate-400 leading-relaxed">
                  60Hz touch sampling takes 16.6ms to register a tap. Enabling 240Hz/360Hz touch polling in Game Booster shaves off <strong>12.5ms</strong> instantly.
                </p>
              </div>

              <!-- Card 2 -->
              <div class="p-3.5 rounded-xl bg-slate-900/90 border border-slate-800">
                <div class="text-amber-400 font-heading font-bold text-xs uppercase mb-1">2. Carbon Thumb Sleeves</div>
                <div class="text-xl font-heading font-black text-white my-1">&mu; &le; 0.05 <span class="text-xs font-normal text-slate-400">Friction</span></div>
                <p class="text-[11px] text-slate-400 leading-relaxed">
                  Finger sweat increases static friction by 300%. Carbon fiber thumb sleeves eliminate drag resistance, ensuring smooth 0.15s gloo swipes.
                </p>
              </div>

              <!-- Card 3 -->
              <div class="p-3.5 rounded-xl bg-slate-900/90 border border-slate-800">
                <div class="text-rose-400 font-heading font-bold text-xs uppercase mb-1">3. Screen Protector Glass</div>
                <div class="text-xl font-heading font-black text-white my-1">0.2mm <span class="text-xs font-normal text-slate-400">High-Conduct</span></div>
                <p class="text-[11px] text-slate-400 leading-relaxed">
                  Cheap thick tempered glass attenuates screen capacitance. Use 0.2mm ultra-thin high-conductance glass or matte hydrogel film.
                </p>
              </div>

              <!-- Card 4 -->
              <div class="p-3.5 rounded-xl bg-slate-900/90 border border-slate-800">
                <div class="text-emerald-400 font-heading font-bold text-xs uppercase mb-1">4. Phone OS Tuning</div>
                <div class="text-xl font-heading font-black text-white my-1">MAX <span class="text-xs font-normal text-slate-400">Pointer Speed</span></div>
                <p class="text-[11px] text-slate-400 leading-relaxed">
                  In Android settings, set Pointer Speed to Maximum, Touch & Hold delay to Short (0.5s), and disable Battery Saver throttling.
                </p>
              </div>
            </div>
          </div>

          <!-- PRO 5-MINUTE PHYSIOLOGICAL HAND WARMUP PROTOCOL -->
          <div class="cyber-panel p-5 rounded-2xl border border-purple-500/40 bg-gradient-to-r from-slate-950 via-[#180f26] to-slate-950">
            <div class="flex items-center gap-2 mb-3">
              <span class="text-xl">✋</span>
              <h4 class="text-base font-heading font-bold text-white uppercase tracking-wider">
                Pro 5-Minute Hand & Reflex Warmup Protocol
              </h4>
            </div>
            <p class="text-xs text-slate-300 mb-4 leading-relaxed">
              Esports physiotherapists recommend running this 5-minute routine before official tournament lobbies and custom room matches:
            </p>

            <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-3">
              <div class="p-3 rounded-xl bg-slate-900/90 border border-purple-500/30">
                <div class="text-[10px] font-mono font-bold text-purple-400 uppercase">Step 1 (30s)</div>
                <div class="text-xs font-heading font-bold text-white mt-1 mb-1">🕷️ Spider Taps</div>
                <p class="text-[11px] text-slate-400 leading-tight">
                  Tap each finger independently in rapid sequence on a flat surface to prime motor cortex synapses for claw grip.
                </p>
              </div>

              <div class="p-3 rounded-xl bg-slate-900/90 border border-purple-500/30">
                <div class="text-[10px] font-mono font-bold text-purple-400 uppercase">Step 2 (45s)</div>
                <div class="text-xs font-heading font-bold text-white mt-1 mb-1">🔄 Wrist Extensors</div>
                <p class="text-[11px] text-slate-400 leading-tight">
                  Extend arm forward, gently pull fingers backward for 15s each hand to release carpal tunnel tension.
                </p>
              </div>

              <div class="p-3 rounded-xl bg-slate-900/90 border border-purple-500/30">
                <div class="text-[10px] font-mono font-bold text-purple-400 uppercase">Step 3 (60s)</div>
                <div class="text-xs font-heading font-bold text-white mt-1 mb-1">🌡️ Warm Water Rinse</div>
                <p class="text-[11px] text-slate-400 leading-tight">
                  Skin temperatures below 20°C decrease nerve velocity by <strong>25ms</strong>. Rinse hands with warm water before scrims.
                </p>
              </div>

              <div class="p-3 rounded-xl bg-slate-900/90 border border-purple-500/30">
                <div class="text-[10px] font-mono font-bold text-purple-400 uppercase">Step 4 (30s)</div>
                <div class="text-xs font-heading font-bold text-white mt-1 mb-1">👁️ 20-20-20 Shift</div>
                <p class="text-[11px] text-slate-400 leading-tight">
                  Alternate focus between phone screen and a distant wall (20 ft) every 3 seconds to warm up ciliary eye muscles.
                </p>
              </div>

              <div class="p-3 rounded-xl bg-slate-900/90 border border-purple-500/30">
                <div class="text-[10px] font-mono font-bold text-purple-400 uppercase">Step 5 (45s)</div>
                <div class="text-xs font-heading font-bold text-white mt-1 mb-1">🫁 Box Breathing</div>
                <p class="text-[11px] text-slate-400 leading-tight">
                  Inhale 4s &bull; Hold 4s &bull; Exhale 4s. Lowers resting heart rate and eliminates adrenaline micro-tremors in 1v1 clutch duels.
                </p>
              </div>
            </div>
          </div>
      ...
      container.querySelectorAll('.reflex-mode-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          const mode = btn.getAttribute('data-mode');
          if (mode && this.reflexTrainer) {
            this.reflexTrainer.setMode(mode);
            const startBtn = document.getElementById('startReflexBtn');
            if (startBtn) {
              if (mode === 'targets') startBtn.textContent = '🎯 Start Headshot Flick (10 Targets)';
              else if (mode === 'blitz') startBtn.textContent = '⏱️ Start 30s Rapid Blitz';
              else if (mode === 'gloo') startBtn.textContent = '🛡️ Start Gloo Reflex (5 Rounds)';
              else if (mode === 'audio') startBtn.textContent = '🔊 Start Audio Reaction (5 Rounds)';
              else startBtn.textContent = '⚡ Start Neural Test (5 Rounds)';
            }
          }
        });
      });
      ...
      if (startBtn) {
        startBtn.addEventListener('click', () => {
          if (this.reflexTrainer) {
            this.reflexTrainer.startTest();
          }
        });
      }

      // Bind Reset Records Button
      document.getElementById('resetReflexRecordsBtn')?.addEventListener('click', () => {
        if (confirm('Reset all saved personal reflex records and tiers?')) {
          if (this.reflexTrainer) {
            this.reflexTrainer.resetRecords();
          }
        }
      });
    }
    ...
    // --- PLAYBOOK & WARMUP PROTOCOLS (15-MIN WARMUP & BUILDING CALLOUTS) ---
    startWarmupTimer() {
      if (this.warmupTimer.active) return;
      this.warmupTimer.active = true;
      clearInterval(this.warmupTimer.interval);
      this.warmupTimer.interval = setInterval(() => {
        if (this.warmupTimer.timeLeft > 0) {
          this.warmupTimer.timeLeft--;
          this.updateWarmupUI();
        } else {
          // Current stage finished
          const currentStage = this.warmupTimer.stages[this.warmupTimer.currentStage];
          if (currentStage) currentStage.completed = true;

          this.playChimeSound('stage_complete');

          if (this.warmupTimer.currentStage < this.warmupTimer.stages.length - 1) {
            this.skipWarmupStage(1);
          } else {
            this.pauseWarmupTimer();
            this.warmupTimer.completed = true;
            this.claimWarmupStreak();
            this.playChimeSound('warmup_finish');
          }
          this.renderTabContent();
        }
      }, 1000);
    ...
    resetWarmupTimer() {
      this.pauseWarmupTimer();
      const current = this.warmupTimer.stages[this.warmupTimer.currentStage];
      this.warmupTimer.timeLeft = current ? current.duration : 180;
      this.updateWarmupUI();
    }

    skipWarmupStage(direction) {
      let newIdx = this.warmupTimer.currentStage + direction;
      if (newIdx < 0) newIdx = 0;
      if (newIdx >= this.warmupTimer.stages.length) newIdx = this.warmupTimer.stages.length - 1;

      this.warmupTimer.currentStage = newIdx;
      this.warmupTimer.timeLeft = this.warmupTimer.stages[newIdx].duration;
      this.renderTabContent();
    }

    toggleWarmupStageComplete(idx) {
      if (this.warmupTimer.stages[idx]) {
        this.warmupTimer.stages[idx].completed = !this.warmupTimer.stages[idx].completed;
        const allDone = this.warmupTimer.stages.every(s => s.completed);
        if (allDone && !this.warmupTimer.completed) {
          this.claimWarmupStreak();
          this.playChimeSound('warmup_finish');
        }
        this.renderTabContent();
    ...
    claimWarmupStreak() {
      this.warmupTimer.completed = true;
      this.warmupTimer.streak = (this.warmupTimer.streak || 0) + 1;
      localStorage.setItem('underdog_warmup_streak', this.warmupTimer.streak.toString());
      this.renderTabContent();
    }

    resetWarmupStreak() {
      this.warmupTimer.streak = 0;
      localStorage.setItem('underdog_warmup_streak', '0');
      this.warmupTimer.completed = false;
      this.warmupTimer.stages.forEach(s => s.completed = false);
      this.renderTabContent();
    }

    updateWarmupUI() {
      const timeDisplay = document.getElementById('warmupTimeDisplay');
      const toggleBtn = document.getElementById('warmupToggleBtn');

      if (timeDisplay) {
        timeDisplay.textContent = this.formatClockTime(this.warmupTimer.timeLeft);
      }
      if (toggleBtn) {
        toggleBtn.innerHTML = this.warmupTimer.active
          ? '<span>⏸️</span> Pause Warmup'
          : '<span>▶️</span> Start Stage Timer';
        toggleBtn.className = this.warmupTimer.active
          ? 'px-4 py-2 rounded-xl bg-amber-600 hover:bg-amber-500 text-white font-sub font-bold uppercase text-xs tracking-wider shadow-lg flex items-center gap-1.5 transition-all'
          : 'px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-sub font-bold uppercase text-xs tracking-wider shadow-lg flex items-center gap-1.5 transition-all';
      }
    }

    getBuildingCalloutsData() {
      return [
        {
          id: 'blue_double',
          category: 'classics',
          name: 'Blue Double-Door (Do-Manzil / Balcony House)',
          regional: 'Do-Manzil, Nil Ghor, Balcony Building',
          icon: '🏢',
          accent: 'cyan',
          hotspots: 'Pochinok, Peak, Mars Electric, Brasilia, Central',
          layout: 'Ground floor kitchen & living area with narrow 90° L-staircase choke point. 2nd floor features an open front balcony, rear escape window, and corner bedroom.',
          grenade: {
            cookTime: '2.2 Seconds',
            angle: 'Throw through 2nd floor East window from 35m outside.',
            trajectory: 'Direct wall-ricochet off bedroom back wall onto the staircase top landing.',
            impact: 'Guaranteed 180+ explosive damage to players waiting on staircase with shotguns.'
          },
          breach: [
            { step: '1', role: 'Support / Smoker', desc: 'Throw smoke grenade directly onto staircase choke to blind stairway campers.' },
            { step: '2', role: 'Rusher 1 & 2', desc: 'Use pickup truck roof or double gloo wall to boost directly onto 2nd floor balcony railing, bypassing the staircase.' },
            { step: '3', role: 'Rusher 3', desc: 'Simultaneous breach through ground entrance with MP40 / Shotgun.' },
            { step: '4', role: 'Sniper / Overwatch', desc: 'Hold 50m rear angle to eliminate players attempting to jump out the rear window.' }
          ],
          defensiveHold: [
            'Player 1 (Anchor): Prone behind 2nd floor bedroom bed with Woodpecker/AC80 covering doorway.',
            'Player 2 (Shotgun Rusher): Crouched on upper staircase corner with M1887/Charge Buster.',
            'Player 3 (Balcony Guard): Holding right-hand peek on balcony door; ready to drop gloo if grenade sound heard.',
            'Player 4 (Flex): Ground floor rotation or secondary crossfire watching rear window.'
          ],
          commsScript: 'Enemy squad in Blue Double! Do not push the stairs! Smoker smoke the staircase now! Rushers boost balcony! Sniper hold the rear jump-out!'
        },
        {
          id: 'l_shape',
          category: 'classics',
          name: 'L-Shape / Broken Roof House',
          regional: 'Broken L, Khula Chhat House, L-Mansion',
          icon: '🏚️',
          accent: 'amber',
          hotspots: 'Bimasakti Strip, Peak Ridge, Purgatory Fields, Moathouse',
          layout: 'L-shaped ground corridor with single straight wooden staircase. Upper floor has open damaged roof section with natural brick half-wall cover.',
          grenade: {
            cookTime: '1.8 Seconds',
            angle: 'Stand at outside 45° angle corner behind brick wall.',
            trajectory: 'High-arc bounce off the broken roof chimney directly into the roof peek corner.',
            impact: 'Flushes out roof snipers who are head-glitching the open sky ledge.'
          },
          breach: [
            { step: '1', role: 'Sniper', desc: 'Wall-bang through exterior wooden door/partitions with M82B to knock campers behind cover.' },
            { step: '2', role: 'Rusher 1', desc: 'Tatsuya dash through front entrance, deploy 0.15s sit-down gloo wall at bottom of stairs.' },
            { step: '3', role: 'Rusher 2', desc: 'Simultaneous flank from outside wooden ladder or side window.' },
            { step: '4', role: 'Anchor', desc: 'Lobs flashbang up stairwell right before rushers advance.' }
          ],
          defensiveHold: [
            'Player 1 (Roof Marksman): Head-glitch broken roof ridge with Marksman rifle covering 200m field.',
            'Player 2 (Close-Quarter Rusher): Crouched under staircase crevice with shotgun to catch blind sprinters.',
            'Player 3 (Second Floor Window): Watches side ladder and perimeter approach.',
            'Player 4 (Support): Drops landmine on 3rd stair step and holds 2nd floor hallway.'
          ],
          commsScript: 'L-Shape roof tagged 140! Wall-bang the wooden door with M82B! Two-man breach now, flash the stairs!'
        },
        {
          id: 'god_compound',
          category: 'multi_story',
          name: 'God Compound / Central Fortress',
          regional: 'Bermuda Mansion, Big House, Central Fortress',
          icon: '🏰',
          accent: 'purple',
          hotspots: 'Peak Center, Pochinok Crossroads, Sentosa Villa, Brasilia Heights',
          layout: 'High perimeter concrete wall with 2 metal gates, central 2-story luxury mansion, detached private vehicle garage, and open inner courtyard.',
          grenade: {
            cookTime: '2.5 Seconds',
            angle: 'Lob over perimeter wall corner at 60° elevation.',
            trajectory: 'Air-burst grenade detonates 2 meters above front door courtyard.',
            impact: 'Clears gloo walls and destroys parked vehicles holding the courtyard.'
          },
          breach: [
            { step: '1', role: 'Convoy Driver', desc: 'Ram pickup truck / Jeep into perimeter gloo wall for instant barricade breach.' },
            { step: '2', role: 'Assaulter 1 & 2', desc: '2-Man entry through side garage door while deploying horizontal gloo tunnel.' },
            { step: '3', role: 'Assaulter 3 & 4', desc: 'Breach main entrance under cover of cluster flashbangs.' },
            { step: '4', role: 'Overwatch', desc: 'Climb garage roof to secure high ground over the inner compound.' }
          ],
          defensiveHold: [
            '2-2 Split Structure: 2 players in main mansion upper floor, 2 players holding garage roof.',
            'Vehicle Barrier: Park Jeep sideways across courtyard doorway as indestructible bullet shield.',
            'Crossfire Lines: Garage roof and mansion 2nd floor create a 90° lethal kill zone across the courtyard.'
          ],
          commsScript: 'Convoy ramming God Compound east gate! Pop two gloos, 2-2 split push! Clear the garage first, then push the main house!'
        },
        {
          id: 'three_story',
          category: 'multi_story',
          name: 'Three-Story Sniper Tower / Water Tower',
          regional: 'Tin-Manzil, Sniper Tower, Pochinok Tower',
          icon: '🗼',
          accent: 'rose',
          hotspots: 'Pochinok, Clock Tower, Katulistiwa, Cape Town',
          layout: 'Ultra-narrow vertical spiral staircase spanning 3 levels with 360° panoramic viewing slits on the 3rd level.',
          grenade: {
            cookTime: '3.0 Seconds',
            angle: 'Vertical upward lob through top floor window slit or 60m grenade launcher.',
            trajectory: 'Direct detonation inside the tight 3rd floor compartment.',
            impact: '100% squad wipe if multiple enemies are crammed in the small top room.'
          },
          breach: [
            { step: '1', role: 'GOLDEN RULE', desc: 'NEVER rush up the spiral staircase! A single shotgun player can wipe 4 players in single file.' },
            { step: '2', role: 'Gatekeep Hold', desc: 'Deploy 2 gloo walls blocking the bottom doorway. Trap the squad inside.' },
            { step: '3', role: 'Long-Range Sniping', desc: 'Step back 60m with Marksman/Sniper rifles and pick their heads through the window slits.' },
            { step: '4', role: 'Zone Pressure', desc: 'Wait for the zone to shrink; they will be forced to jump down and take 70+ fall damage.' }
          ],
          defensiveHold: [
            'Player 1 (Top Sniper): 360° Marksman reconnaissance covering all rotations.',
            'Player 2 (Shotgun Guard): Stationed on 2nd landing aiming directly down the spiral stairs with M1887.',
            'Emergency Exit: Keep 1 gloo wall ready to drop on ground when leaping from 3rd floor during zone shifts.'
          ],
          commsScript: 'Do not push the tower stairs! Trap the bottom door with gloo walls! Step back 50m and sniper pick the windows!'
        },
        {
          id: 'garage_barn',
          category: 'industrial',
          name: 'Garage House / Red Barn / Octagon',
          regional: 'Garage Ghor, Lal Barn, Octagon Bay',
          icon: '🚜',
          accent: 'emerald',
          hotspots: 'Mill Lower Dip, Rim Nam Village, Moathouse, Crossroads',
          layout: 'Wide open double garage bay with ground vehicle lift and a steep wooden ramp leading to a narrow 2nd-floor catwalk railing.',
          grenade: {
            cookTime: '2.0 Seconds',
            angle: 'Bank off the rear corrugated steel wall at 30° angle.',
            trajectory: 'Ricochets off steel plate directly onto the elevated catwalk floor.',
            impact: 'Knocks catwalk defenders head-glitching the upper railing.'
          },
          breach: [
            { step: '1', role: 'Smoker', desc: 'Smoke the wide ground entrance to obscure lines of sight from catwalk.' },
            { step: '2', role: 'Double Gloo Tunnel', desc: 'Deploy 2 horizontal gloo walls in tandem to create a safe corridor up the wooden ramp.' },
            { step: '3', role: 'Flash Rush', desc: 'Throw flashbang onto catwalk railing before sprinting up ramp.' },
            { step: '4', role: 'Rear Cut-off', desc: '1 player holds outside rear corner to intercept enemies dropping through back window.' }
          ],
          defensiveHold: [
            'Catwalk Head-Glitch: Railing provides 80% body concealment while giving full vision of ground entrance.',
            'Crossfire: 1 player on catwalk, 1 player behind ground vehicle stack creating vertical crossfire.',
            'Drop-Down Flank: Defender can jump off catwalk behind attackers as they push the ramp.'
          ],
          commsScript: 'Garage catwalk occupied! Smoke the bay, drop horizontal gloos, flash the ramp, and push the head-glitch!'
        },
        {
          id: 'cement_warehouse',
          category: 'industrial',
          name: 'Cement Warehouse / Mill Hangar',
          regional: 'Godown, Factory Bay, Mill Hangar',
          icon: '🏭',
          accent: 'blue',
          hotspots: 'Mill, Shipyard, Factory, Hangar, Lumber Mill',
          layout: 'Massive cavernous industrial hangar filled with multi-stacked steel shipping containers, high iron catwalk beams, and 4 large sliding vehicle doors.',
          grenade: {
            cookTime: '2.4 Seconds',
            angle: 'Bounce off steel shipping container corner into container gaps.',
            trajectory: 'Ricochets into the 2m blind gap between stacked containers.',
            impact: 'Wipes players healing or ratting in the narrow container alleys.'
          },
          breach: [
            { step: '1', role: 'Container Overwatch', desc: '1 player climbs container ladder to establish high-altitude angle over the whole floor.' },
            { step: '2', role: 'Pre-Fire Aisle Sweep', desc: 'Pre-fire shotgun/SMG around container corners at head level before peeking.' },
            { step: '3', role: 'Pincer Entry', desc: 'Enter simultaneously through North and South sliding doors to catch defenders in crossfire.' },
            { step: '4', role: 'Airborne Leap', desc: 'Leap from container tops with shotgun to shoot defenders over their gloo walls.' }
          ],
          defensiveHold: [
            'High-Ground Container Hold: Prone on top of 3-stack containers with Sniper/Marksman rifle.',
            'Doorway Crossfire: Position players behind container corners covering all 4 sliding door entries.',
            'Over-the-Wall Spray: Jump from container edges to shoot down into enemy gloo walls.'
          ],
          commsScript: 'Warehouse entry! Watch the high container tops! Pre-fire container corners, pincer from both sliding doors!'
        }
      ];
    }

    
    // --- SMART BACKPACK & CLUTCH IQ METHODS ---
    getBackpackData(role, level) {
      const maxCap = level === 2 ? 200 : 300;
      if (role === 'rusher') {
        return {
          roleName: '⚡ Entry Rusher & Fragger',
          capacity: maxCap,
          used: level === 2 ? 185 : 276,
          items: [
            { category: 'Weapons & Ammo', name: 'SMG Ammo (MP40 / Bizon)', qty: level === 2 ? '90 rounds' : '120 rounds', weight: level === 2 ? 45 : 60, icon: '🔫' },
            { category: 'Weapons & Ammo', name: 'Shotgun Shells (M1887 / MAG-7)', qty: level === 2 ? '18 shells' : '25 shells', weight: level === 2 ? 18 : 25, icon: '💥' },
            { category: 'Gloo Walls', name: 'Tactical Gloo Walls', qty: level === 2 ? '5 Gloos' : '7 Gloos', weight: level === 2 ? 50 : 70, icon: '🛡️' },
            { category: 'Medical', name: 'Standard Medkits', qty: '3 Medkits', weight: 30, icon: '🩹' },
            { category: 'Medical', name: 'Inhalers (Instant Cast)', qty: level === 2 ? '3 Inhalers' : '5 Inhalers', weight: level === 2 ? 15 : 25, icon: '💨' },
            { category: 'Medical', name: 'Super Medkit (+200 HP)', qty: level === 2 ? '0 (Skip)' : '1 Super Medkit', weight: level === 2 ? 0 : 15, icon: '💊' },
            { category: 'Throwables', name: 'Cooked Frag Grenade', qty: '2 Frags', weight: 24, icon: '💣' },
            { category: 'Throwables', name: 'Tactical Smoke Screen', qty: '1 Smoke', weight: 12, icon: '🌫️' }
          ],
          whatToDrop: [
            'Drop excess SMG ammo above 120! In competitive scrims, you pick up ammo from boxes of teams you wipe.',
            'Drop Lv 1 Muzzles and extra 2x/4x scopes: rushers need inventory space strictly for Gloo Walls.',
            'Never hold more than 3 standard Medkits: their 4.0s cast time gets you killed in fast close-range trades.'
          ]
        };
      } else if (role === 'assaulter') {
        return {
          roleName: '🎯 Mid-Range Assaulter & Flex',
          capacity: maxCap,
          used: level === 2 ? 190 : 272,
          items: [
            { category: 'Weapons & Ammo', name: 'AR Ammo (Groza / AK / SCAR)', qty: level === 2 ? '120 rounds' : '160 rounds', weight: level === 2 ? 60 : 80, icon: '🔫' },
            { category: 'Weapons & Ammo', name: 'Secondary Marksman Ammo', qty: level === 2 ? '50 rounds' : '80 rounds', weight: level === 2 ? 25 : 40, icon: '🎯' },
            { category: 'Gloo Walls', name: 'Tactical Gloo Walls', qty: level === 2 ? '4 Gloos' : '5 Gloos', weight: level === 2 ? 40 : 50, icon: '🛡️' },
            { category: 'Medical', name: 'Standard Medkits', qty: '3 Medkits', weight: 30, icon: '🩹' },
            { category: 'Medical', name: 'Inhalers (Movement Heals)', qty: '4 Inhalers', weight: 20, icon: '💨' },
            { category: 'Throwables', name: 'Frag Grenades (Air-Burst)', qty: level === 2 ? '2 Frags' : '3 Frags', weight: level === 2 ? 24 : 36, icon: '💣' },
            { category: 'Throwables', name: 'Flashbangs (Breach Assist)', qty: level === 2 ? '1 Flash' : '2 Flashes', weight: level === 2 ? 8 : 16, icon: '⚡' }
          ],
          whatToDrop: [
            'Drop AR ammo above 160: Carrying 300 AR ammo is the #1 mistake underdogs make. Trade 80 ammo for 2 grenades!',
            'Drop shotgun chokes or sniper stocks if not actively running secondary sniper.',
            'Drop duplicate level 2 vests: keep repair kits instead.'
          ]
        };
      } else {
        return {
          roleName: '🔭 Sniper Anchor & Recon Support',
          capacity: maxCap,
          used: level === 2 ? 175 : 249,
          items: [
            { category: 'Weapons & Ammo', name: 'Sniper Ammo (M82B / AWM)', qty: level === 2 ? '30 rounds' : '40 rounds', weight: level === 2 ? 30 : 40, icon: '🎯' },
            { category: 'Weapons & Ammo', name: 'Secondary DMR Ammo (Woodpecker)', qty: level === 2 ? '50 rounds' : '70 rounds', weight: level === 2 ? 25 : 35, icon: '🔫' },
            { category: 'Weapons & Ammo', name: 'Emergency Close SMG Ammo', qty: '20 rounds', weight: 10, icon: '💥' },
            { category: 'Gloo Walls', name: 'Tactical Gloo Walls', qty: level === 2 ? '5 Gloos' : '6 Gloos', weight: level === 2 ? 50 : 60, icon: '🛡️' },
            { category: 'Medical', name: 'Standard Medkits', qty: '4 Medkits', weight: 40, icon: '🩹' },
            { category: 'Medical', name: 'Inhalers & EP Boosters', qty: level === 2 ? '4 Inhalers' : '6 Inhalers', weight: level === 2 ? 20 : 30, icon: '💨' },
            { category: 'Throwables', name: 'Smoke Grenades (Rescue Smokes)', qty: '2 Smokes', weight: 24, icon: '🌫️' }
          ],
          whatToDrop: [
            'NEVER carry 70+ Sniper Ammo! Even in the longest 20-minute match, a sniper fires only 15 to 25 shots.',
            'Drop close-range AR attachments if running double sniper or sniper-marksman combo.',
            'Carry extra smoke grenades instead of frags: a sniper\'s primary duty is reviving knocked teammates from 80m away.'
          ]
        };
      }
    }
    ...
    getClutchScenarios() {
      return [
        {
          id: 'broken_wall',
          title: '🧱 The Broken Brick Wall Shotgun Standoff',
          badge: 'END-GAME 1V1',
          situation: 'Zone shrinks in 8 seconds. Enemy with M1887 is head-glitching a broken brick wall waiting for you. You have 1 Gloo Wall, MAG-7, and 140 HP.',
          choices: [
            {
              id: 'a',
              text: 'Jump straight over the broken wall with MAG-7 spray',
              winRate: '14%',
              isPro: false,
              outcome: '❌ Elimination! The enemy pre-aimed your falling head hitbox. While airborne, you cannot deploy gloo or adjust trajectory.'
            },
            {
              id: 'b',
              text: 'Deploy Gloo Wall at 45° left angle, fake-sprint left, then right-hand slide peek around brick corner',
              winRate: '84%',
              isPro: true,
              outcome: '👑 Clutch Won! Deploying the gloo forces the enemy to flick their crosshair left, breaking their pre-aim. Your right-hand peek gives you first-shot advantage.'
            },
            {
              id: 'c',
              text: 'Crouch behind cover and start cooking a 2.5s frag grenade',
              winRate: '42%',
              isPro: false,
              outcome: '⚠️ High Risk! The enemy hears the grenade pin pull and immediately rushes your corner with M1887 while you have no weapon in hand.'
            }
          ]
        },
        {
          id: 'balcony_jump',
          title: '🏢 Two-Story Balcony Jump vs Charge Buster',
          badge: 'STAIRWAY TRAP',
          situation: 'Enemy camper is on 2nd floor staircase with a fully charged Charge Buster. You are on ground floor. Zone is closing.',
          choices: [
            {
              id: 'a',
              text: 'Rush directly up the stairs with a Gloo Wall in hand',
              winRate: '19%',
              isPro: false,
              outcome: '❌ Headshot Taken! A fully charged Charge Buster shell hits for 240+ damage and can penetrate initial gloo wall deployment latency.'
            },
            {
              id: 'b',
              text: 'Drop smoke on staircase, climb exterior vehicle roof, and boost directly onto 2nd-floor balcony',
              winRate: '88%',
              isPro: true,
              outcome: '👑 Flawless Breach! Staircase camper is staring at the smoke expecting you. You enter from the balcony behind them and take an uncontested knock.'
            },
            {
              id: 'c',
              text: 'Wait on ground floor and hope the zone pushes them down',
              winRate: '35%',
              isPro: false,
              outcome: '⚠️ Coin Toss! The high-ground player retains 100% camera visibility and shoots you the moment you exit the doorway.'
            }
          ]
        },
        {
          id: 'open_field_duel',
          title: '🛡️ Open Field V-Wall Duel (50m, 1 Gloo Left)',
          badge: 'AIM DUEL',
          situation: 'Final circle in open grass with zero rocks. Enemy and you each have 1 Gloo Wall remaining, 50m apart. AC80 Marksman rifle in hand.',
          choices: [
            {
              id: 'a',
              text: 'Spam all your ammo at the enemy gloo wall to break it',
              winRate: '22%',
              isPro: false,
              outcome: '❌ Out-Played! Gloo walls have 800 HP. While you empty your magazine and get stuck in reload animation, enemy peeks and double-taps you.'
            },
            {
              id: 'b',
              text: 'Use third-person camera on right-hand edge, bait 1 enemy bullet, and headshot during their recoil reset',
              winRate: '81%',
              isPro: true,
              outcome: '👑 Headshot Knock! The right-hand camera peek allows you to see the enemy without exposing your hitbox. You shoot the split second their gun resets.'
            },
            {
              id: 'c',
              text: 'Sprint forward in a zigzag across the open field',
              winRate: '11%',
              isPro: false,
              outcome: '❌ Snipe Elimination! Open ground sprint has zero damage reduction. Any Tier-1 marksman rifle downs you in 2 shots.'
            }
          ]
        },
        {
          id: 'one_v_two_cross',
          title: '⚔️ 1v2 End-Game Crossfire Isolation',
          badge: '1V2 CLUTCH',
          situation: 'You are solo against a 2-man squad. Enemy 1 is at 10 o\'clock; Enemy 2 is at 2 o\'clock. Both are pushing simultaneously. You have 3 Gloo Walls.',
          choices: [
            {
              id: 'a',
              text: 'Backpedal and alternate single shots between both enemies',
              winRate: '12%',
              isPro: false,
              outcome: '❌ Crossfire Death! Fighting two enemies at different angles means your back is always exposed to one of them.'
            },
            {
              id: 'b',
              text: 'Deploy 2 Gloo Walls to completely blind Enemy 1\'s line of sight; aggressively shotgun-rush Enemy 2 in an isolated 1v1',
              winRate: '79%',
              isPro: true,
              outcome: '👑 1v2 Clutched! By blocking Enemy 1 with gloo walls, you turn an unwinnable crossfire into two separate 1v1 duels. You knock Enemy 2, then re-engage Enemy 1.'
            },
            {
              id: 'c',
              text: 'Build a 360° gloo wall box around yourself and heal to 200 HP',
              winRate: '28%',
              isPro: false,
              outcome: '⚠️ Trapped! Both enemies immediately surround your box, cook cluster grenades, and blast you through the walls.'
            }
          ]
        },
        {
          id: 'heal_off_smoke',
          title: '💨 Final Zone Heal-Off Smoke Screen',
          badge: '15 DPS COLLAPSE',
          situation: 'Final zone is completely closed (15 HP/sec DPS). Enemy is on other side of a stone. You have 1 Super Medkit and 2 Inhalers.',
          choices: [
            {
              id: 'a',
              text: 'Sprint around the stone to find and shoot the enemy',
              winRate: '25%',
              isPro: false,
              outcome: '❌ Dead in Zone! Blue zone drains 15 HP/s. In the 2 seconds it takes to locate the enemy, your HP drops to zero before you finish shooting.'
            },
            {
              id: 'b',
              text: 'Pop 1 Inhaler instantly for EP/HP buffer, crouch, and begin Super Medkit cast at 130 HP',
              winRate: '86%',
              isPro: true,
              outcome: '👑 Booyah! The Super Medkit restores +200 HP, out-healing standard medkits. The enemy dies to the 15 DPS zone while your cast completes.'
            },
            {
              id: 'c',
              text: 'Spam both inhalers while jumping in place',
              winRate: '45%',
              isPro: false,
              outcome: '⚠️ Close Loss! Inhalers restore 20 HP each, but 15 DPS drains 30 HP in 2 seconds. You run out of heals before the enemy.'
            }
          ]
        },
        {
          id: 'tower_sniper_bait',
          title: '🗼 High Tower Sniper Bait & Push',
          badge: 'TACTICAL ROTATION',
          situation: 'Enemy sniper is holding top of high three-story tower. You need to cross a 40m open road to enter the safe zone.',
          choices: [
            {
              id: 'a',
              text: 'Sprint across the road with knife in hand for maximum run speed',
              winRate: '8%',
              isPro: false,
              outcome: '❌ Snipe Elimination! A straight-line sprint from 40m is the easiest headshot line for an AWM/M82B.'
            },
            {
              id: 'b',
              text: 'Throw smoke grenade 15m away as a fake visual distraction, then deploy 2 zigzag Gloo Walls along low ditch',
              winRate: '83%',
              isPro: true,
              outcome: '👑 Safe Rotation! Sniper turns attention to scope the smoke cloud. By the time they realize you moved along the low ditch, you are safely inside the compound.'
            },
            {
              id: 'c',
              text: 'Stop in the open and try to trade headshots with an AR',
              winRate: '31%',
              isPro: false,
              outcome: '⚠️ Bad Trade! High-ground sniper has full helmet-glitch cover while your whole torso is exposed.'
            }
          ]
        }
      ];
    ...
    renderPlaybookTab(container) {
      if (!this.activePlaybookView) this.activePlaybookView = 'warmup';
      if (!this.calloutActiveCard) this.calloutActiveCard = 'blue_double';
      if (!this.calloutActiveFilter) this.calloutActiveFilter = 'all';

      const allBuildings = this.getBuildingCalloutsData();
      const filteredBuildings = this.calloutActiveFilter === 'all'
        ? allBuildings
        : allBuildings.filter(b => b.category === this.calloutActiveFilter);

      const activeBuilding = allBuildings.find(b => b.id === this.calloutActiveCard) || allBuildings[0];
      const currentStage = this.warmupTimer.stages[this.warmupTimer.currentStage] || this.warmupTimer.stages[0];
      const completedCount = this.warmupTimer.stages.filter(s => s.completed).length;

      const backpack = this.getBackpackData(this.backpackState.role, this.backpackState.level);
      const clutchScenarios = this.getClutchScenarios();
      const activeClutch = clutchScenarios[this.clutchTrainer.activeScenario] || clutchScenarios[0];

      container.innerHTML = `
        <div class="space-y-6 animate-fade-in">
          <!-- TOP PLAYBOOK CYBER BANNER -->
          <div class="cyber-panel p-5 rounded-2xl border border-amber-500/40 bg-gradient-to-r from-slate-950 via-[#1e1308] to-slate-950 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-xl">
            <div>
              <div class="flex items-center gap-2">
                <span class="text-2xl">📜</span>
                <h3 class="text-lg font-heading font-black text-white uppercase tracking-wider">
                  Warm-Up, Callouts, Smart Backpack & Clutch IQ
                </h3>
                <span class="cyber-badge bg-amber-950 text-amber-300 border border-amber-500/50 text-[10px] font-bold">
                  TOURNAMENT PRIMED
                </span>
              </div>
              <p class="text-xs text-slate-300 mt-1 max-w-3xl leading-relaxed">
                Activate your reflexes in the 15-min gym, inspect pro building callouts, optimize backpack loot capacity, and train 1v1 clutch split-second decisions.
              </p>
            </div>

            <!-- View Switcher Tabs -->
            <div class="flex items-center gap-1.5 bg-slate-950/90 p-1.5 rounded-xl border border-slate-800 shrink-0 flex-wrap">
              <button class="pb-nav-btn ${this.activePlaybookView === 'warmup' ? 'active bg-amber-600 text-black font-black shadow-lg' : 'bg-slate-900 text-slate-400 hover:text-white'} px-3 py-1.5 rounded-lg text-xs font-sub font-bold uppercase tracking-wider flex items-center gap-1 transition-all" data-view="warmup">
                <span>⏱️</span> 15-Min Warmup
              </button>
              <button class="pb-nav-btn ${this.activePlaybookView === 'callouts' ? 'active bg-amber-600 text-black font-black shadow-lg' : 'bg-slate-900 text-slate-400 hover:text-white'} px-3 py-1.5 rounded-lg text-xs font-sub font-bold uppercase tracking-wider flex items-center gap-1 transition-all" data-view="callouts">
                <span>🏢</span> Building Callouts
              </button>
              <button class="pb-nav-btn ${this.activePlaybookView === 'backpack' ? 'active bg-amber-600 text-black font-black shadow-lg' : 'bg-slate-900 text-slate-400 hover:text-white'} px-3 py-1.5 rounded-lg text-xs font-sub font-bold uppercase tracking-wider flex items-center gap-1 transition-all" data-view="backpack">
                <span>🎒</span> Smart Backpack
              </button>
              <button class="pb-nav-btn ${this.activePlaybookView === 'clutch' ? 'active bg-amber-600 text-black font-black shadow-lg' : 'bg-slate-900 text-slate-400 hover:text-white'} px-3 py-1.5 rounded-lg text-xs font-sub font-bold uppercase tracking-wider flex items-center gap-1 transition-all" data-view="clutch">
                <span>🧠</span> 1v1 Clutch IQ
              </button>
              <button class="pb-nav-btn ${this.activePlaybookView === 'mindset' ? 'active bg-amber-600 text-black font-black shadow-lg' : 'bg-slate-900 text-slate-400 hover:text-white'} px-3 py-1.5 rounded-lg text-xs font-sub font-bold uppercase tracking-wider flex items-center gap-1 transition-all" data-view="mindset">
                <span>🧘</span> Anti-Tilt
              </button>
            </div>
          </div>
          ...
          <!-- ========================================================= -->
          <!-- VIEW 1: 15-MINUTE PRE-SCRIM WARMUP STATION -->
          <!-- ========================================================= -->
          ${this.activePlaybookView === 'warmup' ? `
            <div class="space-y-6 animate-fade-in">
              <!-- LIVE MULTI-STAGE WARMUP TIMER -->
              <div class="cyber-panel p-6 rounded-2xl border border-amber-500/40 bg-gradient-to-r from-slate-950 via-[#181005] to-slate-950 shadow-2xl relative overflow-hidden">
                <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6 border-b border-slate-800 pb-6">
                  <div>
                    <div class="flex items-center gap-2">
                      <span class="w-3 h-3 rounded-full ${this.warmupTimer.active ? 'bg-amber-400 animate-ping' : 'bg-slate-500'}"></span>
                      <span class="text-xs font-sub font-bold text-amber-400 uppercase tracking-widest">
                        ${this.warmupTimer.active ? 'WARMUP IN PROGRESS • MUSCLE ACTIVATION' : 'STANDBY • READY TO ACTIVATE'}
                      </span>
                    </div>
                    <h4 class="text-xl font-heading font-black text-white uppercase tracking-wider mt-1 flex items-center gap-2">
                      <span>${currentStage.icon}</span> ${currentStage.title}
                      <span class="cyber-badge bg-amber-950 text-amber-300 border border-amber-500/40 text-[10px]">
                        STAGE ${this.warmupTimer.currentStage + 1} OF 4
                      </span>
                    </h4>
                  ...
                  <div class="flex items-center gap-4">
                    <div class="bg-slate-950 px-6 py-3 rounded-2xl border-2 ${this.warmupTimer.active ? 'border-amber-400 shadow-amber-500/20' : 'border-slate-800'} text-center shadow-xl">
                      <div class="text-[10px] font-sub uppercase font-bold text-slate-400">Stage Countdown</div>
                      <div id="warmupTimeDisplay" class="text-3xl lg:text-4xl font-mono font-black text-white tracking-widest mt-0.5">
                        ${this.formatClockTime(this.warmupTimer.timeLeft)}
                      </div>
                      <div class="text-[10px] font-sub text-amber-400 font-bold mt-0.5">Target: ${Math.floor(currentStage.duration/60)} Minutes</div>
                    </div>

                    <div class="flex flex-col gap-2">
                      <button id="warmupToggleBtn" class="px-4 py-2 rounded-xl ${this.warmupTimer.active ? 'bg-amber-600 hover:bg-amber-500' : 'bg-emerald-600 hover:bg-emerald-500'} text-white font-sub font-bold uppercase text-xs tracking-wider shadow-lg flex items-center gap-1.5 transition-all">
                        ${this.warmupTimer.active ? '<span>⏸️</span> Pause Warmup' : '<span>▶️</span> Start Stage Timer'}
                      </button>
                      <div class="flex items-center gap-2">
                        <button id="warmupPrevBtn" class="px-2.5 py-1.5 rounded-lg bg-slate-900 border border-slate-700 hover:border-slate-500 text-slate-300 hover:text-white text-xs font-sub font-bold transition-all" title="Previous Stage">
                          ⏮️ Prev
                        </button>
                        <button id="warmupNextBtn" class="px-2.5 py-1.5 rounded-lg bg-slate-900 border border-slate-700 hover:border-slate-500 text-slate-300 hover:text-white text-xs font-sub font-bold transition-all" title="Next Stage">
                          Next ⏭️
                        </button>
                        <button id="warmupResetBtn" class="px-2.5 py-1.5 rounded-lg bg-slate-900 border border-slate-700 hover:border-slate-500 text-slate-300 hover:text-white text-xs font-sub font-bold transition-all" title="Reset Stage Timer">
                          🔄 Reset
                        </button>
                      </div>
                    </div>
                ...
                <div class="mt-5 space-y-2">
                  <div class="flex justify-between items-center text-xs font-sub">
                    <span class="text-slate-400 uppercase font-bold">Overall Activation Progress:</span>
                    <span class="text-amber-400 font-mono font-bold">${completedCount} / 4 Stages Done (${Math.round((completedCount/4)*100)}%)</span>
                  </div>
                  <div class="w-full bg-slate-900 h-2.5 rounded-full overflow-hidden flex border border-slate-800">
                    ${this.warmupTimer.stages.map((s, idx) => `
                      <div class="h-full transition-all duration-300 ${s.completed ? 'bg-amber-400' : idx === this.warmupTimer.currentStage ? 'bg-amber-600 animate-pulse' : 'bg-slate-800'}" style="width: 25%"></div>
                    `).join('')}
                  </div>
                </div>
              </div>

              <!-- 4 GUIDED STAGE CHECKLIST CARDS -->
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ${this.warmupTimer.stages.map((stage, idx) => `
                  <div class="p-5 rounded-2xl border transition-all cursor-pointer ${idx === this.warmupTimer.currentStage ? 'bg-amber-950/20 border-amber-500/50 shadow-lg ring-1 ring-amber-400/40' : stage.completed ? 'bg-slate-900/90 border-emerald-500/40' : 'bg-slate-950/70 border-slate-800 hover:border-slate-700'}" data-stage-idx="${idx}">
                    <div class="flex items-start justify-between gap-3">
                      <div class="flex items-center gap-3">
                        <button class="stage-check-btn w-6 h-6 rounded-lg border flex items-center justify-center text-xs transition-all ${stage.completed ? 'bg-emerald-500 border-emerald-400 text-black font-black' : 'border-slate-700 bg-slate-900 text-transparent hover:border-slate-500'}" data-check-idx="${idx}">
                          ✓
                        </button>
                        <div>
                          <div class="text-[10px] font-sub uppercase font-bold ${idx === this.warmupTimer.currentStage ? 'text-amber-400' : 'text-slate-500'}">
                            Stage ${idx + 1} • ${Math.floor(stage.duration / 60)} Minutes
                          </div>
                          <h5 class="text-sm font-heading font-black text-white flex items-center gap-1.5 mt-0.5">
                            <span>${stage.icon}</span> ${stage.title}
                          </h5>
                        </div>
                      </div>

                      <span class="px-2.5 py-1 rounded-lg text-[10px] font-mono font-bold ${stage.completed ? 'bg-emerald-950 text-emerald-300 border border-emerald-500/40' : idx === this.warmupTimer.currentStage ? 'bg-amber-950 text-amber-300 border border-amber-500/40' : 'bg-slate-900 text-slate-500'}">
                        ${stage.completed ? 'COMPLETED ✅' : idx === this.warmupTimer.currentStage ? 'ACTIVE 🔥' : 'UPCOMING'}
                      </span>
                    </div>
                    ...
                    <div class="mt-3 p-2.5 rounded-xl bg-slate-950 border border-slate-800/80 text-[11px] text-amber-300/90 flex items-start gap-2">
                      <span class="text-xs shrink-0">💡</span>
                      <div><strong>Pro Tip:</strong> ${stage.proTip}</div>
                    </div>
                  </div>
                `).join('')}
              </div>

              <!-- TOURNAMENT PRIMED ACHIEVEMENT TROPHY CARD -->
              <div class="cyber-panel p-5 rounded-2xl border border-amber-500/30 bg-gradient-to-r from-slate-950 via-[#1b1206] to-slate-950 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div class="flex items-center gap-3">
                  <div class="w-12 h-12 rounded-2xl bg-amber-500/20 border border-amber-500/40 flex items-center justify-center text-2xl shadow-lg shadow-amber-950">
                    🏆
                  </div>
                  <div>
                    <h5 class="text-sm font-heading font-black text-white uppercase tracking-wider flex items-center gap-2">
                      Tournament Primed Discipline Tracker
                      <span class="cyber-badge bg-amber-950 text-amber-300 border border-amber-500/40 text-[10px]">
                        STREAK: ${this.warmupTimer.streak} DAYS
                      </span>
                    </h5>
                    <p class="text-xs text-slate-400 mt-0.5">
                      Never enter Discord scrims cold. Complete all 4 stages daily to build automatic muscle memory.
                    </p>
                  </div>
                </div>

                <div class="flex items-center gap-2 shrink-0">
                  <button id="claimWarmupStreakBtn" class="px-4 py-2 bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-400 hover:to-yellow-400 text-black font-sub font-black uppercase text-xs rounded-xl tracking-wider shadow-lg transition-transform hover:scale-105">
                    Mark All Done & Claim Streak 🔥
                  </button>
                  <button id="resetWarmupStreakBtn" class="px-3 py-2 bg-slate-900 border border-slate-700 hover:border-slate-500 text-slate-400 hover:text-white font-sub text-xs rounded-xl transition-all" title="Reset Daily Streak">
                    Reset
                  </button>
                </div>
              </div>
          ...
          <!-- ========================================================= -->
          <!-- VIEW 2: PRO BUILDING & COMPOUND CALLOUT FLASHCARDS -->
          <!-- ========================================================= -->
          ${this.activePlaybookView === 'callouts' ? `
            <div class="space-y-6 animate-fade-in">
              <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800 pb-3">
                <div>
                  <h4 class="text-base font-heading font-black text-white uppercase tracking-wider flex items-center gap-2">
                    🏢 Pro Compound & Building Callouts Directory
                  </h4>
                  <p class="text-xs text-slate-400 mt-0.5">
                    Select a building to view official pro callouts, high-risk grenade trajectories, and anti-staircase breach protocols:
                  </p>
                </div>

                <div class="flex items-center gap-1.5 bg-slate-950 p-1 rounded-xl border border-slate-800">
                  <button class="callout-filter-btn ${this.calloutActiveFilter === 'all' ? 'active bg-amber-500 text-black font-black' : 'text-slate-400 hover:text-white'} px-3 py-1.5 rounded-lg text-[11px] font-sub font-bold uppercase transition-all" data-filter="all">
                    All (6)
                  </button>
                  <button class="callout-filter-btn ${this.calloutActiveFilter === 'classics' ? 'active bg-amber-500 text-black font-black' : 'text-slate-400 hover:text-white'} px-3 py-1.5 rounded-lg text-[11px] font-sub font-bold uppercase transition-all" data-filter="classics">
                    Bermuda Classics
                  </button>
                  <button class="callout-filter-btn ${this.calloutActiveFilter === 'multi_story' ? 'active bg-amber-500 text-black font-black' : 'text-slate-400 hover:text-white'} px-3 py-1.5 rounded-lg text-[11px] font-sub font-bold uppercase transition-all" data-filter="multi_story">
                    Multi-Story
                  </button>
                  <button class="callout-filter-btn ${this.calloutActiveFilter === 'industrial' ? 'active bg-amber-500 text-black font-black' : 'text-slate-400 hover:text-white'} px-3 py-1.5 rounded-lg text-[11px] font-sub font-bold uppercase transition-all" data-filter="industrial">
                    Industrial
                  </button>
                </div>
              </div>

              <!-- 6 Building Selectable Cards Grid -->
              <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                ${filteredBuildings.map(b => `
                  <button class="callout-card-btn p-3.5 rounded-xl border text-left transition-all ${b.id === activeBuilding.id ? 'bg-amber-950/40 border-amber-400 ring-2 ring-amber-400/50 shadow-xl' : 'bg-slate-900/80 border-slate-800 hover:border-slate-600'}" data-card-id="${b.id}">
                    <div class="text-2xl mb-1.5">${b.icon}</div>
                    <div class="text-xs font-heading font-black text-white truncate">${b.name.split('(')[0]}</div>
                    <div class="text-[10px] text-amber-400 font-sub truncate mt-0.5">${b.regional.split(',')[0]}</div>
                  </button>
                `).join('')}
              </div>

              <!-- Active Building Details -->
              <div class="cyber-panel p-6 rounded-2xl border border-amber-500/40 bg-gradient-to-b from-slate-950 via-[#150d03] to-slate-950 space-y-6 shadow-2xl">
                <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-slate-800/80 pb-5">
                  <div class="flex items-center gap-3">
                    <div class="w-14 h-14 rounded-2xl bg-amber-500/10 border border-amber-500/30 flex items-center justify-center text-3xl shadow-lg">
                      ${activeBuilding.icon}
                    </div>
                    <div>
                      <div class="flex items-center gap-2">
                        <h4 class="text-xl font-heading font-black text-white uppercase tracking-wide">${activeBuilding.name}</h4>
                        <span class="cyber-badge bg-amber-950 text-amber-300 border border-amber-500/40 text-[10px]">PRO CALLOUT</span>
                      </div>
                      <div class="text-xs text-amber-400 font-sub font-bold mt-0.5">
                        Regional: <span class="text-slate-300">${activeBuilding.regional}</span>
                      </div>
                      <div class="text-[11px] text-slate-400 font-sub mt-0.5">
                        Locations: <span class="text-slate-200 font-bold">${activeBuilding.hotspots}</span>
                      </div>
                    </div>
                  </div>
                  <div class="p-3 rounded-xl bg-slate-900 border border-slate-800 max-w-md text-xs text-slate-300 leading-relaxed">
                    <strong>Structure Layout:</strong> ${activeBuilding.layout}
                  </div>
                </div>

                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div class="p-5 rounded-xl bg-slate-900/90 border border-rose-500/30 space-y-3">
                    <div class="flex items-center justify-between border-b border-slate-800 pb-2">
                      <h5 class="text-xs font-heading font-black text-rose-400 uppercase tracking-wider">💣 Grenade Trajectory Angle</h5>
                      <span class="text-[10px] text-rose-300 font-mono">Cook: ${activeBuilding.grenade.cookTime}</span>
                    </div>
                    <div class="text-xs text-slate-300 space-y-2">
                      <div><strong>Angle:</strong> ${activeBuilding.grenade.angle}</div>
                      <div><strong>Trajectory:</strong> ${activeBuilding.grenade.trajectory}</div>
                      <div class="p-2.5 rounded bg-rose-950/40 border border-rose-500/30 text-rose-200 text-[11px]">
                        <strong>Impact:</strong> ${activeBuilding.grenade.impact}
                      </div>
                    </div>
                  </div>

                  <div class="p-5 rounded-xl bg-slate-900/90 border border-cyan-500/30 space-y-3">
                    <h5 class="text-xs font-heading font-black text-cyan-400 uppercase tracking-wider border-b border-slate-800 pb-2">⚡ 4-Man Synchronized Breach Protocol</h5>
                    <div class="space-y-2">
                      ${activeBuilding.breach.map(b => `
                        <div class="flex items-start gap-2 text-xs bg-slate-950/60 p-2 rounded-lg border border-slate-800/80">
                          <span class="w-5 h-5 rounded bg-cyan-500/20 text-cyan-400 font-mono font-bold flex items-center justify-center shrink-0 text-[10px]">${b.step}</span>
                          <div><strong class="text-white">${b.role}:</strong> <span class="text-slate-300">${b.desc}</span></div>
                        </div>
                      `).join('')}
                    </div>
                  </div>
                ...
                <div class="p-4 rounded-xl bg-slate-900/80 border border-slate-800 flex flex-col md:flex-row justify-between items-center gap-4">
                  <div class="text-xs text-purple-300 italic">"${activeBuilding.commsScript}"</div>
                  <button class="copy-building-comms-btn shrink-0 px-3.5 py-2 rounded-xl bg-purple-600 hover:bg-purple-500 text-white text-xs font-sub font-bold uppercase transition-all" data-comms="${activeBuilding.commsScript}">
                    Copy Comms Script 📋
                  </button>
                </div>
              </div>
          ...
          <!-- ========================================================= -->
          <!-- VIEW 3: SMART BACKPACK & PRO LOOT ECONOMY MANAGER -->
          <!-- ========================================================= -->
          ${this.activePlaybookView === 'backpack' ? `
            <div class="space-y-6 animate-fade-in">
              <div class="cyber-panel p-6 rounded-2xl border border-emerald-500/40 bg-gradient-to-r from-slate-950 via-[#071a12] to-slate-950 space-y-6 shadow-xl">
                <!-- Header & Role Selectors -->
                <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-slate-800 pb-4">
                  <div>
                    <div class="flex items-center gap-2">
                      <span class="w-3 h-3 rounded-full bg-emerald-400 animate-pulse"></span>
                      <h4 class="text-base font-heading font-black text-white uppercase tracking-wider">
                        🎒 Smart Backpack & Pro Loot Economy Manager
                      </h4>
                    </div>
                    <p class="text-xs text-slate-400 mt-0.5">
                      Avoid utility starvation and ammo hoarding. Select your squad role and backpack tier to see mathematically optimal loadout ratios:
                    </p>
                  </div>

                  <!-- Role Selector Pills -->
                  <div class="flex items-center gap-2 flex-wrap bg-slate-950 p-1.5 rounded-xl border border-slate-800">
                    <button class="backpack-role-btn ${this.backpackState.role === 'rusher' ? 'active bg-emerald-600 text-white font-bold' : 'text-slate-400 hover:text-white'} px-3 py-1.5 rounded-lg text-xs font-sub transition-all" data-role="rusher">
                      ⚡ Rusher Bag
                    </button>
                    <button class="backpack-role-btn ${this.backpackState.role === 'assaulter' ? 'active bg-emerald-600 text-white font-bold' : 'text-slate-400 hover:text-white'} px-3 py-1.5 rounded-lg text-xs font-sub transition-all" data-role="assaulter">
                      🎯 Assaulter Bag
                    </button>
                    <button class="backpack-role-btn ${this.backpackState.role === 'sniper' ? 'active bg-emerald-600 text-white font-bold' : 'text-slate-400 hover:text-white'} px-3 py-1.5 rounded-lg text-xs font-sub transition-all" data-role="sniper">
                      🔭 Sniper Bag
                    </button>
                  </div>
                </div>

                <!-- Capacity & Level Switcher -->
                <div class="p-4 rounded-xl bg-slate-900 border border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    <span class="text-[10px] font-sub uppercase font-bold text-slate-400">Target Role Profile:</span>
                    <h5 class="text-sm font-heading font-black text-white">${backpack.roleName}</h5>
                  </div>

                  <!-- Level Switcher -->
                  <div class="flex items-center gap-2">
                    <span class="text-xs font-sub text-slate-400 font-bold uppercase">Backpack Tier:</span>
                    <button class="backpack-level-btn px-3 py-1 rounded-lg text-xs font-sub font-bold border transition-all ${this.backpackState.level === 2 ? 'bg-emerald-500/20 text-emerald-300 border-emerald-400' : 'bg-slate-950 text-slate-400 border-slate-700'}" data-level="2">
                      Level 2 (200 Units)
                    </button>
                    <button class="backpack-level-btn px-3 py-1 rounded-lg text-xs font-sub font-bold border transition-all ${this.backpackState.level === 3 ? 'bg-emerald-500/20 text-emerald-300 border-emerald-400' : 'bg-slate-950 text-slate-400 border-slate-700'}" data-level="3">
                      Level 3 (300 Units)
                    </button>
                  </div>

                  <div class="text-right">
                    <span class="text-[10px] font-sub uppercase font-bold text-slate-400">Total Capacity Used:</span>
                    <div class="text-lg font-mono font-black text-emerald-400">${backpack.used} / ${backpack.capacity} Units (${Math.round((backpack.used/backpack.capacity)*100)}%)</div>
                  </div>
                </div>
                  ...
                  ${backpack.items.map(item => `
                    <div class="p-3.5 rounded-xl bg-slate-900/90 border border-slate-800 space-y-1.5">
                      <div class="flex items-center justify-between">
                        ...
                        <span class="px-2 py-0.5 rounded bg-slate-950 border border-slate-800 text-[10px] font-mono text-emerald-300 font-bold">${item.weight} wt</span>
                      </div>
                      <div class="text-xs font-bold text-white leading-tight">${item.name}</div>
                      <div class="text-[11px] text-amber-300 font-sub font-bold">${item.qty}</div>
                    </div>
                  `).join('')}
                </div>

                <!-- What to Drop Critical Warnings -->
                <div class="p-4 rounded-xl bg-rose-950/30 border border-rose-500/40 space-y-2">
                  <div class="flex items-center gap-2">
                    <span class="text-base">⚠️</span>
                    <h5 class="text-xs font-heading font-black text-rose-400 uppercase tracking-wider">
                      Critical Underdog Blunder Alert: What to Drop Immediately
                    </h5>
                  </div>
                  <ul class="space-y-1 text-xs text-rose-200 list-disc list-inside">
                    ${backpack.whatToDrop.map(w => `<li>${w}</li>`).join('')}
                  </ul>
                </div>
              </div>
            </div>
          ...
          <!-- ========================================================= -->
          <!-- VIEW 4: 1V1 CLUTCH IQ & DECISION TRAINER -->
          <!-- ========================================================= -->
          ${this.activePlaybookView === 'clutch' ? `
            <div class="space-y-6 animate-fade-in">
              <div class="cyber-panel p-6 rounded-2xl border border-cyan-500/40 bg-gradient-to-r from-slate-950 via-[#0a1827] to-slate-950 space-y-6 shadow-xl">
                <!-- Header & Scenarios Selector -->
                <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-slate-800 pb-4">
                  <div>
                    <div class="flex items-center gap-2">
                      <span class="w-3 h-3 rounded-full bg-cyan-400 animate-pulse"></span>
                      <h4 class="text-base font-heading font-black text-white uppercase tracking-wider">
                        🧠 1v1 Clutch IQ & Split-Second Decision Trainer
                      </h4>
                    </div>
                    <p class="text-xs text-slate-400 mt-0.5">
                      Tournament matches are decided in 3-second 1v1 clutches. Test your decision-making against world championship benchmarks:
                    </p>
                  </div>
                </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
                  ${clutchScenarios.map((scen, idx) => `
                    <button class="clutch-scenario-btn p-3 rounded-xl border text-left transition-all ${idx === this.clutchTrainer.activeScenario ? 'bg-cyan-950/60 border-cyan-400 text-white ring-2 ring-cyan-400/40' : 'bg-slate-900 border-slate-800 text-slate-400 hover:text-white'}" data-scenario-idx="${idx}">
                      <div class="text-[10px] font-sub uppercase font-bold text-cyan-400">Scenario #${idx + 1}</div>
                      <div class="text-xs font-bold text-white truncate mt-0.5">${scen.title.split(' ')[1] || scen.title}</div>
                    </button>
                  `).join('')}
                </div>
                ...
                <div class="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 space-y-4">
                  ...
                  <p class="text-xs text-slate-300 bg-slate-950 p-3 rounded-xl border border-slate-800 leading-relaxed">
                    <strong>Match Situation:</strong> ${activeClutch.situation}
                  </p>

                  <!-- 3 Choices -->
                  <div class="space-y-3 pt-1">
                    <span class="text-[10px] font-sub uppercase font-bold text-slate-400 block">
                      Choose Your Clutch Move:
                    </span>
                    ${activeClutch.choices.map(c => {
                      const isSelected = this.clutchTrainer.selectedChoice === c.id;
                      return `
                        <div class="clutch-choice-card p-4 rounded-xl border cursor-pointer transition-all ${isSelected ? (c.isPro ? 'bg-emerald-950/40 border-emerald-400 ring-2 ring-emerald-400' : 'bg-rose-950/40 border-rose-500 ring-2 ring-rose-500') : 'bg-slate-950 border-slate-800 hover:border-slate-600'}" data-choice-id="${c.id}">
                          <div class="flex items-start justify-between gap-3">
                            <div class="text-xs font-bold text-white leading-relaxed">
                              Option ${c.id.toUpperCase()}: ${c.text}
                            </div>
                            ${isSelected ? `
                              <span class="px-2.5 py-0.5 rounded text-[10px] font-mono font-bold shrink-0 ${c.isPro ? 'bg-emerald-500 text-black' : 'bg-rose-500 text-white'}">
                                ${c.winRate} WIN RATE
                              </span>
                            ` : `
                              <span class="text-xs text-slate-500 shrink-0 font-sub">Click to Test 👆</span>
                            `}
                          </div>

                          ${isSelected ? `
                            <div class="mt-3 pt-3 border-t border-slate-800/80 text-xs ${c.isPro ? 'text-emerald-200' : 'text-rose-200'} leading-relaxed">
                              ${c.outcome}
                            </div>
                          ` : ''}
                        </div>
          ...
          <!-- ========================================================= -->
          <!-- VIEW 5: ESPORTS MINDSET & ANTI-TILT PROTOCOLS -->
          <!-- ========================================================= -->
          ${this.activePlaybookView === 'mindset' ? `
            <div class="space-y-6 animate-fade-in">
              <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div class="cyber-panel p-5 rounded-xl border border-slate-800 space-y-3">
                  <div class="text-2xl">🧘</div>
                  <h4 class="text-sm font-heading font-black text-white uppercase">The 5-Second Reset Rule</h4>
                  <p class="text-xs text-slate-300 leading-relaxed">
                    When you make a mistake or get knocked, you have exactly 5 seconds to call out enemy location and HP. Zero shrieking, zero complaining. The round is not over.
                  </p>
                  <div class="text-[11px] text-amber-400 font-mono">Rule: Complain in VOD review, not during the round.</div>
                </div>

                <div class="cyber-panel p-5 rounded-xl border border-slate-800 space-y-3">
                  <div class="text-2xl">🔄</div>
                  <h4 class="text-sm font-heading font-black text-white uppercase">Next Game Mentality</h4>
                  <p class="text-xs text-slate-300 leading-relaxed">
                    Tournaments are 6 matches long. Scoring zero points in Match 1 does not mean you cannot win. EVOS Phoenix scored 4 points in Match 1 and still took the World Championship trophy.
                  </p>
                  <div class="text-[11px] text-cyan-400 font-mono">Rule: Clean slate every 20 minutes.</div>
                </div>

                <div class="cyber-panel p-5 rounded-xl border border-slate-800 space-y-3">
                  <div class="text-2xl">🤫</div>
                  <h4 class="text-sm font-heading font-black text-white uppercase">Post-Knock Silence</h4>
                  <p class="text-xs text-slate-300 leading-relaxed">
                    If you die, mute your mic unless called upon. Audio priority belongs to the survivors listening for directional footstep frequencies and reload sounds.
                  </p>
                  <div class="text-[11px] text-rose-400 font-mono">Rule: Dead players do not IGL.</div>
                </div>

                <div class="cyber-panel p-5 rounded-xl border border-slate-800 space-y-3">
                  <div class="text-2xl">🫁</div>
                  <h4 class="text-sm font-heading font-black text-white uppercase">Box Breathing Clutch</h4>
                  <p class="text-xs text-slate-300 leading-relaxed">
                    Adrenaline causes finger micro-tremors in 1v1 situations. Inhale 4s, hold 4s, exhale 4s. Lowers heart rate by 12 BPM in 30 seconds and restores drag accuracy.
                  </p>
                  <div class="text-[11px] text-emerald-400 font-mono">Rule: Calm hands win 1v1 clutches.</div>
                </div>
              </div>
            </div>
          ` : ''}
      ...
      this.bindPlaybookEvents(container);
    }

    bindPlaybookEvents(container) {
      // 1. Playbook View Switcher (Warmup / Callouts / Backpack / Clutch / Mindset)
      container.querySelectorAll('.pb-nav-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          const view = btn.getAttribute('data-view');
          if (view) {
            this.activePlaybookView = view;
            this.renderTabContent();
          }
        });
      });

      // 2. Warmup Timer Controls
      const toggleBtn = container.querySelector('#warmupToggleBtn');
      if (toggleBtn) {
        toggleBtn.addEventListener('click', () => {
          if (this.warmupTimer.active) {
            this.pauseWarmupTimer();
          } else {
      ...
      const prevBtn = container.querySelector('#warmupPrevBtn');
      if (prevBtn) {
        prevBtn.addEventListener('click', () => this.skipWarmupStage(-1));
      }

      const nextBtn = container.querySelector('#warmupNextBtn');
      if (nextBtn) {
        nextBtn.addEventListener('click', () => this.skipWarmupStage(1));
      }

      const resetBtn = container.querySelector('#warmupResetBtn');
      if (resetBtn) {
        resetBtn.addEventListener('click', () => this.resetWarmupTimer());
      }

      // 3. Stage Checklist & Cards
      container.querySelectorAll('.stage-check-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
          e.stopPropagation();
          const idx = parseInt(btn.getAttribute('data-check-idx'), 10);
          if (!isNaN(idx)) this.toggleWarmupStageComplete(idx);
        });
      });

      container.querySelectorAll('[data-stage-idx]').forEach(card => {
        card.addEventListener('click', () => {
          const idx = parseInt(card.getAttribute('data-stage-idx'), 10);
          if (!isNaN(idx)) {
            this.warmupTimer.currentStage = idx;
            this.warmupTimer.timeLeft = this.warmupTimer.stages[idx].duration;
            this.renderTabContent();
          }
        });
      });

      // 4. Claim Warmup Streak Button
      const claimBtn = container.querySelector('#claimWarmupStreakBtn');
      if (claimBtn) {
        claimBtn.addEventListener('click', () => this.claimWarmupStreak());
      }

      const resetStreakBtn = container.querySelector('#resetWarmupStreakBtn');
      if (resetStreakBtn) {
        resetStreakBtn.addEventListener('click', () => {
          if (confirm('Reset your daily warmup streak to 0?')) {
            this.resetWarmupStreak();
          }
        });
      }

      // 5. Building Callouts Filter Buttons
      container.querySelectorAll('.callout-filter-btn').forEach(fBtn => {
        fBtn.addEventListener('click', () => {
          const filter = fBtn.getAttribute('data-filter');
          if (filter) {
            this.calloutActiveFilter = filter;
            this.renderTabContent();
          }
        });
      });

      // 6. Building Callouts Card Selection
      container.querySelectorAll('.callout-card-btn').forEach(cBtn => {
        cBtn.addEventListener('click', () => {
          const cardId = cBtn.getAttribute('data-card-id');
          if (cardId) {
            this.calloutActiveCard = cardId;
            this.renderTabContent();
          }
        });
      });

      // 7. Copy Building Comms Script Button
      container.querySelectorAll('.copy-building-comms-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          const comms = btn.getAttribute('data-comms');
          if (comms && navigator.clipboard) {
            navigator.clipboard.writeText(comms).then(() => {
              const orig = btn.textContent;
              btn.textContent = 'Copied Comms! ✅';
              setTimeout(() => { btn.textContent = orig; }, 1500);
            });
          }
        });
      ...
      // 8. Smart Backpack Role & Level Switchers
      container.querySelectorAll('.backpack-role-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          const role = btn.getAttribute('data-role');
          if (role) {
            this.backpackState.role = role;
            this.renderTabContent();
          }
        });
      });

      container.querySelectorAll('.backpack-level-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          const level = parseInt(btn.getAttribute('data-level'), 10);
          if (level) {
            this.backpackState.level = level;
            this.renderTabContent();
          }
        });
      });

      // 9. Clutch IQ Scenario Switcher
      container.querySelectorAll('.clutch-scenario-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          const idx = parseInt(btn.getAttribute('data-scenario-idx'), 10);
          if (!isNaN(idx)) {
            this.clutchTrainer.activeScenario = idx;
            this.clutchTrainer.selectedChoice = null;
            this.renderTabContent();
          }
        });
      });

      // 10. Clutch IQ Choice Click
      container.querySelectorAll('.clutch-choice-card').forEach(card => {
        card.addEventListener('click', () => {
          const choiceId = card.getAttribute('data-choice-id');
          if (choiceId) {
            this.clutchTrainer.selectedChoice = choiceId;
            this.playRadioBeep();
            this.renderTabContent();
          }
        });
